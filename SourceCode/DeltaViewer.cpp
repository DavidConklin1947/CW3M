...
BOOL DeltaViewer::OnInitDialog()
   ...
   m_hItemEvalModels = m_tree.InsertItem( "Eval Models" );
   m_hItemAutoProcs  = m_tree.InsertItem( "Auto Processes" );
   BOOL ok = m_tree.SetCheck( m_hItemEvalModels, 1 );
   ...
   for ( int i=0; i < gpModel->GetModelCount(); i++ )
      ...
      CString label;
      label.Format( "%s (%i)", pInfo->name, pInfo->handle );
      HTREEITEM hItem = m_tree.InsertItem( label, m_hItemEvalModels );
...
void DeltaViewer::OnNMClickTree(NMHDR *pNMHDR, LRESULT *pResult)
   ...
   if ( item && (flags & TVHT_ONITEMSTATEICON))
      ...
      if ( item == m_hItemEvalModels || item ==  m_hItemAutoProcs )
         {
...
void DeltaViewer::Load()
   ...
   bool showEvalModels = m_tree.GetCheck( m_hItemEvalModels ) ? true : false;
   ...
   ftp->showEvalModels  = showEvalModels;
   ftp->showProcesses   = showProcesses;
   ftp->showOther       = showOther;

   // do models
   int evalModelCount = EnvModel::GetModelCount();
   ...
   if ( evalModelCount > 0 )
      ...
      HTREEITEM hChildItem = m_tree.GetChildItem( m_hItemEvalModels );
      while ( hChildItem != NULL )
         {
...
UINT DeltaViewer::FilterByOpts( LPVOID pParam )
   ...
   int colLulcD    = EnvModel::m_colLulcD;
   ...
   bool showEvalModels  = params->showEvalModels;
   ...
   if ( pItem->mask & LVIF_TEXT )
      ...
      if ( pItem->iSubItem == 0)
         ...
         if ( col == colPolicyID && showPolicies )
            ...
            text.Format( "%s (%i)", (LPCSTR) pPolicy->m_name, policyID );
            }

         else if ( col == colLulcA && showLulc  )
            text.Format( "[%s] Change", EnvModel::m_lulcTree.GetFieldName( 1 ) );
         else if ( col == colLulcB && showLulc )
            text.Format( "[%s] Change", EnvModel::m_lulcTree.GetFieldName( 2 ) );
         else if ( col == colLulcC && showLulc )
            text.Format( "[%s] Change", EnvModel::m_lulcTree.GetFieldName( 3 ) );
         else if ( col == colLulcD && showLulc )
            text.Format( "[%s] Change", EnvModel::m_lulcTree.GetFieldName( 4 ) );
         else
      ...
      //-- STARTING VALUE --//
        ...
        if ( col == colPolicyID )
           ...
           if ( value >= 0 )
               ...
               text.Format( "%s (%i)", (LPCSTR) pOldPolicy->m_name, value );
               }
            else
            ...
            ASSERT( pNode != NULL );
            char name[ 128 ];
            sprintf_s( name, 128, "%s (%i)", (LPCSTR) pNode->m_name, value );
            text = name;
            }
         else
            {
      ...
      //-- ENDING VALUE --//
         ...
         if ( col == colPolicyID )
            ...
            if ( value >= 0 )
               ...
               char policy[ 128];
               sprintf_s( policy, 128, "%s (%i)", (LPCSTR) pNewPolicy->m_name, value );
               text = policy;
               }
            else
               text = "<no prior policy>";
      ...
      // -- Policy Site Constraints --//
         ...
         if ( col == colPolicyID )
            ...
            if ( value >= 0 )
               {
               Policy *pNewPolicy = gpPolicyManager->GetPolicyFromID( value );
               text = (LPSTR)(LPCSTR)pNewPolicy->m_siteAttrStr;
               }
            else
               text = "<n/a>";
            }
      ...
      //-- Policy Outcome" --//
         ...
         if ( col == colPolicyID )
            ...
            if ( value >= 0 )
               {
               Policy *pNewPolicy = gpPolicyManager->GetPolicyFromID( value );
               text = (LPSTR)(LPCSTR)pNewPolicy->m_outcomeStr;
               }
            else
               text = "<n/a>";
            }
...
//   CWnd *pEvalModel  = GetDlgItem( IDC_EVALMODEL );
...
//   pEvalModel->MoveWindow( spacing, heightDlg - 3*(spacing+heightButton), widthButton, heightButton );
