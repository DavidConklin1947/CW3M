...
bool CEnvApp::ProcessEnvCmdLineArgs( EnvCommandLineInfo &cmdInfo )
   ...
   CLI *pCmdLoadProject = cmdInfo.FindCmd( 0 );
   CLI *pCmdExport      = cmdInfo.FindCmd( 1 );
   CLI *pCmdQuery       = cmdInfo.FindCmd( 2 );
...
void EnvCommandLineInfo::ParseParam(const TCHAR* pszParam, BOOL isSwitch, BOOL bLast)
   {
   // /c:cmdInfo.xml
   // /r:scnIndex    run using specified scenario
   ...
   if ( isSwitch )
      ...
      switch( *command  )
         {
         case 'e':
            m_cmds.Add( new CLI( 1, arg ) );
            break;
   
         case 'q':
            m_cmds.Add( new CLI( 2, arg ) );
            break;
   
         case 'o':
            m_cmds.Add( new CLI( 3, arg ) );
            break;
   
         case 'c':
            m_cmds.Add( new CLI( 4, arg ) );
            break;
   
         case 'r':
            m_cmds.Add( new CLI( 5, arg ) );
            break;
 
         case 'm':
            m_cmds.Add( new CLI( 6, arg ) );
            break;
