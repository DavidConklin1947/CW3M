...
int CLogbook::AddLine(const LOGBOOKITEM_INFO &stInfo, const SYSTEMTIME *pstST, BOOL bDisplayTime, BOOL bDisplayType)
 {
  ...
  nRetGUI = nRet;
  Unlock();
  CString msg; msg.Format("m_pLogbookFile->AddLine() returned nRetFile = %d\n", nRetFile);
  OutputDebugString(msg);
  msg.Format("m_pLogbookGUI->AddLine() returned nRetGUI = %d\n", nRetGUI);
  OutputDebugString(msg);
...
BOOL CLogbook::AttachFile(LPCTSTR lpszPathName, LPCTSTR lpszFileName)
{
	DetachFile();

	if (!m_bInitialized || !m_bFileInfos) return FALSE;

	m_pLogbookFile = new CLogbookFile(this, lpszPathName, lpszFileName);
