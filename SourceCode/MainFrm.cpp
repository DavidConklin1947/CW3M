...
int CLogPane::OnCreate(LPCREATESTRUCT lpCreateStruct)
   ...
   m_logbook.AttachFile( exePath, "\\logfile.txt");
   ...
   // ----- ATTACH FILE -----
//   m_logbook.AttachFile(exePath, "Envision_log.txt");
   PWSTR userPath_wchar;
   SHGetKnownFolderPath(FOLDERID_Documents, 0, NULL, &userPath_wchar);
   CString userPath_tchar = "C:\\Users\\Dave\\Desktop";
   m_logbook.AttachFile(userPath_tchar, "Envision_log.txt");
...
CMFCRibbonPanel *CMainFrame::AddViewPanel( CMFCRibbonCategory *pCategory )
   ...
   //pPanelView->Add(new CMFCRibbonButton(ID_VIEW_3D,      _T("3D"), 0, 0));
...
void CMainFrame::AddContextTab_RtViews()
   ...
   int vizCount = EnvModel::GetVisualizerCount();

   for ( int i=0; i < vizCount; i++ )
      {
      ENV_VISUALIZER *pInfo = EnvModel::GetVisualizerInfo( i );

      if ( pInfo->type & VT_RUNTIME )
         {         
         //pRtViewBtn->AddSubItem( new CMFCRibbonLabel(pLayer->m_name)); 
         pRtVizBtn->AddSubItem( new CMFCRibbonButton( ID_RTVIZ_BEGIN + i, pInfo->name ));
         id++;
         }
      }
...
void CMainFrame::AddModels()
   ...
   for ( int i=0; i < EnvModel::GetAutonomousProcessCount(); i++ )
      {
      if ( EnvModel::GetAutonomousProcessInfo( i )->setupFn != NULL )
         {
         CString str( EnvModel::GetAutonomousProcessInfo( i )->name );
         str += " Properties...";   
   ...
   for ( int i=0; i < EnvModel::GetModelCount(); i++ )
      {
      if ( EnvModel::GetModelInfo( i )->setupFn != NULL )
         {
         CString str( EnvModel::GetModelInfo( i )->name );
         str += " Properties...";   
...
void CMainFrame::OnCommandAP(UINT id)
   {
   ENV_AUTO_PROCESS *pInfo = EnvModel::GetAutonomousProcessInfo( id-ID_AUTONOMOUS_PROCESS );            
   ASSERT( pInfo != NULL );
...
void CMainFrame::OnCommandEM(UINT id)
   {
   ENV_EVAL_MODEL *pInfo = EnvModel::GetModelInfo( id-ID_EVAL_MODEL );            
   ASSERT( pInfo != NULL );
   ASSERT( pInfo->setupFn != NULL );
...
void CMainFrame::OnCommandExtModule(UINT id)
   {
   ENV_ANALYSISMODULE *pInfo = gpDoc->GetAnalysisModule( id-ID_EXT_ANALYSISMODULE );            
   ASSERT( pInfo != NULL );
   ...
   e.ptrAddDelta    = NULL;
   pInfo->runFn( &e, gpView->m_hWnd, pInfo->initInfo ); 
   }
