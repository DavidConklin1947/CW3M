...
#pragma message(" - CDIBSection: No dithering")
#endif
#ifdef DIBSECTION_NO_MEMDC_REUSE
#pragma message(" - CDIBSection: No memory DC reuse")
#endif
#ifdef DIBSECTION_NO_PALETTE
#pragma message(" - CDIBSection: No palette support")
#endif


// Standard colors
RGBQUAD CDIBSection::ms_StdColors[] = {
...
/*static*/ int CDIBSection::NumColorEntries(int nBitsPerPixel, int nCompression, DWORD biClrUsed /*= 0*/)
{
    int nColors = 0;
...
//               in the bitmap
// --- Effect : 
/*static*/ int CDIBSection::BytesPerLine(int nWidth, int nBitsPerPixel)
{
...
/*static*/ BOOL CDIBSection::CreateHalftonePalette(CPalette& palette, int nNumColors)
{
...
/////////////////////////////////////////////////////////////////////////////
// CDIBSection diagnostics

#ifdef _DEBUG
void CDIBSection::AssertValid() const
{
    ASSERT(m_hBitmap);
...
void CDIBSection::Dump(CDumpContext& dc) const
{
    CObject::Dump(dc);
}
...
//              the GDI functions
BOOL CDIBSection::SetDither(BOOL bDither)
{
...
HDRAWDIB CDIBSection::GetDrawDibContext()
{
    if (!GetDither())
        return NULL;
...
BOOL CDIBSection::Draw(CDC* pDC, CPoint ptDest, BOOL bForceBackground /*=FALSE*/) 
{ 
    if (!m_hBitmap)
        return FALSE;
...
BOOL CDIBSection::Stretch(CDC* pDC, CPoint ptDest, CSize size,
...
BOOL CDIBSection::SetBitmap(UINT nIDResource)
{
...
// --- Effect : Initialises the bitmap from a resource. If failure, then object is
//              initialised back to an empty bitmap.
BOOL CDIBSection::SetBitmap(LPCTSTR lpszResourceName)
{
...
BOOL CDIBSection::SetBitmap(LPBITMAPINFO lpBitmapInfo, LPVOID lpBits)
{
    DeleteObject();
...
BOOL CDIBSection::Load(LPCTSTR lpszFileName)
{
    CFile file;
...
BOOL CDIBSection::Save(LPCTSTR lpszFileName)
{
...
BOOL CDIBSection::Copy(CDIBSection& Bitmap) 
{ 
...
BOOL CDIBSection::SetColorTable(UINT nNumColors, RGBQUAD *pColors)
{
...
BOOL CDIBSection::CreatePalette()
{
    m_Palette.DeleteObject();
...
BOOL CDIBSection::SetPalette(CPalette* pPalette)
{
    m_Palette.DeleteObject();
...
BOOL CDIBSection::SetLogPalette(LOGPALETTE* pLogPalette)
{
    if (!pLogPalette)
    {
        CreatePalette();
...
// --- Effect : Sets the colors used by the image. Only works if # colors <= 256
BOOL CDIBSection::FillDIBColorTable(UINT nNumColors, RGBQUAD *pRGB)
{
...
//              a given CDIBSection object. If you have a memDC but wish to recreate it
...
CDC* CDIBSection::GetMemoryDC(CDC* pDC /*=NULL*/, BOOL bSelectPalette /*=TRUE*/)
{
#ifdef DIBSECTION_NO_MEMDC_REUSE
    ReleaseMemoryDC(TRUE);
#else
