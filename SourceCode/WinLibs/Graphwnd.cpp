...
int GraphWnd::GetLabelText( MEMBER _label, LPSTR titleText, int maxCount )
   ...
   titleText[ strlen( (LPCSTR) label[ _label ].text ) ] = '\0';
...
void GraphWnd::DrawTitle( CDC *pDC )
   ...
   if ( ! label[ TITLE ].text.IsEmpty() )
      ...
      pDC->TextOut( xMidPt, label[TITLE].rect.top, (LPCSTR) label[TITLE].text, label[ TITLE ].text.GetLength() );
   ...
   for ( i=XAXIS; i <= YAXIS; i++ )
      ...
      if ( ! label[ i ].text.IsEmpty() )
         ...
         if ( orientation == OR_HORZ )
            ...
            pDC->TextOut( xMidPt, label[i].rect.top, (LPCSTR) label[i].text, label[i].text.GetLength() );
            }

         else
            {
            int yMidPt = ( label[i].rect.top + label[i].rect.bottom ) / 2;
            pDC->TextOut( label[i].rect.left-40,  yMidPt, (LPCSTR) label[i].text, label[i].text.GetLength() );
            }
   ...
   for ( i=0; i < (int) m_lineArray.GetSize(); i++ )
      ...
      if ( ! pLine->text.IsEmpty() && ( pLine->showLine || pLine->showMarker ) )
         ...
         pDC->TextOut( pLine->rect.left+m_legendRowHeight+10, pLine->rect.top,
               (LPCSTR) pLine->text, pLine->text.GetLength() );
...
bool GraphWnd::SetLegendText( int lineNo, LPCSTR newText )
   {
...
bool GraphWnd::SetLabelText( MEMBER member, LPCSTR newText )
   {
