      ...
      if ( pInfo->use )
         ...
         if ( initRunFn != NULL )
            ...
            if ( !ok )
               {
               CString msg;
               msg.Format( "The APInitRun(.) function of %s returned false.  This autonomous process will not be used.", pInfo->name );
...
void Sandbox::PushModels()
   ...
   for ( int i=0; i < GetModelCount(); i++ )
      {
      if ( GetModelInfo(i)->use )
         {
         PUSHFN pushFn = GetModelInfo(i)->pushFn;

         m_envContext.pExtensionInfo = GetModelInfo( i );
...
void Sandbox::PopModels()
   ...
   for ( int i=0; i < GetModelCount(); i++ )
      ...
      if ( GetModelInfo(i)->use )
         {
         POPFN popFn = GetModelInfo(i)->popFn;
         m_envContext.pExtensionInfo = GetModelInfo( i );
...
void Sandbox::CalculateGoalScores( Policy *pPolicy,  bool *useMetagoalArray /*=NULL*/ )
   ...
   for ( int year=0; year < yearsToRun; year++ )
      ...
      // call APs
      RunAutonomousProcesses( false );    // pre-year
      RunAutonomousProcesses( true );     // post-year
   ...
   for ( int g=0; g < pPolicy->GetGoalCount(); g++ )
      ...
      if ( useMetagoalArray == NULL || useMetagoalArray[ g ] == true )
         {
         ASSERT( EnvModel::GetMetagoalCount() == pPolicy->GetGoalCount() );
         int modelIndex = EnvModel::GetModelIndexFromMetagoalIndex( g );
         if ( modelIndex >= 0 )
            {
   ...
   void Sandbox::CalculateGoalScores(  CArray<Policy *>  & schedPolicy, Policy *pPolicy, EvalModelLearningStats * emStats,  bool * useModelArray)
      {
      ...
      for ( m_currentYear=0; m_currentYear < emStats->m_yearsToRun; m_currentYear++ )
         ...
         if (1 == emStats->DoEvalYearBase0(m_currentYear))
            ...
            for ( g=0, idx=0; g < EnvModel::GetModelCount(); g++ )
               {
