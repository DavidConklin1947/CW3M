...
RtViewWnd *RtViewPage::AddView( RTV_TYPE rtvType, INT_PTR extra, int run, CRect &viewRect )
   ...
   switch( rtvType )
      ...
      case RTVT_VISUALIZER: // custom types
         {
         ENV_VISUALIZER *pVizInfo = (ENV_VISUALIZER*) extra;  
         ASSERT( pVizInfo != NULL );

         // create the view
         RtVisualizer *pRtVisualizer = new RtVisualizer( this, run, pVizInfo, viewRect );
         pRtVisualizer->Create( NULL, pVizInfo->name, CS_HREDRAW | CS_VREDRAW | WS_CHILD | WS_VISIBLE | WS_OVERLAPPEDWINDOW | WS_CLIPCHILDREN | WS_CLIPSIBLINGS, rect, this, RtViewPanel::m_nextID++ );
		 
         // Note: No InitWindow needed for RtViz

         // create visualizer window (child of the RtViewWnd)
         BOOL success;
         VisualizerWnd *pVizWnd = gpView->m_vizManager.CreateWnd( pRtVisualizer, pVizInfo, run, success );
         pVizWnd->InitWindow();
...
void RtViewPanel::OnCommandViz( UINT id )
   ...
   ENV_VISUALIZER *pInfo = EnvModel::GetVisualizerInfo( index );
...
void RtViewPanel::OnUpdateViz(CCmdUI *pCmdUI)
   {
   pCmdUI->Enable( EnvModel::GetVisualizerCount() > 0 ? 1 : 0 );
   }
