...
int DataPanel::AddDataTable( DbTable *pData )
   {
   if ( pData == NULL || pData->m_pData == NULL )
      return -1;

   DbTableDataSource *pDS = new DbTableDataSource( pData );
   m_dataSourceArray.Add( pDS );

   m_dataSources.AddString( pData->m_tableName );
...
void DataPanel::OnCbnSelchangeDatasources()
   ...
   CString dataSource;
   m_dataSources.GetWindowText( dataSource );
   msg += dataSource;
