...
void OutcomeConstraintsDlg::LoadOutcomeConstraint()
   ...
   if ( m_outcomeConstraintIndex >= 0 )
      ...
      this->m_duration = pConstraint->m_duration;
...
void OutcomeConstraintsDlg::OnBnClickedAddgc()
   ...
   if ( dlg.DoModal() == IDOK )
      ...
      m_globalConstraintIndex = (int) m_constraintArray.Add( new GlobalConstraint( dlg.m_name, CT_RESOURCE_LIMIT, 0 ) );
...
void OutcomeConstraintsDlg::OnEnChangeDuration()
   ...
   pConstraint->m_duration = GetDlgItemInt( IDC_MAINTENANCECOST );
