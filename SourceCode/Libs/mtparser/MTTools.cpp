...
bool MTTools::findSubStr(const MTSTRING &str, const MTSTRING &subStr, unsigned int &ipos)
//bool MTTools::findSubStr(const MTSTRING &str, const MTSTRING &subStr, unsigned int &pos)
{
	size_t pos = size_t(ipos);              // cast to size_t
	size_t strLength = str.size();                //unsigned int strLength = str.size();
	size_t subStrLength = subStr.size();    //unsigned int subStrLength = subStr.size();
	...
	size_t end = strLength - subStrLength;   //unsigned int end = strLength - subStrLength;

	for (size_t t = 0; t <= end; t++)    //for( unsigned int t=0; t <= end; t++ )
	{
		if( isStrBegin(str.c_str()+t, subStr.c_str(), strLength-t, subStrLength ) ) // MIX OF SIZE_T AND UNSIGNED INT
		{
			pos = t;
			return true;
		}
...
{
	size_t length = str.size();           //unsigned int length = str.size();
	newStr = _T("");
...
void MTTools::parseString(const MTSTRING &str, const MTCHAR &c, std::vector<MTSTRING> &tokens)
{
	...
	while( pos < length )
	{
		if( !findCharPos(str, pos, c, separatorPos) )
		{
			...
			separatorPos = static_cast<unsigned int>(length);	// end of the string 
		}
...
{	
	size_t length = word.size();     //unsigned int length = word.size();
	...
	for (size_t t = 0; t<length; t++)  //for( unsigned int t=0; t<length; t++ )
	{
...
bool MTTools::registerTypeLib(const MTCHAR *file)
{
	...
	return ret;
#else
	//disable for linux (not used by Envision
	return false;
#endif
}
