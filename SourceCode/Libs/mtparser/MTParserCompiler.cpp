...
{	
	...
	return (unsigned int)( m_pStates.size()-1 );
}
...
void MTParserCompiler::clearUsedVars()
{
	...
	unsigned int nbUsedVars = (unsigned int) m_usedVariables.size();	
...
{
  int lastID = (unsigned int)( m_curItemTypeStack.size()-1 );
...
{
	int lastID = int( m_curItemTypeStack.size()-1 );
...
{	
	...
	if( m_curPos < m_expression.length() &&  m_expression[m_curPos] == MTOPEN_BRACKET )
	{
		// it's a function
