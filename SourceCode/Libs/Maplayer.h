...
struct LIBSAPI FIELD_ATTR
   ...
   FIELD_ATTR(VData &_value, LPCSTR _label, COLORREF _color, float _minVal, float _maxVal, int _transparency)
...
struct LIBSAPI MAP_FIELD_INFO
   ...
   public:
      ...
      // manage attributes
      int AddAttribute(VData &value, LPCSTR label, COLORREF color, float minVal, float maxVal, int transparency)
         {
...
class LIBSAPI FieldInfoArray : public CArray< MAP_FIELD_INFO*, MAP_FIELD_INFO* >
   ...
   public:
      ...
      MAP_FIELD_INFO *FindFieldInfo(LPCSTR fieldname, int *pIndex = NULL)const;
      MAP_FIELD_INFO *FindFieldInfoFromLabel(LPCSTR label)const;
   };
...
class  LIBSAPI  MapLayer
   ...
   public:
	  ...
	  bool SetBoundingRect(REAL vxMin, REAL vyMin, REAL vxMax, REAL vyMax);

   public:
      // array of flags for Field information
      REAL m_vxMin;    // virtual coord
      REAL m_vxMax;
      REAL m_vyMin;
      REAL m_vyMax;
      REAL m_vzMin;
   ...
   public:
      ...
      int LoadFieldInfoXml(LPCSTR _filename, bool addExtraFields);
   protected:
   ...
   public:
      ...
      MAP_FIELD_INFO *FindFieldInfo(LPCSTR field);           // field is the name of the database field 
      MAP_FIELD_INFO *FindFieldInfoFromLabel(LPCSTR label);  // label is the extended field descriptor
   ...
   public:
      ...
      //int   PopulateNearestDistanceToCoverage( MapLayer *pToLayer, int col, REAL threshold, bool selectedOnlyTo=false, bool selectedOnlyThis=false, LPCSTR label=NULL, int colNearestToIndex=-1 );
      int   PopulateNearestDistanceToCoverage(MapLayer *pToLayer, int col, REAL threshold, Query *pToQuery = NULL, Query *pThisQuery = NULL, LPCSTR label = NULL, int colNearestToIndex = -1, int colNearestToValue = -1, int colNearestSourceValue = -1);
      //int   PopulateNumberNearbyPolys( MapLayer *pToLayer, int col, REAL threshold, bool selectedOnlyTo=false, bool selectedOnlyThis=false, LPCSTR label=NULL, REAL nearThreshold=10 );
      int   PopulateNumberNearbyPolys(MapLayer *pToLayer, int col, REAL threshold, Query *pToQuery = NULL, Query *pThisQuery = NULL, LPCSTR label = NULL, REAL nearThreshold = 10);

      int   PopulateLengthOfIntersection(MapLayer *pToLayer, int col, bool selectedOnlyTo = false, bool selectedOnlyThis = false, LPCSTR label = NULL);
      //int   PopulateSelectedLengthOfIntersection( MapLayer *pToLayer, int col, int colToSelectFrom, LPCSTR label=NULL  );  // Not Implimented
    ...
    /*  void  SetCellSize(float width, float height) { m_cellWidth = width; m_cellHeight = height; }
      ...
      int  SaveDataDB(LPCSTR path, BitArray *pSaveArray = NULL) const { return m_pDbTable->SaveDataDBF(path, pSaveArray); }
	  ...
	  void SetDataU(int rec, int col, double value); // Set data unconditionally (i.e. even if m_readOnly is true)
	  void SetDataU(int rec, int col, int value); // Set data unconditionally (i.e. even if m_readOnly is true)
	  void SetDataU(int rec, int col, bool value); // Set data unconditionally (i.e. even if m_readOnly is true)
	  void SetDataU(int rec, int col, COleVariant &value); // Set data unconditionally (i.e. even if m_readOnly is true)
	  void SetDataU(int rec, int col, const VData &value);
   ...
   //   bool SetColData( int col, float value, bool includeNoData );    // deprecated, use VData version below
	  ...
	  bool SetColDataU(int col, VData value); // Set data unconditionally (i.e. even if m_readOnly is true, and with includeNoData==true)
      ...
      void SetBins(int col, float binMin, float binMax, int binCount, TYPE type, BSFLAG flag = BSF_LINEAR, BinArray *pExtBinArray = NULL, LPCSTR units = NULL);
      void SetBins(int col, CStringArray &, BinArray *pExtBinArray = NULL, TYPE type = TYPE_STRING, LPCSTR units = NULL);  // for string bins
      void SetBins(MapLayer *pSourceLayer, int sourceCol);

      int  SetUniqueBins(int col, TYPE type = TYPE_STRING, BinArray *pExtBinArray = NULL, LPCSTR units = NULL);
   ...
   public:
	  ...
	  Poly* AddNewPolyline(Poly &polyGeometry, const CUIntArray &parentList); // returns ptr to the new Poly
	  Poly* AddNewPoint(Poly &polyGeometry, const CUIntArray &parentList); // returns ptr to the new Point
	//  void  DeletePoly(int polyIndex);
...
inline
void MapLayer::SetDataU(int rec, int col, COleVariant &value)
{
	ASSERT(m_pData != NULL);
	ASSERT(rec >= 0 && rec < m_pData->GetRowCount());
	ASSERT(col >= 0 && col < m_pData->GetColCount());

	bool read_only_flag = m_readOnly;
	m_readOnly = false;
	ASSERT(m_pData != NULL);
	m_pData->Set(col, rec, value);
	m_readOnly = read_only_flag;
}
...
inline
void MapLayer::SetDataU(int rec, int col, double value) // Set data unconditionally
{
	ASSERT(m_pData != NULL);
	ASSERT(rec >= 0 && rec < m_pData->GetRowCount());
	ASSERT(col >= 0 && col < m_pData->GetColCount());
...
inline
void MapLayer::SetDataU(int rec, int col, int value) // Set data unconditionally
{
	ASSERT(m_pData != NULL);
	ASSERT(rec >= 0 && rec < m_pData->GetRowCount());
	ASSERT(col >= 0 && col < m_pData->GetColCount());
...
inline
void MapLayer::SetDataU(int rec, int col, bool value) // Set data unconditionally
{
	ASSERT(m_pData != NULL);
	ASSERT(rec >= 0 && rec < m_pData->GetRowCount());
	ASSERT(col >= 0 && col < m_pData->GetColCount());
	...
	bool read_only_flag = m_readOnly;
	m_readOnly = false;
	COleVariant v((short)value);
	SetDataU(rec, col, v);
	m_readOnly = read_only_flag;
} // end of SetDataU()

inline
void MapLayer::SetDataU(int rec, int col, const VData &value)
{
	ASSERT(m_pData != NULL);
	ASSERT(rec >= 0 && rec < m_pData->GetRowCount());
	ASSERT(col >= 0 && col < m_pData->GetColCount());
...
Bin *MapLayer::AddBin(int col, COLORREF color, LPCTSTR label, TYPE binType, float binMin, float binMax)
   ...
   if (::IsInteger(binType))
      bin.m_strVal.Format("%i", (int)((binMin + binMax) / 2));
   else if (::IsReal(binType))
      bin.m_strVal.Format("%g", (binMin + binMax) / 2);
