...
bool GDALWrapper::Init( void )
   ...
   CString gdal_lib;
   // was    gdal_lib = "C:\\Envision\\GDAL\\release-1600-x64\\bin\\gdal18.dll";
   // now    gdal_lib = "C:\\Program Files\\GDAL\\gdal111.dll";
   //  gdal_lib = "gdal111.dll"; 
   ...
   gdal_lib = "gdal19.dll";
  #endif
   m_hInst = ::AfxLoadLibrary(gdal_lib);
   if (m_hInst == NULL)
   { // Didn't find it.  Try in C:\ProgramFiles (x86)\WillametteINFEWSmodel
      CString path_cstr; path_cstr = gdal_lib;
      CString alt_path = "C:\\Program Files (x86)\\WillametteINFEWSmodel\\" + path_cstr;
      m_hInst = LOAD_LIBRARY(alt_path);
      gdal_lib = alt_path;
   }

   if ( m_hInst == 0 )
      {
      CString msg; msg.Format("GDALWrapper: Unable to find GDAL binaries at %s", gdal_lib);    
      Report::ErrorMsg( msg );
      return false;
      }
  #if defined( _WIN64 )
