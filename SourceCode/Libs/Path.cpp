...
public:
    CStringLock(CString & s) : m_string(&s)
    {
        ...
        // fixes an incompatibility with CString, see "NullBuffer" in .cpp 
        if (!s.GetLength())
            m_buffer = NullBuffer; 
    }
...
/// CString incompatibility:
/// http://www.codeproject.com/string/stdstring.asp
/// If the contained string is empty, CString.GetBuffer returns a pointer to a constant
...
//
//    CString s;
//    result = GetEnvironmentVariable(envVar, CString(s, result), result);
//    return s;
//}
