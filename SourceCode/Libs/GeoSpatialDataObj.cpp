...
bool GeoSpatialDataObj::Open(LPCTSTR filename, LPCTSTR variable)
   ...
   if (m_datasetH == NULL)
      ...
      msg.Format(_T("GeoSpatialDataObj: Unable to open/read file '%s', check path, variable, and/or filename."), (LPCTSTR)m_GeoFilename);
      Report::ErrorMsg(msg);
      return false;
      }
   //m_geoTransform = new double[6];   
...
float GeoSpatialDataObj::Get(double xcoord, double ycoord, int &twoDindexvalue, int set3Dvalue, CString prjFile, bool isUTM, bool *pOKflag)
{
	*pOKflag = true;
	...
	if (isUTM)
	{
		utmzone = GetUTMZoneFromPrjWKT(prjFile);
		northing = ycoord;
		easting = xcoord;
	}
	else
	{
		...
		utmzone = (int)floor((LongTemp + 180) / 6) + 1;
	...
	{
		CString msg;
		msg.Format(_T("GeoSpatialDataObj::Get(double xcoord,double ycoord) Coord2LatLong failed. xcoord = %f, ycoord = %f"), xcoord, ycoord);
		Report::ErrorMsg(msg);
		*pOKflag = false;
		return(0.f);
	}
	...
	xx = (int)floor(m_geox);
	// if (xx >= 8640) xx -= 8640; // This is a kludge specifically for the 2.5 minute grid used by WW2100.
	yy = (int)floor(m_geoy);
	...
	{
		CString msg;
		msg.Format("GeospatialDataObject::Get(): Bad centroid (%g,%g) passed in; prjFile = %s, isUTM = %d, xx = %d, yy = %d, m_nXSize = %d, m_nYSize = %d, utmzone = %d"
			", lat = %f, lng = %f",
			(float)xcoord, (float)ycoord, prjFile, isUTM, xx, yy, m_nXSize, m_nYSize, utmzone, lat, lng);
		Report::ErrorMsg(msg);
		*pOKflag = false;
		return(0.f);
	}
...
bool GeoSpatialDataObj::Set(int col, int row, LPCSTR value)
   {
