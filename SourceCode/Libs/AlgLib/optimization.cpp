        ...
        for(;;)
        {
            ...
            if( ndc>0 )
            {
                ...
                for(;;)
                {
                     ...
                     */
                    s->debugflops = s->debugflops+2*(nr-nsc)*ae_sqr((double)ndc, _state)+ae_pow((double)ndc, 3, _state)/3;
                    for(i=0; i<=ndc-1; i++)
                    {
                        for(j=0; j<=ndc-1; j++)
                        {
    ...
    if( state->algokind==1 )
    {
        ...
        for(;;)
        {
            ...
            for(;;)
            {
                 ...
                 */
                state->repncholesky = state->repncholesky+1;
                state->debugphase3flops = state->debugphase3flops+ae_pow((double)n, 3, _state)/3;
