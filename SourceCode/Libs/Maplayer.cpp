...
MAP_FIELD_INFO *FieldInfoArray::FindFieldInfo(LPCSTR fieldname, int *pIndex /*=NULL*/) const
   {
   for (int i = 0; i < GetSize(); i++)
      {
...
MAP_FIELD_INFO *FieldInfoArray::FindFieldInfoFromLabel(LPCSTR label) const
   {
   for (int i = 0; i < GetSize(); i++)
      {
...
bool MapLayer::SetBoundingRect(REAL vxMin, REAL vyMin, REAL vxMax, REAL vyMax) 
{
	 m_vxMin = vxMin;
	 m_vxMax = vxMax;
	 m_vyMin = vyMin;
...
void MapLayer::SetBins(int col, float binMin, float binMax, int binCount, TYPE type, BSFLAG bsFlag, BinArray *pExtBinArray, LPCSTR units)
   {
   if (col == USE_ACTIVE_COL)
      col = m_activeField;
...
void MapLayer::SetBins(int col, CStringArray &valueArray, BinArray *pExtBinArray/*=NULL*/, TYPE type/*=TYPE_STRING*/, LPCSTR /*units=NULL*/)
   {
   BinArray *pBinArray = NULL;
...
int MapLayer::SetUniqueBins(int col, TYPE type, BinArray *pExtBinArray /*=NULL*/, LPCSTR units /*=NULL*/)
   {
...
MAP_FIELD_INFO *MapLayer::FindFieldInfo(LPCSTR fieldname)
   {
   return m_pFieldInfoArray->FindFieldInfo(fieldname);
   }

MAP_FIELD_INFO *MapLayer::FindFieldInfoFromLabel(LPCSTR label)
   {
...
bool MapLayer::SetColDataU(int col, VData value)
{
	if (m_pDbTable == NULL)
		return false;
	...
	for (int row = 0; row < rows; row++) SetDataU(row, col, value);
...
int MapLayer::PopulateNearestDistanceToCoverage(MapLayer *pToLayer, int col, REAL threshold, Query *pToQuery /*= NULL*/, Query *pThisQuery/*=NULL*/, LPCSTR label/*=NULL*/, int colNearestToIndex /*=-1*/, int colNearestToValue /*=-1*/, int colNearestSourceValue /*=-1*/)
   {
...
int MapLayer::PopulateNumberNearbyPolys(MapLayer *pToLayer, int col, REAL threshold, Query *pToQuery /*=NULL*/, Query *pThisQuery/*=NULL*/, LPCSTR label /*=NULL*/, REAL nearThreshold /*=-1*/)
   {
   if (m_layerType != LT_LINE && m_layerType != LT_POLYGON && m_layerType != LT_POINT)
      return -1;
...
int MapLayer::PopulateNearestDistanceToSelectedCoverage( MapLayer *pToLayer, int col, REAL threshold, int colToSelectFrom, VData &value, LPCSTR label  )
   {
   if ( m_layerType != LT_LINE && m_layerType != LT_POLYGON )
      return -1;
...
int MapLayer::PopulateLengthOfIntersection(MapLayer *pToLayer, int col, bool selectedOnlyTo /*=false*/, bool selectedOnlyThis /*=false*/, LPCSTR label /*=NULL*/)
   {
   if (m_layerType != LT_LINE && m_layerType != LT_POLYGON)
      return -1;
...
Poly* MapLayer::AddNewPolyline(Poly &polyGeometry, const CUIntArray &parentList)
{
	// This function does the following:
	//    * A new polygon, copy constructed from the arg, polyGeometry,
	//      is appended to end of m_pPolyArray->
	...
	ASSERT(polyGeometry.m_vertexArray.GetCount() >= 2);
	ASSERT(polyGeometry.m_vertexPartArray.GetCount() == 1);
	ASSERT(parentList.GetSize() < 2);
	...
	for (int i = 0; i < fieldCount; i++)
	{
		if (i == colArea) varArray[i] = 0.f;
		else varArray[i] = GetNoDataValue();
	}
	...
	return pPoly;
} // end of AddNewPolyline()


Poly* MapLayer::AddNewPoint(Poly &polyGeometry, const CUIntArray &parentList)
{
	// This function does the following:
	//    * A new point, copy constructed from the arg, polyGeometry,
	//      is appended to end of m_pPolyArray->
	//    * polyGeometry.m_id is set to the m_id of the new point
	//    * the new point is flagged as dead
	//    * A new record is created in m_pData with all fields set to GetNoDataValue()

	ASSERT(polyGeometry.m_vertexArray.GetCount() == 1);
	ASSERT(polyGeometry.m_vertexPartArray.GetCount() == 1);
	ASSERT(parentList.GetSize() < 2);
	...
	pPoly->m_xMin = (float)v.x;
	pPoly->m_xMax = (float)v.x;
	pPoly->m_yMin = (float)v.y;
	pPoly->m_yMax = (float)v.y;
...
int MapLayer::LoadFieldInfoXml(LPCSTR filename, bool addExtraFields)
   {
...
inline double IDUlayer::Att(int IDUpolyNdx, int col)
{
   double attribute;
...
inline int IDUlayer::AttInt(int IDUindex, int col)
{
   int attribute;
...
inline float IDUlayer::AttFloat(int IDUindex, int col)
{
   float attribute;
   GetData(IDUindex, col, attribute);
   return(attribute);
