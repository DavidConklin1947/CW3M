...
int IDataObj::AppendCol( LPCSTR label ) //, float *dataArray /*=NULL*/ )
   {
   int *dataArray = new int[ GetRowCount() ];
...
int IDataObj::InsertCol( int insertBefore, LPCSTR label )
   {
   int *dataArray = new int[ GetRowCount() ];
...
int IDataObj::ReadAscii( LPCSTR _fileName, TCHAR delimeter, BOOL showMsg )
   {
   CString fileName;
...
int IDataObj::WriteAscii( LPCSTR fileName, TCHAR delimiter, int colWidth )
   {
   ...
   if ( fp == NULL )
      {
      TCHAR buffer[ 128 ];
      strcpy_s( buffer, 128, "IDataObj: Couldn't open file " );
      ...
      if (fp == NULL)
         {
         CString msg; msg.Format("Couldn't open %s either.", newFileName);
         Report::WarningMsg(msg);
      return 0;
      }

      CString msg; msg.Format("Opened %s instead.\nRemember to change the file name back to %s!", newFileName.GetString(), fileName);
      Report::WarningMsg(msg);
