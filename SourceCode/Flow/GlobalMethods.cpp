...
CArray< float, float > GlobalMethod::m_iduVPDarray;
...
bool ExternalMethod::Step(FlowContext *pFlowContext)
{
   if (m_use == false)
      return true;
   ...
   switch (m_method)
   {
      ...
      case GM_EXTERNAL:
      {
         ...
         pFlowContext->m_extFnInitInfo = m_initInfo;
         pFlowContext->pFlowModel->m_parameter1 = m_parameter1;
         pFlowContext->pFlowModel->m_parameter2 = m_parameter2;
         m_extFn(pFlowContext);
      }
      return true;
...
bool GlobalMethod::CompileQuery( FlowContext *pFlowContext )
   ...
   if ( m_query.GetLength() > 0 && m_pQuery == NULL )
      ...
      if ( m_pQuery == NULL )
         {
         CString msg;
         msg.Format( "Flow:  Error compiling global method '%s' query '%s' - this query will be ignored", (LPCTSTR) m_name, (LPCTSTR) m_query );
         Report::ErrorMsg( msg );
         return false;
         }
      }
...
bool GlobalMethodManager::Init( FlowContext *pFlowContext )
   ...
   GlobalMethod::m_iduVPDarray.SetSize(iduCount);
   ...
   for ( int i=0; i < iduCount; i++ )
      ...
      GlobalMethod::m_iduVPDarray[i] = 0.f;
...
bool GlobalMethodManager::StartYear( FlowContext *pFlowContext )
   ...
   for ( int m=0; m < methodCount; m++ )
      {
      CString msg;
      msg.Format( "Flow Global Method '%s' query found  %i IDUs", (LPCTSTR) m_gmArray[ m ]->m_name, foundArray[ m ] );
      Report::LogMsg( msg );
      }
...
bool GlobalMethodManager::EndRun( FlowContext *pFlowContext )
   ...
   for ( int m=0; m < (int) m_gmArray.GetSize(); m++ )
      ...
      if ( pMethod != NULL )
         ...
         CString msg;
         msg.Format( "Flow Global Method '%s' Runtime: %.1f seconds", (LPCTSTR) pMethod->m_name, pMethod->m_runTime );
         Report::LogMsg( msg );
         }
      }
