...
#define NUM_OF_CLIMATE_GRID_ROWS (66)
#define NUM_OF_CLIMATE_GRID_COLUMNS (54)
#define NUM_OF_CLIMATE_GRIDCELLS (NUM_OF_CLIMATE_GRID_ROWS*NUM_OF_CLIMATE_GRID_COLUMNS)
#define LOWEST_POPULATED_GRIDCELL_INDEX 288
#define HIGHEST_POPULATED_GRIDCELL_INDEX 3493

/*! \file */ 
///\enum FLUXTYPE The type of flux
enum FLUXTYPE  { FT_UNDEFINED=0, FT_PRECIP=1  };
///\enum FLUXLOCATION The type of state variable that acts as the source for the flux
enum FLUXLOCATION   { FL_UNDEFINED=0, FL_REACH=1, FL_HRULAYER=2, FL_RESERVOIR=3, FL_GROUNDWATER=4, FL_IDU=5 };
///\enum FLUXSOURCE The method used to specify flux values
enum FLUXSOURCE { FS_UNDEFINED=0, FS_FUNCTION=1, FS_DATABASE=2 };
///\enum HRULAYER_TYPE A descriptor of the type of state variable represented by the HRULayer
enum HRULAYER_TYPE { HT_SOIL=0, HT_SNOW=1, HT_VEG=2 };
...
enum CDTYPE {
       ...
       CDT_SPHUMIDITY  = 64,
       CDT_WINDSPEED   = 128,
       CDT_UAS = 256, // eastward wind component
       CDT_VAS = 512, // northward wind component
       CDT_RHMIN = 1024, // minimum relative humidity
       CDT_RHMAX = 2048 }; // maximum relative humidity
...
enum WFAINDEX { // water_flux_array indexes for various HRULayer fluxes, used in AddFluxFromGlobalHandler
...
public:
   ...
   FlowContext &operator=( FlowContext &fc ) { pEnvContext = fc.pEnvContext; time = fc.time; timeStep = fc.timeStep;
         dayOfYear = fc.dayOfYear; pHRU = fc.pHRU; pHRULayer = fc.pHRULayer; pReach = fc.pReach; reachSubnodeIndex = fc.reachSubnodeIndex;
   ...
   LPCTSTR    initInfo;
   CString m_extFnInitInfo;
   FlowProcess * m_pFlowProcess;
   void Reset( void ) { pHRULayer=NULL; pHRU=NULL; pReach=NULL; pFlux=NULL; pFluxInfo=NULL; reachSubnodeIndex = -1; iduIndex = -1; }
