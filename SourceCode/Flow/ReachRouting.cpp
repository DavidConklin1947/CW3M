...
//       pSubreach->m_waterParcel.MixIn(subreach_precipWP); // ??? This violates conservation of mass because IDU surface areas overlap stream surface areas.
         pSubreach->m_waterParcel.Evaporate(pSubreach->m_evap_m3, pSubreach->m_evap_kJ);
...
// not yet         double subreach_kJ = pSubreach->m_waterParcel.ThermalEnergy(subreach_tempC) + pReach->SubReachNetRad_kJ(l);
         ...
         { // Theoretically, the water is losing heat to the atmosphere by radiation.
            ...
            if (pSubreach->m_waterParcel.WaterTemperature() > skin_temp_degC)
            { // The water is warmer than the air, so it is plausible that the water might cool down
               ...
               double temp_diff = pSubreach->m_waterParcel.WaterTemperature() - skin_temp_degC;
         ...
         {
            double thermal_energy_kJ = pSubreach->m_waterParcel.ThermalEnergy() + subreach_net_rad_kJ;
            pSubreach->m_waterParcel.m_temp_degC = WaterParcel::WaterTemperature(pSubreach->m_waterParcel.m_volume_m3, thermal_energy_kJ);

            if (l == 0 && pSubreach->m_waterParcel.m_temp_degC >= 50.)
            {
               CString msg;
               msg.Format("SolveReachKinematicWave() temperature >= 50 degC in reach %d subreach %d: orig kJ = %f,\n"
                  "pSubreach->m_sw_kJ = %f, pSubreach->m_lw_kJ = %f\n"
                  "pSubreach->m_subreach_surf_area_m2 = %f, pSubreach->m_volume_m3 = %f, pSubreach->m_subreach_manning_depth_m = %f, pSubreach->m_subreach_width_m = %f, m_temp_degC = %f",
                  pReach->m_reachID, l, pSubreach->m_waterParcel.ThermalEnergy(), pSubreach->m_sw_kJ, pSubreach->m_lw_kJ,
                  pSubreach->m_subreach_surf_area_m2, pSubreach->m_waterParcel.m_volume_m3, pSubreach->m_manning_depth_m, pSubreach->m_subreach_width_m, pSubreach->m_waterParcel.m_temp_degC);
               Report::LogMsg(msg);
            }
         }
         ASSERT(pSubreach->m_waterParcel.m_temp_degC >= 0);
