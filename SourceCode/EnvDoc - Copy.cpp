...
void SetBoolHtml( CString &html, LPCSTR label, bool value );

void NotifyProc( EM_NOTIFYTYPE type, INT_PTR, INT_PTR );
void NotifyProc( EM_NOTIFYTYPE type, INT_PTR param, INT_PTR extra)
   {
   switch( type )
      {
      case EMNT_IDUCHANGE:
      ...
      case EMNT_RUNVIZ:
         if ( param == 1 && gpModel->m_showRunProgress )  // in model loop?
            {
            ENV_EXTENSION *pInfo = (ENV_EXTENSION*) extra;
...
#ifndef _WIN64
void GetDBValue( CDaoRecordset &rs, LPCSTR field, long &value );
void GetDBValue( CDaoRecordset &rs, LPCSTR field, int &value );
void GetDBValue( CDaoRecordset &rs, LPCSTR field, float &value );
void GetDBValue( CDaoRecordset &rs, LPCSTR field, CString &value );
void GetDBValue( CDaoRecordset &rs, LPCSTR field, bool &value );
#endif 
...
bool CEnvDoc::InitDoc( void )
   ...
   Report::reportMsgProc = ReportProc;
   Report::statusMsgProc = StatusMsgProc;
   Report::msgWndProc    = ReportBoxProc;
...
void CEnvDoc::Clear()
   ...
   if ( gpScenarioManager )
      {
      delete gpScenarioManager;
      gpScenarioManager = NULL;
      }
...
/* --*/
   ...
   switch( levels )
      {
      case 1:  m_model.m_pIDULayer->CopyColData( EnvModel::m_colStartLulc, EnvModel::m_colLulcA );   break;
      case 2:  m_model.m_pIDULayer->CopyColData( EnvModel::m_colStartLulc, EnvModel::m_colLulcB );   break;
      case 3:  m_model.m_pIDULayer->CopyColData( EnvModel::m_colStartLulc, EnvModel::m_colLulcC );   break;
      case 4:  m_model.m_pIDULayer->CopyColData( EnvModel::m_colStartLulc, EnvModel::m_colLulcD );   break;
      }
...
int CEnvDoc::SaveFieldInfoXML( MapLayer *pLayer, LPCSTR filename )
   {
   // write header
   ofstream file;
   file.open( filename );
...
int CEnvDoc::SaveAs( LPCTSTR _filename  /* if NULL, prompt for path */ )
 ...
 //           overlayField = pLayer->GetFieldLabel( pLayer->m_overlayCol );
   ...
   fputs( "                                  M O D E L S \n", fp );
   fputs( "==================================================================================================\n", fp );
   fputs( " Note that a single DLL can contain multiple models, each must be specified on \n", fp );
   ...
   for ( int i=0; i < gpModel->GetModelCount(); i++ )
      {
      ENV_EVAL_MODEL *pInfo = gpModel->GetModelInfo( i );
   ...
   fputs( "<!--\n", fp );
   fputs( "===================================================================================\n", fp );
   fputs( "                      A U T O N O M O U S   P R O C E S S E S \n", fp );
   fputs( "===================================================================================\n", fp );
   ...
   fputs( "<autonomous_processes>\n", fp );

   for ( int i=0; i < gpModel->GetAutonomousProcessCount(); i++ )
      {
      ENV_AUTO_PROCESS *pInfo = gpModel->GetAutonomousProcessInfo( i );
...
/*
   ...
   for ( int i=0; i < EnvModel::GetModelCount(); i++ )
      ...
      if ( pInfo->apNpCdf.get()!= NULL )
         {
         string cdf;
         HistogramCDF::writeQuantiles( cdf, *pInfo->apNpCdf, (LPCTSTR) pInfo->name );
   ...
   for ( int i=0; i < EnvModel::GetMetagoalCount(); i++ )
      ...
      if ( pInfo->pEvalModel != NULL )
         fprintf( fp, "model ='%s' ", pInfo->pEvalModel->name );
...
void CEnvDoc::RunModel()
   ...
   gpMain->SetRunProgressRange( 0, cellCount*m_model.m_yearsToRun );
   gpMain->SetRunProgressPos( 0 );
...
int CEnvDoc::OpenDocXml( LPCTSTR filename )
   ...
   if ( gpMapPanel->m_pMap->GetLayerCount() == 0 )
      {
      AfxMessageBox( "No IDU coverage loaded - Envision can not continue... This is likely caused by a bad <layer> specification in your ENVX file", MB_OK );
      return -1;
      }
   ...
   for ( int i=0; i < gpMapPanel->m_pMap->GetLayerCount(); i++ )
      {
      MapLayer *pLayer = gpMapPanel->m_pMap->GetLayer( i );
      gpView->m_dataPanel.AddDataTable( pLayer->m_pDbTable );
      }
...
void CEnvDoc::OnExportPoliciesashtmldocument()
   ...
   //m_model.m_pIDULayer->SetActiveField( m_model.m_pIDULayer->GetFieldCol( EnvModel::m_lulcTree.GetFieldName( 1 ) ) );  //"LULC_A" ) );
   ...
   for ( int i=0; i < count; i++ )
      ...
      if ( pPolicy->m_pSiteAttrQuery == NULL )
         {
         m_model.m_pIDULayer->ClearQueryResults();
       ...
       if ( cSubKeys )
          ...
          for ( DWORD i=0; i < cSubKeys; i++ ) 
            ...
            if ( result == ERROR_SUCCESS) 
               ...
               ENV_ANALYSISMODULE *pInfo = new ENV_ANALYSISMODULE( hDLL, runFn, -1, 0, extModuleName, path, true );
               pInfo->initInfo = initInfo;
               pInfo->type = 0;
               pInfo->use = use;
...
int CEnvDoc::ReconcileDataTypes(void)
   ...
   //ASSERT ( m_model.GetModelCount() > 0 );
   for ( int i=0; i < m_model.GetModelCount(); i++ ) 
      {
      if (0 < (col = m_model.GetModelInfo(i)->col))
            m_model.m_pIDULayer->SetFieldType( col, TYPE_DOUBLE, false);
      }
   
   for ( int i=0; i < m_model.GetAutonomousProcessCount(); i++ ) 
      {
      if (0 < (col = m_model.GetAutonomousProcessInfo(i)->col))
         m_model.m_pIDULayer->SetFieldType(col, TYPE_DOUBLE, false);
      }
