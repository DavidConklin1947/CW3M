...
Generate PolicyOutcomeParser.cpp and PolicyOutcomeParser.hpp from this file with this command
   ...
   >bison -d -ppo PolicyOutcomeGrammer.y -o PolicyOutcomeParser.cpp

the -p option changes the prefix from yy to whatever is specified, in this case "po"
...
outcome
   : STRING '=' INUM   '[' directive ']'  { $$ = new OutcomeInfo( $1, $3, 0, $5 ); }
   | STRING '=' DNUM   '[' directive ']'  { $$ = new OutcomeInfo( $1, $3, 0, $5 ); }
   | STRING '=' FNUM   '[' directive ']'  { $$ = new OutcomeInfo( $1, $3, 0, $5 ); }
   | STRING '=' STRING '[' directive ']'  { $$ = new OutcomeInfo( $1, $3, 0, $5 ); }
   | STRING '=' EVAL   '[' directive ']'  { $$ = new OutcomeInfo( $1, $3, 0, $5 ); }
   | STRING '=' INUM   { $$ = new OutcomeInfo( $1, $3, 0 ); }   
   | STRING '=' DNUM   { $$ = new OutcomeInfo( $1, $3, 0 ); }
   | STRING '=' FNUM   { $$ = new OutcomeInfo( $1, $3, 0 ); }
   | STRING '=' STRING { $$ = new OutcomeInfo( $1, $3, 0 ); }
   | STRING '=' EVAL   { $$ = new OutcomeInfo( $1, $3, 0 ); }
