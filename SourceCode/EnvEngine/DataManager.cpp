...
DataManager *gpDataManager = NULL;
...
void DataManager::RemoveAllData( void )
   ...
   if ( m_modelDataObjMatrices != NULL )
      {
      for ( int i=0; i < EnvModel::GetResultsCount(); i++ )
...
bool DataManager::CreateDataObjects( EnvModel *pModel )
   {
   if ( pModel == NULL )
      return false;

   int rows         = pModel->m_yearsToRun + 1;  // +1 for initial data
   int resultsCount = EnvModel::GetResultsCount();
   ...
   if ( m_collectLandscapeScoreData )
      ...
      for ( int i=0; i < resultsCount; i++ )
         {
         int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
         pEvalScoresData->SetLabel( i+1, EnvModel::GetModelInfo( modelIndex )->name );
         }
      ...
      for ( int i=0; i < resultsCount; i++ )
         {
         int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
         pRawScoresData->SetLabel( i+1, EnvModel::GetModelInfo( modelIndex )->name );
         }
   ...
   int outVarCount = gpModel->GetOutputVarCount( 3 );
   int outputAppVarCount = 0;
   int totalAppVarCount = gpModel->GetAppVarCount();
   ...
   if ( totalOutputCount > 0 )
      ...
      VDataObj *pModelOutputsData = new VDataObj( totalOutputCount+1, rows );
      pModelOutputsData->SetName( "ModelOutput" );
      pModelOutputsData->SetLabel( 0, "Year" );
      int col = 1;
      for ( int i=0; i < EnvModel::GetAutonomousProcessCount(); i++ )
         {
         ENV_AUTO_PROCESS *pInfo = EnvModel::GetAutonomousProcessInfo( i );

         if ( pInfo->use && pInfo->outputVarFn != NULL )
            {
            MODEL_VAR *modelVarArray = NULL;
            int varCount = pInfo->outputVarFn( pInfo->id, &modelVarArray );
            ...
            for ( int j=0; j < varCount; j++ )
               ...
               if ( mv.pVar != NULL )
                  ...
                  label += ".";
                  label += mv.name;
                  pModelOutputsData->SetLabel( col, label );
                  col++;
                  }
               }
            }
      ...
      for ( int i=0; i < totalAppVarCount; i++ )
         ...
         if ( pVar->m_avType == AVT_APP && pVar->IsGlobal() )
            pModelOutputsData->SetLabel( col++, pVar->m_name );
         }

      m_currentDataObjs[ DT_MODEL_OUTPUTS ] = pModelOutputsData;
      m_dataObjs[ DT_MODEL_OUTPUTS ].Add( pModelOutputsData );
   ...
   if ( m_collectActorData )
      ...
      int valueCount = EnvModel::GetActorValueCount(); 
      ...
      for ( int j=0; j < groupCount; j++ )
         {
         ActorGroup *pGroup = gpActorManager->GetActorGroup(j);
   ...
   for ( int j=0; j < policyCount; j++ )
      {
      Policy *pPolicy = gpPolicyManager->GetPolicy( j );

      if ( pPolicy->m_use ) 
         value = 0;
      else
         value = "---";
      pPolicySummaryData->Set( 0, j, VData( gpPolicyManager->GetPolicy( j )->m_name ) );
   ...
   EnvModel::m_pIDULayer->ClearSelection();
   m_currentDataObjs[ DT_POLICY_SUMMARY ] = pPolicySummaryData;
   m_dataObjs[ DT_POLICY_SUMMARY ].Add( pPolicySummaryData );

   // global Constraints
   int gcCount = gpPolicyManager->GetGlobalConstraintCount();
   IDataObj *pGCData = new IDataObj( (gcCount*4)+1, rows );
   pGCData->SetName( "Global Constraints Summary" );
   pGCData->SetLabel( 0, "Time (years)" );
   int col = 1;
   for ( int i=0; i < gcCount; i++ )
      {
      GlobalConstraint *pGC = gpPolicyManager->GetGlobalConstraint( i );
   ...
   if ( gpModel->m_pSocialNetwork != NULL )
      ...
      for ( int l=0; l < layerCount; l++ )
         ...
         for ( int i=0; i < metricCount; i++ )
            {
            CString metric;
            gpModel->m_pSocialNetwork->GetMetricLabel( i, metric );

            CString label( gpModel->m_pSocialNetwork->GetLayer( l )->m_name );
            label += ".";
            label += metric;
   ...
   m_runInfoArray.Add( RUN_INFO( pModel->GetScenario(), pModel->GetScenario()->m_runCount, pModel->m_startYear, pModel->m_endYear ) );
...
bool DataManager::CollectData( EnvModel *pModel, int yearOfRun )
   {
   int row = yearOfRun;
   int currentYear = pModel->m_currentYear;
   ...
   if ( pEvalScoresData != NULL )
      ...
      for ( int i=0; i < resultsCount; i++ )
         {
         int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
         pEvalScoresData->Set( i+1, row, pModel->GetLandscapeMetric( modelIndex ) );
         }
      }
   ...
   if ( pRawScoresData != NULL )
      ...
      for ( int i=0; i < resultsCount; i++ )
         {
         int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
         pRawScoresData->Set( i+1, row, pModel->GetLandscapeMetricRaw( modelIndex ) );
         }
      }

   // model output variables (autonomous processes followed by eval models)
   int outVarCount = EnvModel::GetOutputVarCount( 3 );
   int outputAppVarCount = 0;
   int totalAppVarCount = gpModel->GetAppVarCount();
   for ( int i=0; i < totalAppVarCount; i++ )
      {
   ...
   if ( totalOutputCount > 0 )
      {
      VDataObj *pModelOutputsData = (VDataObj*) m_currentDataObjs[ DT_MODEL_OUTPUTS ];
      ASSERT( pModelOutputsData != NULL );
      pModelOutputsData->Set( 0, row, (float) currentYear ); // yearOfRun );
      ...
      for ( int i=0; i < EnvModel::GetAutonomousProcessCount(); i++ )
         ...
         if ( pInfo->use && pInfo->outputVarFn != NULL )
            ...
            for ( int j=0; j < varCount; j++ )
               ...
               if ( mv.pVar != NULL )
                  {
                  pModelOutputsData->Set( col, row, VData( mv.pVar, mv.type, false ) );  // convert to non-ptr type
                  col++;
                  }
               }
            }
      ...
      for ( int i=0; i < EnvModel::GetModelCount(); i++ )
         ...
         if ( pInfo->use && pInfo->outputVarFn != NULL )
            ...
            for ( int j=0; j < varCount; j++ )
               ...
               if ( mv.pVar != NULL )
                  {
                  pModelOutputsData->Set( col, row, VData( mv.pVar, mv.type, false ) );
                  col++;
                  }
               }
            }
      ...
      int totalAppVarCount = gpModel->GetAppVarCount( 0 );
      for ( int i=0; i < totalAppVarCount; i++ )
         {
         AppVar *pVar = gpModel->GetAppVar( i );
         ...
         if ( pVar->m_avType == AVT_APP && pVar->IsGlobal() )
            {
            pModelOutputsData->Set( col, row, pVar->GetValue() );
            col++;
            }
         }
      }  // end of:  if ( outputCount > 0 )
   ...
   for ( int i=0; i < resultsCount; i++ )
      ...
      ENV_EVAL_MODEL *pModelInfo = pModel->GetModelInfo( modelIndex );

      if ( pModelInfo->pDataObj != NULL )
         {
         ...
         int run = pModel->m_currentRun;
         pDOM->SetDataObj( run, yearOfRun, pModelInfo->pDataObj );
   ...
   if ( pActorWtData != NULL  )
      {
      int actorCount = gpActorManager->GetActorCount();
      int groupCount = gpActorManager->GetActorGroupCount();

      if ( actorCount > 0 )
         {
         int actorValueCount = gpActorManager->GetActorGroup( 0 )->GetValueCount();
         ...
         for ( int i=0; i < actorCount; i++ )
            {
            Actor *pActor = gpActorManager->GetActor( i );

            if ( pActor && pActor->GetPolyCount() > 0 )
               {
               int group = gpActorManager->GetActorGroupIndexFromID( pActor->GetID() );
   ...
   if ( pActorCountData != NULL  )
      ...
      for ( int i=0; i < groupCount; i++ )
         {
         ActorGroup *pGroup = gpActorManager->GetActorGroup( i );
   ...
   if ( yearOfRun == gpModel->m_endYear-gpModel->m_startYear )
      {
      VDataObj *pPolicySummaryData =  (VDataObj*) m_currentDataObjs[ DT_POLICY_SUMMARY ];
      ASSERT( pPolicySummaryData != NULL  );

      for ( int j=0; j < gpPolicyManager->GetPolicyCount(); j++ )
         {
         Policy *pPolicy = gpPolicyManager->GetPolicy( j );
         if ( pPolicy->m_use )
            {
            float potentialAreaPct = 100 * pPolicy->m_potentialArea  / EnvModel::m_pIDULayer->GetTotalArea();
            float appliedAreaPct   = 100 * pPolicy->m_cumAppliedArea / EnvModel::m_pIDULayer->GetTotalArea();
            ...
            if ( gpModel->m_collectPolicyData )
   ...
   // global constraints
   IDataObj *pGlobalConstraintsData =  (IDataObj*) m_currentDataObjs[ DT_GLOBAL_CONSTRAINTS ];
   if ( pGlobalConstraintsData != NULL  )
      ...
      for ( int i=0; i < gcCount; i++ )
         {
         GlobalConstraint *pGC = gpPolicyManager->GetGlobalConstraint( i );
         pGlobalConstraintsData->Set( col++, row, (int) pGC->m_currentValue );
         pGlobalConstraintsData->Set( col++, row, (int) pGC->m_currentInitValue );
         pGlobalConstraintsData->Set( col++, row, (int) pGC->m_currentMaintenanceValue );
         pGlobalConstraintsData->Set( col++, row, (int) pGC->m_value );
         }
      }
   ...
   if ( pSocialNetworkData != NULL  )
      ...
      for ( int layer=0; layer < layerCount; layer++ )
         ...
         for ( int i=0; i < metricCount; i++ )
            {
            float value = gpModel->m_pSocialNetwork->GetMetricValue( layer, i );
...
void DataManager::EndRun( bool discardDeltaArray )
   ...
   // model output variables (autonomous processes followed by eval models)
   int outVarCount = EnvModel::GetOutputVarCount( 3 );  // total for models and processes
   int outputAppVarCount = 0;
   int totalAppVarCount = gpModel->GetAppVarCount();
   for ( int i=0; i < totalAppVarCount; i++ )
      {
   ...
   if ( totalOutputCount > 0 )
      ...
      VDataObj *pModelOutputsData = (VDataObj*) m_currentDataObjs[ DT_MODEL_OUTPUTS ];
      ASSERT( pModelOutputsData != NULL );
      //pModelOutputsData->Set( 0, row, (float) yearOfRun );
         ...
         if ( pInfo->use && pInfo->outputVarFn != NULL )
            ...
            for ( int j=0; j < varCount; j++ )
               ...
               if ( mv.type == TYPE_PDATAOBJ )
                  {
                  // get the original data obj ptr (stored in the pModelOutputsData)
                  DataObj *pData = NULL;
                  pModelOutputsData->Get( col, 0, pData );
                  ...
                  if ( pDataCopy != NULL )
                     {
                     // update all the ptr records in the pModelOutputsData for this
                     m_modelOutputDataObjArray.Add( pDataCopy );

                     for ( int j=0; j < pModelOutputsData->GetRowCount(); j++ )
                        {
                        pModelOutputsData->Set( col, j, VData(pDataCopy, TYPE_PDATAOBJ, false ) );   // store copied dataobj ptr in each row
                        }
                     }
            ...
            for ( int j=0; j < varCount; j++ )
               ...
               if ( mv.type == TYPE_PDATAOBJ )
                  ...
                  if ( pDataCopy != NULL )
                     ...
                     // update all the ptr records in the pModelOutputsData for this
                     for ( int k=0; k < pModelOutputsData->GetRowCount(); k++ )
                        {
                        pModelOutputsData->Set( col, k, VData(pDataCopy, TYPE_PDATAOBJ, false ) );   // store copied dataobj ptr in each row
                        }
                     }
                  }  // end of: if ( mv.type == PDATAOBJ )
...
bool DataManager::CreateMultiRunDataObjects()
   {
   int rows = gpModel->m_iterationsToRun;
   int resultsCount = EnvModel::GetResultsCount();
   ...
   for ( int i=0; i < resultsCount; i++ )
      {
      int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
      pData->SetLabel( i+1, EnvModel::GetModelInfo( modelIndex )->name );
      }
   ...
   int varCount = pScenario->GetVarCount( int( V_META | V_MODEL | V_AP ), false );
   pData = new FDataObj( varCount+1, rows );
   pData->SetName( "Multirun Scenario Settings" );
   pData->SetLabel( 0, "Run" );
   for ( int i=0; i < varCount; i++ )
      pData->SetLabel( i+1, pScenario->GetVarInfo( i ).name );

   m_currentMultiRunDataObjs[ DT_MULTI_PARAMETERS ] = pData;
   m_multiRunDataObjs[ DT_MULTI_PARAMETERS ].Add( pData );

   int first = gpModel->m_currentRun;
   m_multirunInfoArray.Add( MULTIRUN_INFO( first, first + rows-1, gpModel->GetScenario() ) );

   return true;
   }


bool DataManager::CollectMultiRunData( EnvModel *pModel, int run )   // note: run - zero-based
   {
   int row = run; 
   int resultsCount = EnvModel::GetResultsCount();
   ...
   if ( pData != NULL )
      ...
      for ( int i=0; i < resultsCount; i++ )
         {
         int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
         float score = pModel->GetLandscapeMetric( modelIndex );
   ...
   if ( pData != NULL )
      ...
      for ( int i=0; i < resultsCount; i++ )
         {
         int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
         float rawScore = pModel->GetLandscapeMetricRaw( modelIndex );
   ...
   if ( pData != NULL )
      ...
      for ( int i=0; i < varCount; i++ )
         {
         VAR_INFO & vi = pScenario->GetVarInfo( i );
         float value;
         switch( vi.type )
            {
...
FDataObj *DataManager::CalculateLulcTrends( int level, int run /*=-1*/)
   ...
   CString label = EnvModel::m_lulcTree.GetFieldName( level );
   label += " Trends (pct)";

   // allocate a data object
   int lulcCount = EnvModel::m_lulcTree.GetNodeCount( level );
   ...
   for ( int i=0; i < lulcCount; i++ )
      {
      LulcNode *pNode = EnvModel::m_lulcTree.FindNodeFromIndex( level, i );
      CString label;
   ...
   switch( level )
      {
      case 1:  colLulc = EnvModel::m_colLulcA;  break;
      case 2:  colLulc = EnvModel::m_colLulcB;  break;
      case 3:  colLulc = EnvModel::m_colLulcC;  break;
      case 4:  colLulc = EnvModel::m_colLulcD;  break;
      }

   ColumnTrace lulcCol( EnvModel::m_pIDULayer, colLulc, run );  // Note: the column trace is rolled back to inital conditions inthe constructor
   ...
   UINT_PTR firstUnapplied = gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer, NULL );

   // interate through cells to get starting value
   for ( MapLayer::Iterator i = EnvModel::m_pIDULayer->Begin(); i != EnvModel::m_pIDULayer->End(); i++ )
      {
      int lulc;
      lulcCol.Get( i, lulc );
      ...
      if ( lulc > 0 )
         ...
         if (ok) //ASSERT( ok );   
            {
            float area;
            EnvModel::m_pIDULayer->GetData(i, EnvModel::m_colArea, area);
   ...
   for ( int year = startYear; year < endYear; year++ )
      ...
      for ( INT_PTR i=from; i < to; i++ )
         ...
         gpModel->ApplyDelta( EnvModel::m_pIDULayer, delta ); 
         ...
         if ( delta.col == colLulc )
            ...
            float area;
            int value;
            EnvModel::m_pIDULayer->GetData( delta.cell, EnvModel::m_colArea, area );
   ...
   pDeltaArray->SetFirstUnapplied( EnvModel::m_pIDULayer, SFU_END );      // deltas have no concept of the delta array they belong to, so ApplyDelta does not set FirstUnapplied
   gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer, pDeltaArray );   // unapply all of this delta array
   gpModel->ApplyDeltaArray( EnvModel::m_pIDULayer, NULL, 0, firstUnapplied == 0 ? 0 : firstUnapplied-1);      // return map to it's starting state
...
void DataManager::CalculateLulcTrends( int level, FDataObj **ppData, int fromYearClosed, int toYearOpen, int run )
   ...
   // get the LULC nodes
   //LulcNodeArray *pNodeArray = &(EnvModel::m_lulcTree.GetRootNode()->m_childNodeArray);
   int lulcCount = EnvModel::m_lulcTree.GetNodeCount( level ); //(int) pNodeArray->GetCount();
   ...
   for ( int i=0; i < lulcCount; i++ )
      {
      LulcNode *pNode = EnvModel::m_lulcTree.FindNodeFromIndex( level, i ); //pNodeArray->GetAt( i );
      lulcMap.SetAt( pNode->m_id, i );
      }
   ...
   if ( pDataObj == NULL )
      ...
      ColumnTrace lulcCol( EnvModel::m_pIDULayer, colLulc, run);
      int lulcIndex = -1;
...
void DataManager::CalculateAppliedPolicyScores( FDataObj **ppData, int fromYearClosed, int toYearOpen, int run )
   ...
   if ( pDataObj == NULL )
      ...
      for ( int i=0; i < metagoalCount; i++ )
         ...
         //pDataObj->SetLabel( i+1, EnvModel::GetModelInfo( modelIndex )->name );
         METAGOAL *pInfo = EnvModel::GetMetagoalInfo( i ); 
         pDataObj->SetLabel( i+1, pInfo->name );
         }
      }
   ...
   for ( int year = fromYearClosed; year<toYearOpen; year++ )
      ...
      int colPolicyID = EnvModel::m_colPolicy;
      int colArea     = EnvModel::m_colArea;
      int policyCount = 0;
      ...
      for ( INT_PTR i=from; i < to; i++ )
         ...
         if ( colPolicyID == delta.col )
            ...
            float area;  // m2
            EnvModel::m_pIDULayer->GetData( delta.cell, colArea, area );
            appliedArea += area;

            // get the policy and evaluate it's effectiveness at this value
            int polID = -1;
            bool ok = delta.newValue.GetAsInt( polID );
            Policy *pPolicy = gpPolicyManager->GetPolicyFromID( polID );
            ASSERT( pPolicy != NULL );

            for ( int j=0; j < metagoalCount; j++ )
               {
               //int modelIndex = EnvModel::GetModelIndexMetagoalIndex( j );
   ...
   for ( int year = fromYearClosed; year<toYearOpen; year++ )
      ...
      for ( INT_PTR i=from; i < to; i++ )
         ...
         if ( colPolicyID == delta.col )
            ...
            for ( int j=0; j < metagoalCount; j++ )
               {
               //int modelIndex = EnvModel::GetModelIndexFromActorValueIndex( j );
...
void DataManager::CalculateAreaQuery( IDataObj **ppData, int fromYearClosed, int toYearOpen, int col, VData &value, int run  )
   ...
   // variables
   float totalArea  = EnvModel::m_pIDULayer->GetTotalArea();
   ...
   if ( pDataObj == NULL )
      ...
      CString name( EnvModel::m_pIDULayer->GetFieldLabel( col ) );
      name += " = ";
      ...
      for( MapLayer::Iterator cell = EnvModel::m_pIDULayer->Begin(); cell != EnvModel::m_pIDULayer->End(); ++cell )
         {
         EnvModel::m_pIDULayer->GetData( cell, EnvModel::m_colArea, area );
         EnvModel::m_pIDULayer->GetData( cell, col, currentVal );
...
IDataObj *DataManager::CalculatePolicyApps( int run /*= -1*/ )
   {
   DeltaArray *pDeltaArray = this->GetDeltaArray( run );
   if ( pDeltaArray == NULL ) return NULL;
   ...
   UINT_PTR firstUnapplied = gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer );    // move map to starting conditions
   ...
   for ( int year=startYear; year < endYear; year++ )
      ...
      for ( INT_PTR i=from; i<to; i++ )
         ...
         if ( delta.col == EnvModel::m_colPolicy )
            {
            int policyID;
            delta.newValue.GetAsInt( policyID );

            int policyIndex = gpPolicyManager->GetPolicyIndexFromID( policyID );
   ...
   pDeltaArray->SetFirstUnapplied( EnvModel::m_pIDULayer, SFU_END );      // deltas have no concept of the delta array they belong to, so ApplyDelta does not set FirstUnapplied
   gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer, pDeltaArray );
   gpModel->ApplyDeltaArray( EnvModel::m_pIDULayer, NULL, 0, firstUnapplied == 0 ? 0 : firstUnapplied-1);      // return map to it's starting state
...
FDataObj *DataManager::CalculatePolicyEffectivenessTrendDynamic( int modelIndexX, int modelIndexY, int run /*= -1*/ )
   ...
   int metagoalIndexX = EnvModel::GetMetagoalIndexFromModelIndex( modelIndexX );
   int metagoalIndexY = EnvModel::GetMetagoalIndexFromModelIndex( modelIndexY );
   ...
   pData->SetLabel( 0, EnvModel::GetModelInfo( modelIndexX )->name );
   pData->SetLabel( 1, EnvModel::GetModelInfo( modelIndexY )->name );

   UINT_PTR firstUnapplied = gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer ); // roll map back to starting condition
...
FDataObj *DataManager::CalculatePolicyEffectivenessTrendStatic( int run /*= -1*/ )
   ...
   int goalCount = EnvModel::GetMetagoalCount();
   //int resultsCount = EnvModel::GetResultsCount();

   // move map back to starting conditions
   UINT_PTR firstUnapplied = gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer );
...
VDataObj *DataManager::CalculatePolicySummary( int run /*= -1*/ )
   ...
   int resultsCount = 0;  //EnvModel::GetResultsCount();
   int valueCount = EnvModel::GetActorValueCount();
   ...
   for ( int j=0; j < policyCount; j++ )
      ...
      if ( pi.inUse )
         {
         pData->Set( 0, usedPolicyCount, VData( gpPolicyManager->GetPolicy( j )->m_name ) );
   ...
   for ( int i=0; i < resultsCount; i++ )
      {
      int modelIndex = EnvModel::GetModelIndexFromResultsIndex( i );
      CString label = EnvModel::GetModelInfo( modelIndex )->name;
   ...
   for ( int i=0; i < valueCount; i++ )
      {
      METAGOAL *pGoal = EnvModel::GetActorValueMetagoalInfo( i );
   ...
   int colPolicyID = EnvModel::m_colPolicy;
   int colActor    = EnvModel::m_colActor;
   ...
   for ( UINT_PTR i=0; i < deltaCount; i++ )
      {
      DELTA &delta = pDeltaArray->GetAt( i );
      gpModel->ApplyDelta( EnvModel::m_pIDULayer, delta ); // apply it
   ...
   for ( int i=0; i < policyCount; i++ )
      ...
      for ( int j=0; j < cellArray.GetSize(); j++ )
         {
         EnvModel::m_pIDULayer->GetData( cellArray[ j ], EnvModel::m_colArea, cellArea );
      ...
      for ( int k=0; k < valueCount; k++ )
         ...
         for ( int j=0; j < cellArray.GetSize(); j++ )
            {
            int actor;
            EnvModel::m_pIDULayer->GetData( cellArray[ j ], colActor, actor );

            if ( actor >= 0 )
               {
               Actor *pActor = gpActorManager->GetActor( actor );            
...
VDataObj *DataManager::CalculatePolicyByActorArea( int run /* = -1 */ )
   ...
   Scenario *pScenario = ri.pScenario;
   int policies = gpPolicyManager->GetPolicyCount();
   int actors = gpActorManager->GetActorGroupCount();
   ...
   for ( int j=0; j < actors; j++ )
      {
      ActorGroup *pGroup = gpActorManager->GetActorGroup( j ); //Policy *pPolicy =  gpPolicyManager->GetPolicy( j );
   ...
   for ( INT_PTR i=0; i < pDeltaArray->GetSize(); i++ )
      ...
      if ( d.col == EnvModel::m_colPolicy )  // this is the policy ID
         {
         policyApps++;

         float area;
         EnvModel::m_pIDULayer->GetData( d.cell, EnvModel::m_colArea, area );

         int actor;
         EnvModel::m_pIDULayer->GetData( d.cell, EnvModel::m_colActor, actor );
...
FDataObj *DataManager::CalculateTrendsWeightedByLulcA( int trendCol, double scaler /*= 1.0*/, AREA_FACTOR af /*=AF_IGNORE*/, int run /*= -1*/, bool includeTotal /*=true*/ )
   ...
   CString name;
   name = EnvModel::m_pIDULayer->GetFieldLabel( trendCol );
   ...
   int     lulcA;
   int     lulcAIndex;
   float   noDataValue = EnvModel::m_pIDULayer->GetNoDataValue();

   UINT_PTR firstUnapplied = gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer );  //roll back to starting conditions
   float totalArea = 0;
   ...
   for ( MapLayer::Iterator cell = EnvModel::m_pIDULayer->Begin(); cell != EnvModel::m_pIDULayer->End(); cell++ )
      ...
      EnvModel::m_pIDULayer->GetData( cell, trendCol, trendValue );

      if ( trendValue != EnvModel::m_pIDULayer->GetNoDataValue() )
         {
         EnvModel::m_pIDULayer->GetData( cell, EnvModel::m_colLulcA, lulcA );
   ...
   for ( int year=startYear; year < endYear; year++ )
      ...
      for ( int cell = EnvModel::m_pIDULayer->Begin(); cell != EnvModel::m_pIDULayer->End(); cell++ )
         {
         EnvModel::m_pIDULayer->GetData( cell, EnvModel::m_colArea, area );
         EnvModel::m_pIDULayer->GetData( cell, trendCol, trendValue );
...
VDataObj *DataManager::CalculateEvalScoresByLulcA( int year, int run /*=-1*/ )
   ...
   for ( int i=0; i < EnvModel::GetModelCount(); i++ )
      {
      ENV_EVAL_MODEL *pInfo = EnvModel::GetModelInfo( i );
      if ( pInfo->col >= 0 && pInfo->showInResults > 0 )
         {
   ...
   // initial values set, start computing changes
   UINT_PTR firstUnapplied = gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer );  // roll back current map to starting conditions
   float totalArea = 0;
   ...
   for ( int cell = EnvModel::m_pIDULayer->Begin(); cell != EnvModel::m_pIDULayer->End(); cell++ )
      ...
      for ( int i=0; i < evalModelCount; i++ )
         ...
         EnvModel::m_pIDULayer->GetData( cell, col, score );
...
FDataObj *DataManager::CalculateLulcTransTable( int run /*= -1*/, int level /*= 1*/, int year /*= -1*/, bool areaBasis /*= false*/, bool includeAlps /*= true*/ )
   {
   int         lulcCount   = gpModel->m_lulcTree.GetNodeCount( level );
   ...
   switch ( level )
      {
      case 1:
         lulcCol = gpModel->m_colLulcA;
         break;
      case 2:
         lulcCol = gpModel->m_colLulcB;
         break;
      case 3:
         lulcCol = gpModel->m_colLulcC;
         break;
      case 4:
         lulcCol = gpModel->m_colLulcD;
         break;
      default:
         ASSERT(0);
      }

   LulcNode *pNode = gpModel->m_lulcTree.GetRootNode();
   int index = 0;
   while ( pNode != NULL )
      {
      ...
      pNode = gpModel->m_lulcTree.GetNextNode();
      }
   ...
   if ( year == -1 ) // meaning throughout the run
      ...
      pDeltaArray->GetIndexRangeFromYear( year, from, to );
      if ( from != 0 )
         gpModel->ApplyDeltaArray( EnvModel::m_pIDULayer, pDeltaArray, 0, from-1 );   // roll map forward to just prior to given year
      }
   ...
   for ( INT_PTR i=from; i<to; i++ )
      {
      DELTA &delta = pDeltaArray->GetAt(i);
      gpModel->ApplyDeltaArray( EnvModel::m_pIDULayer, pDeltaArray, i, i );
      ...
      if ( delta.col == lulcCol )
         ...
         if ( fromLulc != toLulc )  // ignore redundant changes
            ...
            if ( ok1 && ok2 )
               ...
               //      {
               //      float area;
               //      bool ok = EnvModel::m_pIDULayer->GetData( delta.cell, EnvModel::m_colArea, area );
               //      ASSERT( ok );
               //      v += area;
               //      }
               //   else
   ...
   UINT_PTR _firstUnapplied = pDeltaArray->GetFirstUnapplied( EnvModel::m_pIDULayer );

   if ( _firstUnapplied > 0 )
      gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer, pDeltaArray, _firstUnapplied-1 );
...
FDataObj *DataManager::CalculateScenarioVulnerability( int multiRun )
   ...
   int colDistStr = EnvModel::m_pIDULayer->GetFieldCol( "DIST_STR" );
   //ASSERT( colDistStr >= 0 );
   if ( colDistStr < 0 )
      return NULL;
   ...
   int cells = EnvModel::m_pIDULayer->GetRowCount();
   ...
   for ( int run = startRun; run <= endRun; run++ )
      ...
      for ( UINT_PTR i=0; i < deltas; i++ )
         ...
         if ( delta.col == EnvModel::m_colLulcC )
            {
            ...
            if ( int( oldStatus ) < 4 && int( newStatus ) < 4 && oldStatus != newStatus )
               ...
               Poly *pPoly = EnvModel::m_pIDULayer->GetPolygon( delta.cell );
               ASSERT( pPoly != NULL );
               ...
               if ( pPoly->GetParentCount() > 0 )
                  ...
                  while( pPoly->GetParentCount() > 0 )
                     ...
                     pPoly = EnvModel::m_pIDULayer->GetPolygon( parentIndex );
                     }

                  // get areas
                  float childArea, parentArea;
                  EnvModel::m_pIDULayer->GetData( delta.cell, EnvModel::m_colArea, childArea );
                  EnvModel::m_pIDULayer->GetData( pPoly->m_id, EnvModel::m_colArea, parentArea );      // pPoly now points to the root parent
      ...
      pDeltaArray->SetFirstUnapplied( EnvModel::m_pIDULayer, SFU_END );      // deltas have no concept of the delta array they belong to, so ApplyDelta does not set FirstUnapplied
      gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer, pDeltaArray );      // reset map to starting state
   ...
   gpModel->ApplyDeltaArray( NULL, gpModel->m_pDeltaArray, firstUnapplied == 0 ? 0 : firstUnapplied-1, gpModel->m_pDeltaArray->GetCount()-1);
...
LU_STATUS GetLulcStatus( int cell, int lulcC, int colDistStr )
   ...
   switch( lulcC )
      ...
      case 99:
         {
         float distStr;
         EnvModel::m_pIDULayer->GetData( cell, colDistStr, distStr );
...
bool DataManager::SaveRun( LPCSTR fileName, int run /*= -1*/ )
   {
...
bool DataManager::LoadRun( LPCSTR fileName )
   {
   FILE *fp;
   ...
   // Load DeltaArray
   ASSERT(EnvModel::m_pIDULayer);
   m_pDeltaArray = new DeltaArray( EnvModel::m_pIDULayer, gpModel->m_startYear );
...
bool DataManager::ExportRunDeltaArray( LPCSTR _path, int run, int startYear, int endYear, bool selectedOnly, bool includeDuplicates, LPCTSTR fieldList /*=NULL*/ )
   {
   //--------------------------------------------------
   ...
   int fieldCount = EnvModel::m_pIDULayer->GetFieldCount();
   ...
   if ( ! includeAllFields )
      ...
      while ( token != NULL )
         {
         int col = EnvModel::m_pIDULayer->GetFieldCol( token );
   ...
   for ( INT_PTR i=0; i < size; i++ )
      ...
      // selected only?
      if ( selectedOnly && ( EnvModel::m_pIDULayer->IsSelected( delta.cell ) == false ) )
         continue;
      ...
      fprintf_s( fp, _T("%i,%i,%i,%s,%s,%s,%i\n"),
         delta.year, delta.cell, delta.col, EnvModel::m_pIDULayer->GetFieldLabel( delta.col ), (LPCTSTR) oldValue, (LPCTSTR) newValue, delta.type );
      }
...
bool DataManager::ExportRunData( LPCTSTR _path, int run /*=-1*/ )   /// Note: run is 0-based, doesn't include m_startRunNumber
   ...
   for ( int j=0; j < DT_LAST; j++ )
      ...
      if ( dataObjArray.GetSize() > 0 )
         ...
         if ( pData != NULL )
            ...
            filename.Format( "%s_%s_Run%i.csv", (LPCTSTR) dataObjName, (LPCTSTR) scname, scrun + gpModel->m_startRunNumber );
            ...
            for ( int col=0; col < cols; col++ )
               ...
               if ( v.GetType() == TYPE_PDATAOBJ )
                  ...
                  filename.Format( "%s_%s_Run%i.csv", (LPCTSTR) subDOName, (LPCTSTR) scname, scrun + gpModel->m_startRunNumber );
   ...
   int lulcLevels = EnvModel::m_lulcTree.GetLevels();
...
bool DataManager::ExportRunMap( LPCTSTR path, int run /*=-1*/ )    // assume path is terminated with a '\'
   ...
   CString filename;
   filename.Format( _T("%sMap_Year%i_%s_Run%i.shp" ), path, gpModel->m_currentYear, (LPCTSTR) scname, scrun + gpModel->m_startRunNumber );

   CString msg( "Exporting map: " );
   msg += filename;
   Report::StatusMsg( msg );

   gpModel->m_pIDULayer->SaveShapeFile( filename );
   return true;
   }
...
bool DataManager::ExportRunBmps( LPCTSTR path, int run /*=-1*/ )    // assume path is terminated with a '\'
   ...
   if ( gpModel->m_exportBmpSize > 0 )
      ...
      if ( gpModel->m_exportBmpFields.GetLength() > 0 )
         {
         TCHAR buffer[ 256 ];
         lstrcpy( buffer, gpModel->m_exportBmpFields );
         ...
         while( field != NULL )
            {
            int col = gpModel->m_pIDULayer->GetFieldCol( field );
            if ( col < 0 )
               {
               CString msg;
               ...
               {
               CString filename;
               filename.Format( _T("%s%s_Run%i_Year_%i_%s.bmp" ), path, (LPCTSTR) scname, scrun  + gpModel->m_startRunNumber, gpModel->m_currentYear, field );
...
#ifdef _WINDOWS            
               Raster raster( gpModel->m_pIDULayer, false ); // don't copy polys, 
               raster.Rasterize( col, gpModel->m_exportBmpSize, RT_32BIT, 0, NULL, DV_COLOR );
         ...
         {
         int activeCol = gpModel->m_pIDULayer->GetActiveField();

         if ( activeCol >= 0 )
            {
            CString filename;

            filename.Format( _T("%s%s_Run%iYear_%i_%s.bmp" ), path, (LPCTSTR) scname, scrun  + gpModel->m_startRunNumber, gpModel->m_currentYear, gpModel->m_pIDULayer->GetFieldLabel( activeCol ) );
...
#ifdef _WINDOWS
            Raster raster( gpModel->m_pIDULayer, false ); // don't copy polys, 
            raster.Rasterize( activeCol, gpModel->m_exportBmpSize, RT_32BIT, 0, NULL, DV_COLOR );
            raster.SaveDIB( filename );
#endif
            }
         }
...
int DataManager::ImportMultiRunData( LPCSTR path, int multirun /* =-1 */ )
   {
   ...
   Scenario *pScenario = gpScenarioManager->GetScenario( scenario+position );
   ASSERT( pScenario != NULL );
   ...
   for (int i=0; i < gpScenarioManager->GetCount(); i++)
      {
      if (gpScenarioManager->GetScenario(i)->m_name == pScenario->m_name)
      runScenario = i;
      }   
   ...
   while( 1 )
      ...
      DeltaArray *pDA = new DeltaArray( EnvModel::m_pIDULayer, gpModel->m_startYear );
      ok = pDA->Load( runPath );
      ...
      pDA->SetFirstUnapplied( EnvModel::m_pIDULayer, SFU_END );   // In a true run, FirstUnapplied = delta count for each delta array
      m_deltaArrayArray.Add( pDA );
      m_pDeltaArray = pDA;
      gpModel->m_pDeltaArray = pDA;
   ...
   gpModel->m_currentMultiRun = 1;
   gpModel->m_currentRun = runLast;
   gpModel->SetScenario( pScenario );
...
bool DataManager::FixSubdividePointers(DeltaArray *pDA)
   ...
   for (UINT_PTR d=0; d < deltas; d++)
      ...
      if ( EnvModel::m_pIDULayer->IsDefunct( deltaCheck.cell ) )
         {
         pDA->RemoveAt(d);
         deltas--;
         d--;
      ...
      if ( deltaCheck.type != DT_SUBDIVIDE ) //.col != DELTA::SUBDIVIDE)
         {
         count += gpModel->ApplyDelta( EnvModel::m_pIDULayer, deltaCheck );
         continue;
         }
      ...
      if (back > 4)
         EnvModel::m_pIDULayer->GetData( deltaCheck.cell, EnvModel::m_colPolicy, policyID);
      else
      ...
      QueryEngine *queryEng = new QueryEngine(EnvModel::m_pIDULayer);
      ...
      Poly *pPoly = EnvModel::m_pIDULayer->GetPolygon(deltaCheck.cell);
      ...
      do
         ...
         n += 100;
         neighbors = new int[ n ];
         count = EnvModel::m_pIDULayer->GetNearbyPolysFromIndex( pPoly, neighbors, NULL, n, 0 );
         } 
      while ( n <= count );
      ...
      for ( int i=0; i<count; i++ )
         ...
         if ( result )
            {
            Poly *pPoly = EnvModel::m_pIDULayer->GetPolygon( neighbors[i] );
            ASSERT( pPoly );
      ...
      if ( bufferUnion.GetVertexCount() > 0 )
         ...
         if ( pOutside->GetVertexCount() == 0 || pOutside->GetArea() <= 0 )
            ...
            /*int outcome = gpModel->SelectPolicyOutcome( m_insideBufferMultiOutcomeArray );
            if (outcome >= 0)
               {
               const MultiOutcomeInfo& bMultiOutcome = m_insideBufferMultiOutcomeArray.GetMultiOutcome( outcome );
               gpModel->ApplyMultiOutcome( bMultiOutcome, cell, persistence );
               }
            return true;*/
      ...
      //--------------------------------------------------------------   
         ...
         int dynamicPolyCount = EnvModel::m_pIDULayer->GetRecordCount( MapLayer::ALL );
   ...
   pDA->SetFirstUnapplied( EnvModel::m_pIDULayer, SFU_END );      // deltas have no concept of the delta array they belong to, so ApplyDelta does not set FirstUnapplied
   gpModel->UnApplyDeltaArray( EnvModel::m_pIDULayer, pDA );
   
   return true;
   }

bool DataManager::ExportMultiRunData( LPCSTR _path, int multirun /*=-1*/ )
   {
     WAIT_CURSOR;
   ...
   for ( int run=runFirst; run <= runLast; run++ )
      {
      CString runPath;
      runPath.Format( "%s\\Run_%i\\", (PCTSTR) path, run + gpModel->m_startRunNumber );  // need to revisit!
