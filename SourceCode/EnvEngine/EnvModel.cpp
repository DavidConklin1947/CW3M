...
int   EnvModel::m_referenceCount = 0;
float EnvModel::m_policyPrefWt   = 0.5f;
int   EnvModel::m_colArea = -1;
int   EnvModel::m_colLulcA = -1;
int   EnvModel::m_colLulcB = -1;
int   EnvModel::m_colLulcC = -1;
int   EnvModel::m_colLulcD = -1;
int   EnvModel::m_colPolicy = -1;
int   EnvModel::m_colActor = -1;
int   EnvModel::m_colScore = -1;
int   EnvModel::m_colStartLulc = -1;
int   EnvModel::m_colStartCons = -1;
int   EnvModel::m_colLastDecision = -1;
int   EnvModel::m_colNextDecision = -1;
int   EnvModel::m_colPolicyApps = -1;
int   EnvModel::m_colUtility = -1;

CUIntArray EnvModel::m_colActorValues;

RandUniform EnvModel::m_randUnif( 1, 1 );              // stream, seed
RandNormal  EnvModel::m_randNormal( 0, 1, 42 );        // mean, stddev, seed

LulcTree EnvModel::m_lulcTree;

PtrArray< ENV_EVAL_MODEL   > EnvModel::m_modelInfoArray( false );
PtrArray< ENV_AUTO_PROCESS > EnvModel::m_apInfoArray( false );
PtrArray< ENV_VISUALIZER   > EnvModel::m_vizInfoArray( false );
PtrArray< METAGOAL         > EnvModel::m_metagoalInfoArray( true );

CArray< int, int > EnvModel::m_actorValueToMetagoalMap;
CArray< int, int > EnvModel::m_scarcityToModelMap;
CArray< int, int > EnvModel::m_metagoalToModelMap;
CArray< int, int > EnvModel::m_resultsToModelMap;

int ENV_EVAL_MODEL::nextHandle = DT_MODEL;
int ENV_AUTO_PROCESS::nextHandle = DT_PROCESS;
...
//=======================================================================================
inline int ENV_EVAL_MODEL::ScaleScore(float & arg) const
   {
...
bool AppVar::Evaluate( int idu /*=-1*/ )
   ...
   switch( m_avType )
      ...
      case AVT_APP:
         ...
         return m_pMapExpr->Evaluate( idu );
         }
      }
...
EnvModel::EnvModel()
 : m_startYear( 0 ),
   ...
   m_envContext.weatherYear = m_startYear;
   m_envContext.run = -1;
   ...
   m_envContext.pExtensionInfo = NULL;
#ifndef NO_MFC
...
EnvModel::~EnvModel()
   ...
   if ( m_pQueryEngine != NULL )
      ...
      m_pQueryEngine = NULL;
      PolicyManager::m_pQueryEngine = NULL;
      }
...
int EnvModel::AddModel( ENV_EVAL_MODEL *mi ) 
   {
   int index = (int) m_modelInfoArray.Add( mi );

   //if ( mi->decisionUse & DU_SELFINTEREST ) 
   //   m_actorValueToModelMap.Add( index );

   //if ( mi->decisionUse & DU_ALTRUISM ) 
   //   m_scarcityToModelMap.Add( index );

   //if ( mi->decisionUse & DU_ALTRUISM || mi->decisionUse & DU_SELFINTEREST ) 
   //   m_metagoalToModelMap.Add( index );

   if ( mi->use && mi->showInResults ) 
      m_resultsToModelMap.Add( index );
...
void EnvModel::AddMetagoal( METAGOAL *pGoal )
   ...
   if ( pGoal->pEvalModel == NULL )
      m_metagoalToModelMap.Add( -1 );
   else
      {
      int modelIndex = EnvModel::FindModelIndex( pGoal->pEvalModel->name );
      m_metagoalToModelMap.Add( modelIndex );

      ASSERT( pGoal->decisionUse & DU_ALTRUISM );
      m_scarcityToModelMap.Add( modelIndex );
      }
   }
...
void EnvModel::RemoveMetagoal( int i )
   ...
   m_metagoalToModelMap.RemoveAt( i );
   }
...
int EnvModel::FindMetagoalIndex( LPCSTR name )
   {
   for ( int i=0; i < EnvModel::GetMetagoalCount(); i++ )
      {
      if ( m_metagoalInfoArray[ i ]->name.CompareNoCase( name ) == 0 )
...
void EnvModel::FreeLibraries()
   {
   for ( int i=0; i < m_modelInfoArray.GetSize(); i++ )
      {
      BOOL bRes = FREE_LIBRARY( m_modelInfoArray[ i ]->hDLL );
      delete m_modelInfoArray[ i ];
      }

   for ( int i=0; i < m_apInfoArray.GetSize(); i++ )
      {
      FREE_LIBRARY( m_apInfoArray[ i ]->hDLL );
      delete m_apInfoArray[ i ];
      }

   for ( int i=0; i < m_vizInfoArray.GetSize(); i++ )
      {
      FREE_LIBRARY( m_vizInfoArray[ i ]->hDLL );
      delete m_vizInfoArray[ i ];
      }
   }
...
void EnvModel::SetScenario( Scenario *pScenario )
   ...
   if ( pScenario == NULL )
      ...
      m_scenarioIndex = gpScenarioManager->GetScenarioIndex( pScenario );
...
int EnvModel::GetActorValueIndexFromModelIndex( int modelIndex )
   {
   ENV_EVAL_MODEL *pModel = GetModelInfo( modelIndex );
   int valueIndex = 0;
...
int EnvModel::GetScarcityIndexFromModelIndex( int modelIndex )
   {
   ENV_EVAL_MODEL *pModel = GetModelInfo( modelIndex );
   int scarcityIndex = 0;
...
int EnvModel::GetMetagoalIndexFromModelIndex( int modelIndex )
   {
   /*
   int metagoalCount = -1;
   for ( int i=0; i <= modelIndex; i++ )
      {
      if ( GetModelInfo(i)->decisionUse & DU_ALTRUISM || GetModelInfo(i)->decisionUse & DU_SELFINTEREST )
         metagoalCount++;
      }
...
int EnvModel::GetResultsIndexFromModelIndex( int modelIndex )
   {
   int resultsCount = -1;
   for ( int i=0; i <= modelIndex; i++ )
      {
      if ( GetModelInfo(i)->showInResults )
         resultsCount++;
      }
...
int EnvModel::FindAutonomousProcessIndex( LPCSTR name )
   {
   int count = GetAutonomousProcessCount();

   for ( int i=0; i < count; i++ )
      {
      ENV_AUTO_PROCESS *pInfo = GetAutonomousProcessInfo( i );
      if ( pInfo->name.CompareNoCase( name ) == 0 )
         return i;
      }
...
ENV_AUTO_PROCESS* EnvModel::FindAutonomousProcessInfo( LPCSTR name )
   {
   int index = FindAutonomousProcessIndex( name );

   if ( index < 0 )
      return NULL;
   else
      return GetAutonomousProcessInfo( index );
   }
...
int EnvModel::FindModelIndex( LPCSTR name )
   ...
   for ( int i=0; i < count; i++ )
      {
      ENV_EVAL_MODEL *pInfo = GetModelInfo( i );
      if ( pInfo->name.CompareNoCase( name ) == 0 )
         return i;
      }
...
int EnvModel::GetInputVarCount( int flag )
   ...
   if ( flag & 1 )
      {
      for ( int i=0; i < GetAutonomousProcessCount(); i++ )
         {
         ENV_AUTO_PROCESS *pInfo = GetAutonomousProcessInfo( i );

         if ( pInfo->use && pInfo->inputVarFn != NULL )
            {
            MODEL_VAR *modelVarArray = NULL;
            int varCount = pInfo->inputVarFn( pInfo->id, &modelVarArray );
            count += varCount;
            }
         }
      }
...
int EnvModel::Run( int runFlag )
   ...
   // set new output path
   CString exportPath = PathManager::GetPath( PM_IDU_DIR );  // {IduDir}\Outputs\{ScenarioName} during the course of a run
   exportPath += "Outputs\\";

   // don't allow blanks or slashes in name
   CString scenarioName( gpScenarioManager->GetScenario( m_scenarioIndex )->m_name );
   CleanFileName( scenarioName );

   exportPath += scenarioName;
   exportPath += "\\";
   PathManager::SetPath( PM_OUTPUT_DIR, exportPath );
   ...
   //int appVarCount = GetAppVarCount(); 
   ...
   //char *string = new char[msg.GetLength() +1];
   
   //string = msg.GetBuffer(msg.GetLength() +1);
   
   //sprintf_s( string, 128, "Year: %i", this->m_currentYear );
   ...
   try
      ...
      while ( m_currentYear < m_endYear )  // m_currentYear is initialized in Reset
         ...
         Report::StatusMsg( "Running Autonomous Processes (Pre)..." );
         RunAutonomousProcesses( false );
         Report::StatusMsg( "Running Autonomous Processes (Pre)...Completed" );
         ...
         if ( m_pSocialNetwork )
            ...
            Report::StatusMsg( "Running Social Network...Completed" );
            }
         ...
         Report::StatusMsg( "Running Autonomous Processes (Post)..." );
         RunAutonomousProcesses( true );
         Report::StatusMsg( "Running Autonomous Processes (Post)...Completed" );
         ...
         if ( ( m_evalFreq > 0 ) && ( yearOfRun % m_evalFreq ) == 0 )
            {
            Report::StatusMsg( "Running Evaluation" );
            // generate landscape metrics to guide policy, cultural metaprocesses
            RunEvaluation();
            Report::StatusMsg( "Running Evaluation...Completed" );
            }
         ...
         m_pDataManager->CollectData( this, yearOfRun );  // 0-based
   ...
   //???Check - note: moved to ENDRUN notification in EnvDoc, handled in EnvView::EndRun()
   //ENV_ASSERT( m_pIDULayer->GetActiveField() >= 0  && gpMapPanel != NULL);
   //gpMapPanel->m_mapWnd.OnSelectField( gpMapPanel->m_pMap->GetActiveLayer()->GetActiveField() );
   
   
   
   /*???V5
   if ( gpMapPanel != &gpView->m_outputTab.m_mapTab && !( this->m_dynamicUpdate & 2 ) )
   {
   gpView->m_outputTab.m_mapTab.OnSelectField( gpView->m_outputTab.m_mapTab.m_pMap->GetActiveLayer()->GetActiveField() );
   }
   */
   /*
   switch ( m_runStatus )
   ...
   if ( m_exportOutputs )
      ...
      m_pDataManager->ExportRunData( exportPath ); //gpResultsPanel->ExportModelOutputs( -1, false );
      }
   ...
   for ( int i=0; i < m_apInfoArray.GetCount(); i++ )
      {
      ENV_AUTO_PROCESS *pInfo = m_apInfoArray.GetAt( i );

      if ( pInfo->use )
         {
         time.Format( _T("--%s: %g secs (Auto Process)" ), (LPCTSTR) pInfo->name, pInfo->runTime );
         time.Replace( _T("%"), _T("percent") );
         Report::LogMsg( time );
         }
      }

   for ( int i=0; i < this->m_modelInfoArray.GetCount(); i++ )
      {
      ENV_EVAL_MODEL *pInfo = m_modelInfoArray.GetAt( i );

      if ( pInfo->use )
         {
         time.Format( _T("--%s: %g secs (Eval Model)" ), (LPCTSTR) pInfo->name, pInfo->runTime );
         time.Replace( _T("%"), _T("percent") );
         Report::LogMsg( time );
         }
      }
...
void EnvModel::EndRun( void )
   ...
   for ( int i=0; i < modelCount; i++ )
      ...
      if ( pInfo->use && pInfo->endRunFn != NULL )
         ...
         BOOL ok = pInfo->endRunFn( &m_envContext );
   ...
   for ( int i=0; i < apCount; i++ )
      {
      ENV_AUTO_PROCESS *pInfo = m_apInfoArray[ i ];
      if ( pInfo->use && pInfo->endRunFn != NULL )
         {
         // call the function (eval models should return scores in the range of -3 to +3)
         Notify( EMNT_UPDATEUI, 16, (INT_PTR)(LPCTSTR) pInfo->name );
   ...
   Notify( EMNT_ENDRUN, 0 );   // update visualizers, stop recorders
...
void EnvModel::ApplyMandatoryPolicies()
   {
   int iduCount = m_pIDULayer->GetRecordCount();   // active records by default
   PolicyScoreArray policyScoreArray;
   policyScoreArray.SetSize( gpPolicyManager->GetPolicyCount() );

   if ( gpPolicyManager->GetMandatoryPolicyCount() == 0 )
      return;
   ...
   for ( MapLayer::Iterator idu = m_pIDULayer->Begin(); idu != m_pIDULayer->End(); idu++ )
      ...
      int mandatoryCount = CollectRelevantPolicies( idu, m_currentYear, PT_MANDATORY, policyScoreArray );
      ...
      for ( int i=0; i < mandatoryCount; i++ )
         ...
         if ( rn <= pPolicy->m_compliance )
            ApplyPolicy( pPolicy, idu, 0 );   // apply mandatory policy
         }
      }
   ...
   for ( int i=0; i < gpPolicyManager->GetPolicyCount(); i++ )
      ...
      if ( pPolicy->m_use && pPolicy->m_mandatory )
         {
         ...
         for ( MapLayer::Iterator cell = m_pIDULayer->Begin(); cell != m_pIDULayer->End(); cell++ )
            ...
            if ( DoesPolicyApply( pPolicy, cell ) )
               ...
               if ( rn <= pPolicy->m_compliance )
                  {
                  ApplyPolicy( pPolicy, cell, 0 );   // apply mandatory policy
                  ApplyDeltaArray( m_pIDULayer ); // in case a scheduled policy subdivides want to prevent another acting on the 'defunct' idu. can live with invalidated iterator.
                  }
               }
            }
...
void EnvModel::ApplyScheduledPolicies()
   ...
   for ( int i=0; i < gpPolicyManager->GetPolicyScheduleCount(); i++ )
      {
      POLICY_SCHEDULE &ps = gpPolicyManager->GetPolicySchedule( i );
...
void EnvModel::RunGlobalConstraints( void )   // initializes, applies maintenance costs
   {
   // first, set global constraints to zero
   ...
   if ( m_activeConstraintList.GetCount() > 0 )
      ...
      while( pos != NULL )
         ...
         pGC->ApplyPolicyConstraint( pConstraint, pInfo->idu, pInfo->applicationArea, false );  // maintenance
         
         // time to expire this one?
         pInfo->periodToDate++;
         if ( pInfo->periodToDate >= pConstraint->m_duration )
            {
            delete pInfo;
            pInfo = NULL;
...
void EnvModel::RunActorLoop()
   ...
   //gpPolicyManager->ResetPolicyAppliedCount();

   Notify( EMNT_ACTORLOOP, 0 );

   if ( m_shuffleActorIDUs )
      gpActorManager->ShuffleActors();

   int actorCount = gpActorManager->GetActorCount();   // this ensures any actor added this step are not invoked
...
//#pragma omp parallel for shared( consideredPolicyCount, appliedPolicyCount, consideredCellCount)
   ...
   //   #pragma omp atomic
      ...
      //if ( m_notifyProc != NULL && m_showRunProgress )
      //   m_notifyProc( m_consideredCellCount, m_extra );

      }  // end of:  for ( actor < actorCount )

   //ApplyDeltaArray( m_pIDULayer );  moved up to accomodate multicell outcomes...
...
bool EnvModel::DoesPolicyApply( Policy *pPolicy, int cell )
   ...
   for ( int i=0; i < constraintCount; i++ )
      ...
      // budget exists in global constraint, so what the new cost increment is
      float area = pPolicy->GetMaxApplicationArea( cell );  // idu area, or max expand area if an outcome include Expand()
...
float EnvModel::GetAltruismScore( int cell, float *polObjScores )
   ...
   for ( i=0, sc=0; i < metagoalCount; i++ )
      ...
      if ( pGoal->decisionUse & DU_ALTRUISM ) // is this goal used in alteruistic decision-making?
         {
         ENV_ASSERT( pGoal->pEvalModel != NULL );

         int modelIndex = this->GetModelIndexFromMetagoalIndex( i );
         ENV_ASSERT( 0 <= modelIndex && modelIndex < GetModelCount() );
...
////////
////////   // get the number of scores (one per model that is considered in the scarcity calculations)
////////   //int m = m_modelInfoArray.GetSize();
////////   int metagoalCount = this->GetMetagoalCount();
...
////////      ENV_ASSERT( 0 <= modelIndex && modelIndex < GetModelCount() );
////////
////////      if ( m_modelInfoArray[modelIndex]->decisionUse & DU_ALTRUISM )
////////         {
////////         ENV_ASSERT( -3.0 <= m_landscapeMetrics[modelIndex] && m_landscapeMetrics[modelIndex] <= 3.0 );
////////         s[scarcityIndex] = (-1.0f)*GetGsFromAbundance( m_landscapeMetrics[modelIndex], m_modelInfoArray[modelIndex]->metagoal );
...
float EnvModel::GetSocialNetworkScore( Actor *pActor, Policy *pPolicy )
   ...
   for ( int i=0; i < m_pSocialNetwork->GetLayerCount(); i++ )
      ...
      ENV_EVAL_MODEL *pModel = pLayer->m_pModel;  //
      
      int modelIndex = EnvModel::FindModelIndex( pModel->name );
      int metagoalIndex = EnvModel::GetMetagoalIndexFromModelIndex( modelIndex );
...
void EnvModel::ApplyPolicy( Policy *pPolicy, int cell, float score )
   {
   ...
   //
   if ( AddDelta( cell, m_colPolicy, m_currentYear, pPolicy->m_id, DT_POLICY ) < 0 )
      return;  

   bool readOnly = m_pIDULayer->m_readOnly;
   m_pIDULayer->m_readOnly = false;
   m_pIDULayer->SetData( cell, m_colScore, score );
   m_pIDULayer->m_readOnly = readOnly;

   AddDelta( cell, m_colLastDecision, m_currentYear, m_currentYear, DT_POLICY );
   ...
   if ( pPolicy->m_exclusive )
      ...
      AddDelta( cell, m_colNextDecision, m_currentYear, nextDecisionYear, DT_POLICY );
      }

   int apps;
   m_pIDULayer->GetData( cell, m_colPolicyApps, apps );
   AddDelta( cell, m_colPolicyApps, m_currentYear, apps+1, DT_POLICY );
   ...
   if ( pPolicy->GetMultiOutcomeCount() > 0 )  
      ...
      ApplyMultiOutcome( multiOutcome, cell, persistence );
      ...
      m_pIDULayer->GetData( cell, m_colArea, pPolicy->m_appliedArea );   // area of IDU policy is applied to
      }
   ...
   if ( constraintCount > 0 )
      ...
      for ( int i=0; i < constraintCount; i++ )
         ...
         if ( pGC != NULL )
            ...
            if ( pGC->ApplyPolicyConstraint( pConstraint, cell, totalArea, true ) == false )
               {
               //CString msg;
            ...
            if ( pConstraint->m_duration > 0 )   // maintenance costs only
               m_activeConstraintList.AddConstraint( pConstraint, cell, pPolicy->m_appliedArea );
            }
         }
      }
...
int EnvModel::RunCulturalMetaprocess()
   ...
   //int goalCount = m_modelInfoArray.GetSize();

   //// for each actor
   //for ( int j=0; j < gpActorManager->GetActorCount(); j++ )
   //   {
   //   Actor *pActor = gpActorManager->GetActor( j );

   //   // adjust actor weights appropriately
   //   // actors values step toward scarcity in goal space
   //   for ( int i=0; i < goalCount; i++ )
   //      {
   //      float s = (-1.0f)*m_modelInfoArray[ i ]->score;  // s is "scarcity"
...
void EnvModel::InitModels()   
   ...
   m_envContext.pExtensionInfo = NULL;         // opaque ptr to ENV_EVAL_MODEL or ENV_AUTO_PROCESS as appropriate; thus deprecates: id, col, score
#ifndef NO_MFC
   m_envContext.pWnd = NULL;
#endif
      ...
      for ( int i=0; i < GetAutonomousProcessCount(); i++ )
         ...
         if ( pInfo->use )
            ...
            if ( initFn != NULL )
               ...
               initFn( &m_envContext, (LPCTSTR) pInfo->initInfo );
            ...
            if ( pInfo->initRunOnStartup && pInfo->initRunFn != NULL )
               {
               CString msg = "Initializing ";
               msg += pInfo->name;
               msg += ": InitRun()";
               Report::StatusMsg( msg );
               pInfo->initRunFn( &m_envContext, true );
               }
            }
         }
      }
		...
		for (int i = 0; i < GetModelCount(); i++)
		{
			...
			if (pInfo->use)
			{
				...
				if (initFn != NULL)
				{
					...
					m_envContext.handle = pInfo->handle;
					m_envContext.pExtensionInfo = pInfo;
					pInfo->initFn(&m_envContext, pInfo->initInfo);
				}

				if (pInfo->use && pInfo->initRunOnStartup && pInfo->initRunFn != NULL)
				{
					CString msg = "Initializing ";
					msg += pInfo->name;
					msg += ": InitRun()";
   ...
   //ENV_AUTO_PROCESS *pInfo = m_pModel->GetMetaProcessInfo(0);
...
void EnvModel::InitRun()
   ...
   // where  n = 0,1,2,... 
   m_endYear = m_currentYear + m_yearsToRun;
   ...
   if ( m_runStatus == RS_PRERUN )
      ...
      int emCount  = (int) m_modelInfoArray.GetSize();
      ...
      for ( i=0; i < gpPolicyManager->GetPolicyCount(); i++ )
         m_resetInfo.policyArray.Add( new Policy( *gpPolicyManager->GetPolicy( i ) ) );

      // make copies of original actors.  Note that any "new" actors added during the
      // run are not used here.
      for ( i=0; i < gpActorManager->GetActorCount(); i++ )
         m_resetInfo.actorArray.Add( new Actor( *gpActorManager->GetActor( i ) ) );
      ...
      Notify( EMNT_INITRUN, m_currentRun );
         ...
         if ( pInfo->use )
            ...
            if ( initRunFn != NULL )
               ...
               if ( !ok )
                  {
                  CString msg;
                  msg.Format( "The APInitRun(.) function of %s returned false.  This autonomous process will not be used.", (PCTSTR) pInfo->name );
                  Report::ErrorMsg( msg );
      ...
      m_pDataManager->CollectData( this, 0 );    // collect initial output data prior to run
...
bool EnvModel::Reset()
   ...
   if ( m_runStatus != RS_PRERUN )
      ...
      ASSERT( m_resetInfo.policyArray.GetSize() == gpPolicyManager->GetPolicyCount() );
      ...
      ASSERT( m_resetInfo.actorArray.GetSize() == gpActorManager->GetActorCount() );
...
bool EnvModel::RunEvaluation( void )
   ...
   for ( int i=0; i < modelCount; i++ )
      ...
      if ( pInfo->use )
         ...
         //UpdateUI( 8, (LONG_PTR) m_envContext.pExtensionInfo );   // extension name
...
#ifndef NO_MFC
            if ( Report::WarningMsg( msg, "Warning", MB_YESNO ) == IDNO )
#endif
               reportErrors = false;
            }

         m_landscapeMetrics[i].scaled = pInfo->score     = score;
         m_landscapeMetrics[i].raw    = pInfo->rawScore  = m_envContext.rawScore;
         pInfo->pDataObj = m_envContext.pDataObj;
         }  // end of:  if ( pInfo->use )
      else
         {
         m_landscapeMetrics[i].scaled = pInfo->score    = 0;
         m_landscapeMetrics[i].raw    = pInfo->rawScore = 0;
         pInfo->pDataObj = NULL;
         }
      }  // end of:  for( i < modelCount )
...
{	
	PtrArray< ENV_PROCESS> modelInfoArray(false);
	for( int i = 0; i < m_modelInfoArray.GetSize(); i++)
		modelInfoArray.Add( m_modelInfoArray.GetAt(i) );

   EnvProcessScheduler::init( modelInfoArray );
   int scaleError;
   static bool reportErrors = true;  
   ...
   while ( i < modelCount )
      {
      PtrArray<ENV_PROCESS> runnableModels(false);
      EnvProcessScheduler::getRunnableProcesses( modelInfoArray, runnableModels );
      while( runnableModels.GetSize() > 0 )
         {
         ProcessWinMsg();
         if( EnvProcessScheduler::countActiveThreads() < EnvProcessScheduler::m_maxThreadNum )
            {
            ENV_EVAL_MODEL *pInfo = dynamic_cast<ENV_EVAL_MODEL*>( runnableModels.GetAt(0) );//m_modelInfoArray[ i ];
            ...
            if( pInfo )
               ...
               if ( pInfo->use )
                  ...
                  *pEnvContext = m_envContext;
                  ...
                  //	if ( Report::WarningMsg( msg, MB_YESNO ) == IDNO )
                  //		reportErrors = false;
                  //}
      ...
      for( ; itr != itrEnd; itr++ )
         ...
         {
         ENV_EVAL_MODEL* pInfo = NULL;
         int t = -1;
         for( int cnt = 0; cnt < modelCount; cnt++ )
            {
            if( itr->second.id == m_modelInfoArray[cnt] )
               // is this id unique in the list??
               {
               t = cnt;
               pInfo = m_modelInfoArray[cnt];
               break;
               }
            }
         if( pInfo && t >= 0 )
            ...
            if( itr->second.pEnvContext != NULL )
               ...
               m_landscapeMetrics[t].scaled = pInfo->score     = score;
               m_landscapeMetrics[t].raw    = pInfo->rawScore  = pEnvContext->rawScore;
               pInfo->pDataObj = pEnvContext->pDataObj;
               /////////////////////////////////////
               ...
               {				
               m_landscapeMetrics[t].scaled = pInfo->score    = 0;
               m_landscapeMetrics[t].raw    = pInfo->rawScore = 0;
               pInfo->pDataObj = NULL;
               }
            }			
         }
...
///////////////////////////////////////////////////////////////////////////////////////////
void EnvModel::RunAutonomousProcesses( bool isPostYear )
   {
#ifndef NO_MFC
   if ( m_runParallel )
      return RunAutonomousProcessesParallel( isPostYear );
#endif

   int count = (int) m_apInfoArray.GetSize();
   ...
   for ( INT_PTR i=0; i < count; i++ )
      {
      ENV_AUTO_PROCESS *pInfo = m_apInfoArray[ i ];
      if ( pInfo->use && pInfo->timing == timing && pInfo->runFn != NULL )
         {
         m_envContext.pExtensionInfo  = static_cast<ENV_PROCESS*>(pInfo);
         m_envContext.id = pInfo->id;
         m_envContext.handle = pInfo->handle;
         m_envContext.col = pInfo->col;
         m_envContext.firstUnseenDelta = m_apFirstUnseenDelta[i];
         ...
         //UpdateUI( 8, (LONG_PTR) m_envContext.pExtensionInfo );   // extension info
         //Notify( EMNT_UPDATEUI, 8, (INT_PTR) m_envContext.pExtensionInfo );
...
///////////////////////////////////////////////////////////////////////////////////////////
#ifndef NO_MFC
void EnvModel::RunAutonomousProcessesParallel( bool isPostYear )
   {
   PtrArray< ENV_PROCESS> apInfoArray(false);
   for( int i = 0; i < m_apInfoArray.GetSize(); i++)
      apInfoArray.Add( m_apInfoArray.GetAt(i) );
   ...
   while( i < count )
      {
      PtrArray<ENV_PROCESS> runnableProcesses(false);
      ...
      while( runnableProcesses.GetSize() > 0 )
         ...
         if(  EnvProcessScheduler::countActiveThreads() < EnvProcessScheduler::m_maxThreadNum )
            {
            ENV_AUTO_PROCESS* pInfo = dynamic_cast<ENV_AUTO_PROCESS*>(runnableProcesses.GetAt(0));
...
void EnvModel::RunVisualizers( bool isPostYear )
   ...
   for ( INT_PTR i=0; i < count; i++ )
      {
      ENV_VISUALIZER *pInfo = m_vizInfoArray[ i ];
      if ( pInfo->use && pInfo->runFn != NULL )
         {
         m_envContext.pExtensionInfo  = static_cast<ENV_VISUALIZER*>(pInfo);
         m_envContext.id = pInfo->id;
         m_envContext.handle = pInfo->handle;
         m_envContext.col = -1;
...
int EnvModel::ApplyDelta( MapLayer *pLayer, DELTA &delta )
   ...
   // SubDivide
      ...
      if ( 0 <= actor && actor < gpActorManager->GetActorCount() )
         {
         pActor = gpActorManager->GetActor( actor );
...
void EnvModel::StoreIDUCols()
   ...
   if ( m_colActor < 0 && gpActorManager->m_actorInitMethod == AIM_IDU_GROUPS )
      {
   ...
   if ( m_colUtility < 0 && gpModel->m_decisionElements & DE_UTILITY )
      {
      ...
      if ( pInfo->col != -1 )
         ...
         if ( col < 0 )
            ...
            msg.Format( "Evaluative Model, %s, field, %s, is missing; and will be added.  ",
               (PCTSTR) pInfo->name, (PCTSTR) pInfo->fieldName);
            Report::InfoMsg( msg );
...
void EnvModel::SetIDULayer( MapLayer *pLayer )
   ...
   m_pExprEngine = new MapExprEngine( pLayer );
...
bool EnvModel::EvaluateExpr( MapExpr *pMapExpr, bool useQuery, float &result )
 ...
 void EnvModel::CreateModelAppVars( void )
   ...
   for ( int i=0; i < GetAutonomousProcessCount(); i++ )
      ...
      if ( pInfo->outputVarFn != NULL )
         ...
         for ( int j=0; j < varCount; j++ )
            ...
            //CString msg;
            //msg.Format( "%s ModelVar %i of %i, name:%s", (LPCTSTR) pInfo->name, 
            //   j, varCount, pModelVar->name );
            //Report::LogMsg( msg );
   ...
   for ( int i=0; i < GetModelCount(); i++ )
      ...
      pAppVar->m_pProcessInfo = pInfo;
      pAppVar->SetValue( VData( &(pInfo->rawScore), TYPE_FLOAT, true ) );
      ...
      pAppVar->m_pProcessInfo = pInfo;
      pAppVar->SetValue( VData( &(pInfo->score), TYPE_FLOAT, true ) );
      AddAppVar( pAppVar, false );
...
bool EnvModel::InitSocialNetwork( void )
   ...
   m_pSocialNetwork = new SocialNetwork;
...
int EnvModel::ChangeIDUActor( EnvContext *pContext, int idu, int groupID, bool randomize )
   ...
   if ( gpActorManager->m_actorInitMethod != AIM_IDU_GROUPS )  // only ACTOR GROUP defined in ACTOR field allowed to change
      return -2;

   ActorGroup *pGroup =  gpActorManager->GetActorGroupFromID( groupID );

   if ( pGroup == NULL )
      return -3;

   int index = -1;
   Actor *pOldActor = gpActorManager->GetActorFromIDU( idu, &index );
   ...
   Actor *pNewActor = gpActorManager->CreateActorFromGroup( pGroup, randomize );  // creates actors, sets values
   if ( pNewActor == NULL )
      return -5;
   ...
   //   gpActorManager->RemoveActor( pOldActor->m_index );

   gpActorManager->AddActor( pNewActor );
...
int EnvModel::RunSensitivityAnalysis()
      ...
      if ( pModel->inputVarFn != NULL )
         ...
         for ( int j=0; j < varCount; j++ )
            ...
            if ( mv.useInSensitivity && mv.pVar != NULL )
               ...
               saModelCount++;
               CString name;
               name.Format( "%s.%s", (LPCTSTR) pModel->name, (LPCTSTR) mv.name );
               saNameArray.Add( name );
               saPtrArray.Add( &mv );
               m_iterationsToRun++;
               }
   ...
   for ( int i=0; i < apCount; i++ )
	   {
      ENV_AUTO_PROCESS *pModel = GetAutonomousProcessInfo( i );
      if ( pModel->inputVarFn != NULL )
         {
         MODEL_VAR *modelVarArray = NULL;
         int varCount = pModel->inputVarFn( pModel->id, &modelVarArray );
...
void EnvModel::Notify( EM_NOTIFYTYPE type, INT_PTR param  )
   {
   if ( EnvModel::m_notifyProc == NULL )
      return;

   m_notifyProc( type, param, m_extra );
   }
...
bool EnvModel::VerifyIDULayer( void )
   ...
   if ( badAreaCount || badLulcCount || zeroLulcCount || badLulc || badFieldCount )
      ...
      msg += msgDetails;
      Report::WarningMsg( msg, "Warning", MB_OK | MB_ICONWARNING );
...
void EnvModel::ApplySubstituteStrings(CString& str)
{
   if (str.Find('{') < 0) return;

   // iterate through the substitute stringss, applying substitutions as appropriate
   for (int i = 0; i < (int)m_envContext.m_substituteStrings.GetSize(); i++)
   {
      SubstituteString * pSubstitute = m_envContext.m_substituteStrings.GetAt(i);
