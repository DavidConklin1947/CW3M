...
ColumnTrace::ColumnTrace( MapLayer *pLayer, int column, int run /*= -1*/, const DeltaArray *pDeltaArray /*= NULL*/ )
:  m_run( run ),
   ...
   m_rows = EnvModel::m_pIDULayer->GetRecordCount( MapLayer::ALL );
   ...
   if (! pDeltaArray)
      m_pDeltaArray = gpModel->m_pDataManager->GetDeltaArray( run );
   else
   ...
   for ( int row=0; row < m_rows; row++ )
      {
      bool ok = EnvModel::m_pIDULayer->GetData( row, m_columnIndex, data );
      ASSERT( ok );
   ...
   if ( gpModel->m_currentYear > 0 ) 
      {
      DeltaArray *pDeltaArray = gpModel->m_pDeltaArray;
      ASSERT (pDeltaArray);

      for (INT_PTR i=pDeltaArray->GetFirstUnapplied(pLayer)-1; i >= 0; i-- )
         {
