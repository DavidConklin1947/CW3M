...
void SNLayer::LSReact()  //get input node (landscape signal) and compare it to the threshold of each node it connects to.  
{
   ...
   SNNode* InputNode = GetInputNode();
	...
	{	
		...
		{
			if (abs(temp->m_pToNode->m_landscapeGoal - LandscapeScore)*temp->m_weight > threshold)   //if (distance between the orgs goal and the landscape signal)*reactivity of org/importance of metric to the org > threshold
			{
...
bool SocialNetwork::LoadXml( LPCTSTR filename )
   ...
   while ( pXmlLayer != NULL )
      ...
      bool ok = TiXmlGetAttributes( pXmlLayer, attrs, "layer", filename );
      ...
      if ( ok )
         ...
         pLayer->m_pModel = gpModel->FindModelInfo( pLayer->m_name );
         ...
         int actorGroupCount = (int) gpActorManager->GetActorGroupCount();
         for ( int i=0; i < actorGroupCount; i++ )
            pLayer->AddNode( NNT_OUTPUT, gpActorManager->GetActorGroup( i )->m_name, 5.0 );//third argument should be 'landscape goal'
...
void SocialNetwork::SetInputActivations( void )
   ...
   for ( int i=0; i < this->GetLayerCount(); i++ )
     ...
     pLayer->SetInCountTable();
      ...
      float activation = pLayer->m_pModel->score;    // -3 to +3, +3=abundant
...
float SocialNetwork::GetMetricValue( int layer, int i )
 ...
 int SNLayer::Activate( void )
   ...
   for (int i = 0; i < interiorConnections.GetSize(); i++)
		{
			SNConnection *pconnection  = interiorConnections[i];
			pconnection->m_weight += (threshold - abs(pconnection->m_pFromNode->m_activationLevel - pconnection->m_pToNode->m_activationLevel))*((pconnection->m_pFromNode->m_activationLevel + pconnection->m_pToNode->m_activationLevel)/2)/(threshold);
		}
