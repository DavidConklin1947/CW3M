...
BOOL DeltaExportDlg::OnInitDialog()
   ...
   int runs = gpDataManager->GetRunCount();

   for ( int i=0; i < runs; i++ )
      {
      //RUN_INFO &ri = gpDataManager->GetRunInfo( i );
...
void DeltaExportDlg::OnOK()
   ...
   DeltaArray *pDA = gpDataManager->GetDeltaArray( run );
   ASSERT( pDA != NULL );
...
void DeltaExportDlg::SetDefaultPath()
   ...
   RUN_INFO &ri = gpDataManager->GetRunInfo( run );
