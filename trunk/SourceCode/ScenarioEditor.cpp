...
void ScenarioEditor::LoadVarTree()
   ...
   //      hItem = m_inputVars.InsertItem( pInfo->name, m_hModelUsage );
   //      m_inputVars.SetItemData( hItem, varIndex );   // index of the associated ENV_EVAL_MODEL structure
   //      varIndex++;
   //      }
   //   }
   //
   //int processCount = EnvModel::GetAutonomousProcessCount();
   //
   //if ( processCount > 0 )
   //   {
   ...
   //      hItem = m_inputVars.InsertItem( pInfo->name, m_hProcessUsage );
   //      m_inputVars.SetItemData( hItem, varIndex );   // index of the associated ENV_AUTO_PROCESS structure
   //      varIndex++;
   //      }
   //   }
   ...
   if ( modelCount > 0 )
      ...
      for( int i=0; i < modelCount; i++ )
         ...
         if ( pModel->inputVarFn != NULL )
            ...
            if ( varCount > 0 )
               {
               HTREEITEM hModelItem = m_inputVars.InsertItem( pModel->name, m_hModels ); 
   ...
   if ( apCount > 0 )
      ...
      for( int i=0; i < apCount; i++ )
         ...
         if ( pAP->inputVarFn != NULL )
            ...
            if ( varCount > 0 )
               {
               HTREEITEM hProcessItem = m_inputVars.InsertItem( pAP->name, m_hProcesses ); 
...
void ScenarioEditor::LoadModelVar()
   ...
   if ( hItem != NULL )
      ...
      // or is it a model/process input variable
         ...
         VAR_INFO &var = m_pScenario->GetVarInfo( varInfoIndex );
         m_pInfo = &var;
...
void ScenarioEditor::OnBnClickedAdd()
   {
   m_pScenario = new Scenario( "New Scenario" );
