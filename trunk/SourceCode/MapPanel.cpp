         ...
         if ( pLayer == NULL )
            ...
            switch( pLayer->m_layerType )
               ...
               case LT_LINE:
                  ...
                  CString msg;
                  msg.Format( "%s - PolyID: %d   ", (LPCSTR) pLayer->m_name, pPoly->m_id );
...
void SetupBins( Map *pMap, int layerIndex, int nID /*=-1*/, bool force /*= false*/ )
   ...
   if ( layerIndex == 0 )
      ...
      int colPolicy  = pLayer->GetFieldCol( "POLICY" ); // EnvModel::m_colPolicy;
      ...
      // anything else 
         ...
         for ( int i=0; i < EnvModel::m_colActorValues.GetSize(); i++ )
            {
            if ( nID == EnvModel::m_colActorValues[ i ] )
               {
               isValueField = true;
...
void MapPanel::OnRunQuery() 
   ...
   QueryDlg dlg( pLayer, m_mapFrame.m_currentPoly );
   dlg.DoModal();
   }
