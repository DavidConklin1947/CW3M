...
void PolEditor::MakeLocalCopyOfPolicies()
   {   
   m_policyArray.RemoveAll();
   m_policyArray = gpPolicyManager->GetPolicyArray();
...
bool PolEditor::StoreChanges()
   ...
   PolicyManager::m_pQueryEngine->ShowCompilerErrors( false );
...
void PolEditor::ShowSites()
   ...
   Query *pQuery = PolicyManager::m_pQueryEngine->ParseQuery( query, 0, "Policy Editor" );
   if ( pQuery == NULL )
      {
   ...
   int count = PolicyManager::m_pQueryEngine->SelectQuery( pQuery, true ); //, true );
