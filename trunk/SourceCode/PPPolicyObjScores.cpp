...
int ScoreGrid::AppendRow()
   ...
   for ( int i=0; i < metagoalCount; i++ )
      {
      goals += EnvModel::GetMetagoalInfo( i )->name;
      goals += "\n";
      }
...
bool PPPolicyObjScores::StoreObjChanges()
   ...
   for ( int i=0; i < m_scoreGrid.GetNumberRows(); i++ )
      ...
      Query *pQuery = PolicyManager::m_pQueryEngine->ParseQuery( query, 0, "Policy Objectives Scores Setup" );
      if ( pQuery == NULL )
   ...
   for ( int i=0; i < m_modifierGrid.GetNumberRows(); i++ )
      ...
      if ( _type != SM_SCENARIO )
         {
         Query *pQuery = PolicyManager::m_pQueryEngine->ParseQuery( condition, 0, "Policy Modifier Setup" );
         if ( pQuery == NULL )
            {
