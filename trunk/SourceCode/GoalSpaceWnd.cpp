...
int GoalSpaceWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
   ...
   for ( int i=0; i < resultsCount; i++ )
      ...
      ENV_EVAL_MODEL *pModel = EnvModel::GetModelInfo( modelIndex );

      if ( pModel->use )
         {
         m_goalCombo0.AddString( pModel->name );
         m_goalCombo1.AddString( pModel->name );
         m_goalCombo2.AddString( pModel->name );
         }
      }
...
void GoalSpaceWnd::RefreshData()
   ...
   //// Actor Values
   //int goalCount  = EnvModel::GetModelCount();
...
void GoalSpaceWnd::OnGoalChange()
   ...
   m_plot.SetAxesLabels( EnvModel::GetModelInfo( g0 )->name, EnvModel::GetModelInfo( g1 )->name, EnvModel::GetModelInfo( g2 )->name );
