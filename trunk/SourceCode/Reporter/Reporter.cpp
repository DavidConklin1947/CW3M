...
BOOL Reporter::Init( EnvContext *pEnvContext, LPCTSTR initStr ) 
   { 
   EnvAutoProcess::Init( pEnvContext, initStr );

   m_pMapExprEngine = new MapExprEngine( (MapLayer*) pEnvContext->pMapLayer );
   
   bool ok = LoadXml( pEnvContext, initStr );   // sets up all the various dataobjs
...
BOOL Reporter::Run( EnvContext *pEnvContext  )
   ...
   for ( MapLayer::Iterator idu = pLayer->Begin(); idu != pLayer->End(); idu++ )
      ...
      EvaluateOutputs( m_outputArray, pLayer, idu, area );

      // evaluate groups
      for ( int j=0; j < (int) m_outputGroupArray.GetSize(); j++ )
         {
         OutputGroup *pGroup = m_outputGroupArray[ j ];
         EvaluateOutputs( pGroup->m_outputArray, pLayer, idu, area );
         } 
      }  // end of:  for each IDU
...
BOOL Reporter::EndRun( EnvContext *pContext )
   {
 ...
 void Reporter::EvaluateOutputs( PtrArray < Output > &outputArray, MapLayer *pLayer, int currentIDU, float area )
   ...
   for ( int k=0; k < (int) outputArray.GetSize(); k++ )
      ...
      if ( pOutput->m_use )
         {
         double value = 0;
         bool ok = m_pMapExprEngine->EvaluateExpr( pOutput->m_pMapExpr, true );
...
bool Reporter::LoadXml( EnvContext *pEnvContext, LPCTSTR filename )
   ...
   while( pXmlConst != NULL )
      ...
      ok = TiXmlGetAttributes( pXmlConst, constAttrs, filename, NULL );
      if ( ok )
   ...
   while( pXmlInput != NULL )
      ...
      ok = TiXmlGetAttributes( pXmlInput, inputAttrs, filename, NULL );
      if ( ok )
         {
         // add it to the list
   ...
   if ( LoadXmlOutputs( pXmlRoot, NULL, pLayer, filename ) > 0 )
      {
      CString msg;
   ...
   while( pXmlOutputGroup != NULL )
      ...
      XML_ATTR groupAttrs[] = {
         ...
         { "stratify_by",   TYPE_STRING,   &stratifyBy,                 false, 0 },
         { "pivot_table",   TYPE_INT,      &(pGroup->m_makePivotTable), false, 0 },
         { NULL,            TYPE_NULL,     NULL,                        false, 0 } };

      ok = TiXmlGetAttributes( pXmlOutputGroup, groupAttrs, filename, pLayer );
      if ( ! ok )
         {
         delete pGroup;
         continue;
      ...
      if ( stratifyBy != NULL )
         pGroup->ParseFieldSpec( stratifyBy, pLayer );

      m_outputGroupArray.Add( pGroup );

      // get outputs
      LoadXmlOutputs( pXmlOutputGroup, pGroup, pLayer, filename );
...
int Reporter::LoadXmlOutputs( TiXmlElement *pXmlParent, OutputGroup *pGroup, MapLayer *pLayer, LPCTSTR filename )
   {
   ...
   while( pXmlOutput != NULL )
      ...
      if ( pGroup && pGroup->m_stratifyByStr.GetLength() > 0 )
         stratifyBy = pGroup->m_stratifyByStr;
      ...
      XML_ATTR outputAttrs[] = {
         ...
         { "stratify_by",      TYPE_CSTRING,  &stratifyBy,                  false, 0 },
         { "pivot_table",      TYPE_BOOL,     &(pOutput->m_makePivotTable), false, 0 },
         { NULL,               TYPE_NULL,     NULL,                         false, 0 } };
   
      bool ok = TiXmlGetAttributes( pXmlOutput, outputAttrs, filename, pLayer );
      if ( ok )
         {
         switch (type[0])
            {
         ...
         pOutput->m_pMapExpr = m_pMapExprEngine->AddExpr( pOutput->m_name, expr, pOutput->m_query );
         ...
         if ( pOutput->m_use && stratifyBy.GetLength() > 0 && pLayer->GetFieldCol( stratifyBy ) )
            {
            int fields = pOutput->ParseFieldSpec( stratifyBy, pLayer );
