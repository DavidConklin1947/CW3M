...
class ObjectiveArray : public CArray< ObjectiveScores*, ObjectiveScores* >
{
public:
...
class OutcomeDirective
   ...
   public:
      ...
      OutcomeDirective( LPCSTR typeStr, int _period ) : period( _period ) { outcome.SetNull(); SetType( typeStr); }
      OutcomeDirective( LPCSTR typeStr, int _period, LPCSTR value ) : period( _period ), outcome( value ) { SetType( typeStr ); }
      OutcomeDirective( LPCSTR typeStr, int _period, float  value ) : period( _period ), outcome( value ) { SetType( typeStr ); }
      OutcomeDirective( LPCSTR typeStr, int _period, double value ) : period( _period ), outcome( value ) { SetType( typeStr ); }
      OutcomeDirective( LPCSTR typeStr, int _period, int    value ) : period( _period ), outcome( value ) { SetType( typeStr ); }

      OutcomeDirective(const OutcomeDirective &od ) : type( od.type ), period( od.period ), outcome( od.outcome ) { }

      void ToString( CString& str );

      void SetType( LPCSTR typeStr )
         {
         switch( *typeStr )
            {
...
class OutcomeFunction
   ...
   public:
      ...
      static OutcomeFunction* MakeFunction( LPCSTR functionName, Policy *pPolicy );
   };
...
class OutcomeInfo
   ...
   public:
      ...
      OutcomeInfo( LPCSTR _field, VData _value, int _delay, OutcomeDirective *pDirective=NULL );
//         : value( _value ), col( -1 ), m_pFunction( NULL ), m_pDirective( pDirective )
//         { lstrcpy( field, _field ); }

      OutcomeInfo( OutcomeFunction *pFunction ) 
         : col( -1 ), m_pFunction( pFunction ), m_pDirective( NULL ), m_pMapExpr( NULL )
         { field[ 0 ]=NULL; value.type = TYPE_NULL; }

      OutcomeInfo( ) : col( -1 ), m_pFunction( NULL ), m_pDirective( NULL ), m_pMapExpr( NULL ) { field[ 0 ]=NULL; value.type = TYPE_NULL; }
...
class BufferOutcomeFunction : public OutcomeFunction
   {
   //
   // Entry syntax:
   ...
   public:
      ...
      virtual void ToString( CString &str ) { str.Format( "Buffer( %s, %s, %s, %s )", 
         (LPCSTR) m_pArgs->GetAt( 0 ), (LPCSTR) m_pArgs->GetAt( 1 ),
         (LPCSTR) m_pArgs->GetAt(2), (LPCSTR) m_pArgs->GetAt(3) ); }
...
class AddPointOutcomeFunction : public OutcomeFunction
   {
   //
   // Entry syntax:
   ...
   public:
      ...
      virtual void ToString( CString &str ) { str.Format( "AddPoint( %s, %s, %s )", 
         (LPCSTR) m_pArgs->GetAt( 0 ), (LPCSTR) m_pArgs->GetAt( 1 ), (LPCSTR) m_pArgs->GetAt(2) ); }
...
class IncrementOutcomeFunction : public OutcomeFunction
   ...
   public:
      ...
      virtual void ToString( CString &str ) { str.Format( "Increment( %s, %s )", (LPCSTR) m_pArgs->GetAt( 0 ), (LPCSTR) m_pArgs->GetAt( 1 ) ); }
...
// Maintenance costs are tracked in a list (EnvModel::m_activeConstraintList)
...
{
public:
   GlobalConstraint( LPCTSTR name, CTYPE type, LPCTSTR expr );
   ...
   GlobalConstraint & operator = ( GlobalConstraint & gc )
       { m_name = gc.m_name; m_type = gc.m_type; m_value = gc.m_value; m_currentValue = gc.m_currentValue; m_expression = gc.m_expression; gc.m_pMapExpr = gc.m_pMapExpr; return *this; }   
...
public:
   PolicyConstraint( Policy *pPolicy, LPCTSTR name, CBASIS basis, LPCTSTR initCostExpr, LPCTSTR maintenanceCostExpr, int duration, LPCTSTR lookup );
   ...
   bool ParseCost( LPCTSTR costExpr, float &cost, int &col );
...
protected:
   ...
   bool  GetCost( int idu, float costToUse, int colCost, float &cost );
...
public:
   ...
   float GetGoalScore( int metagoalIndex );  // -1 <= metagoalIndex < EnvModel::GetMetagoalCount();  use -1 for global objective
...
class PolicyArray : public CArray< Policy*, Policy* >
{
public:
...
public:
   ...
   static void SetQueryEngine( QueryEngine *pQE ) { m_pQueryEngine = pQE; }
   ...
   Policy *FindPolicy( LPCSTR name, int *index=NULL );
   ...
   static QueryEngine *m_pQueryEngine;  // memory managed elsewhere.  Must be set before and policies are added
