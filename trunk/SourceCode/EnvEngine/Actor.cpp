...
int ActorManager::CreateActorsFromIduWts( void )
   ...
   for ( MapLayer::Iterator i = EnvModel::m_pIDULayer->Begin(MapLayer::ALL); i != EnvModel::m_pIDULayer->End(MapLayer::ALL); ++i)
      {
      ...
      for ( int val=0; val < valueCount; ++val )
         { 
         EnvModel::m_pIDULayer->GetData((int)i, EnvModel::m_colActorValues[ val ], wt );
         
         if ( wt == EnvModel::m_pIDULayer->GetNoDataValue() )
            {
            wt = 0.0f;
            badActorCount++;               
            }
            ...
            msg.Format("IDU coverage error specifying actor weights: Invalid value in record %d of col %s" ,
               (int)i, EnvModel::m_pIDULayer->GetFieldLabel(EnvModel::m_colActorValues[val]));
            Report::ErrorMsg(msg);
      ...
      ASSERT( EnvModel::m_colActor >= 0 );
      EnvModel::m_pIDULayer->SetData((int) i, EnvModel::m_colActor, pActor->m_index ); // SUBDIVIDE fails without this column
...
int ActorManager::CreateActorsFromIduGroups( void )
   ...
   int colGroupID = EnvModel::m_pIDULayer->GetFieldCol( _T( "ACTOR" ) );
   if ( colGroupID < 0 )
      {
   ...
   for ( MapLayer::Iterator i = EnvModel::m_pIDULayer->Begin(MapLayer::ALL); i != EnvModel::m_pIDULayer->End(MapLayer::ALL); ++i)
      {
      bool ok = EnvModel::m_pIDULayer->GetData( i, colGroupID, groupID );
      ASSERT( ok );

      if ( groupID == EnvModel::m_pIDULayer->GetNoDataValue() )
         continue;
      ...
      ASSERT( EnvModel::m_colActor >= 0 );
      //EnvModel::m_pIDULayer->SetData((int) i, EnvModel::m_colActor, pActor->m_index ); 
...
int ActorManager::CreateActorsFromQuery( void )
   ...
   if ( colGroupID < 0 )
      ...
      colGroupID = EnvModel::m_pIDULayer->m_pDbTable->AddField( _T("ACTOR"), TYPE_INT );
      }

   QueryEngine qe( EnvModel::m_pIDULayer );
   ...
   for ( int i=0; i < groupCount; i++ )
      ...
      int selCount = EnvModel::m_pIDULayer->GetSelectionCount();

      for ( int j=0; j < selCount; j++ )
         {
         int idu = EnvModel::m_pIDULayer->GetSelection( j );
         EnvModel::m_pIDULayer->SetData( idu, colGroupID, pGroup->m_id );
         }
...
int ActorManager::CreateActorUniform()
   ...
   if ( GetActorGroupCount() == 0 )
      ...
      pGroup->SetValueCount( EnvModel::GetActorValueCount() );
   ...
   pActor->SetPolyCount( EnvModel::m_pIDULayer->GetPolygonCount( MapLayer::ALL ) );
...
int ActorManager::AddActorSelfInterestColumns( bool bForce /*=false*/)
   {
   int err = 0;
   int actorValueCount = EnvModel::GetActorValueCount();
   ...
   for ( int i=0; i < actorValueCount; i++ )
      ...
      //ENV_EVAL_MODEL* pInfo = EnvModel::GetActorValueModelInfo( i );
      CString field;
      ...
      int col = EnvModel::m_pIDULayer->GetFieldCol( field );
      if ( col < 0 )
         {
         if ( ! bForce )
            {
            ...
            {
            col = EnvModel::m_pIDULayer->m_pDbTable->AddField( field, TYPE_FLOAT );
            //???? gpDoc->SetChanged( CHANGED_COVERAGE );
            }
         }

      EnvModel::m_colActorValues.Add( col );
      }
...
void ActorManager::CopyActorValuesToCells()
   ...
   for ( int i=0; i < valueCount; i++ )
      EnvModel::m_pIDULayer->SetColNoData( EnvModel::m_colActorValues[ i ] );
   ...
   for ( int actor=0; actor < actorCount; actor++ )
      ...
      for ( int i=0; i < cellCount; i++ )
         ...
         for ( int j=0; j < valueCount; j++ )
            EnvModel::m_pIDULayer->SetData( cell, EnvModel::m_colActorValues[ j ], pActor->GetValue( j ) );         
...
int ActorManager::LoadXml( TiXmlNode *pXmlActors, bool appendToExisting )
   ...
   while ( ( pXmlActorNode = pXmlActors->IterateChildren( pXmlActorNode ) ) != NULL )
      ...
      if ( group.m_decisionElements < 0 )
         group.m_decisionElements = gpModel->m_decisionElements;
      ...
      while ( pGoalScoreNode = pGoalScoresNode->IterateChildren( pGoalScoreNode ) )
         ...
         if ( ! goal.IsEmpty() )
            ...
            int metagoalIndex = EnvModel::FindMetagoalIndex( goal );
            if ( metagoalIndex < 0 )
               {
               ...
               {
               int goalIndex = EnvModel::GetActorValueIndexFromMetagoalIndex( metagoalIndex );
      ...
      if ( j != actorValueCount && showMessage )
         ...
         msg.Format( "Mismatched value counts for actor %s (%i): XML input has %i, while %i models specify value maps", 
             (LPCSTR) pGroup->m_name, pGroup->m_id, j, actorValueCount );
         Report::WarningMsg( msg );
         showMessage = false;
         }
...
int ActorManager::SaveXml( FILE *fp, bool includeHdr, bool useFileRef  )
   ...
   for ( int i=0; i < GetActorGroupCount(); i++ )
      ...
      for ( int j=0; j < actorValueCount; j++ )
         ...
         //ENV_EVAL_MODEL *pInfo = EnvModel::GetModelInfo( modelIndex );
         METAGOAL *pInfo = EnvModel::GetActorValueMetagoalInfo( j );
