...
bool BufferOutcomeFunction::m_enabled = true;
...
GOAL_SCORE::~GOAL_SCORE()
   {
   if ( pQuery )
      PolicyManager::m_pQueryEngine->RemoveQuery( pQuery );  // deletes the query
   }

SCORE_MODIFIER::~SCORE_MODIFIER()
   {
   if ( pQuery )
      PolicyManager::m_pQueryEngine->RemoveQuery( pQuery );
   }
...
OutcomeInfo::OutcomeInfo( LPCSTR _field, VData _value, int _delay, OutcomeDirective *pDirective ) 
   : value( _value )
   , col( -1 )
   , m_pMapExpr( NULL )
   , m_pFunction( NULL )
   ...
   if ( value.GetType() == TYPE_STRING || value.GetType() == TYPE_DSTRING )
      {
      CString name;
      name.Format("Policy_%i", gpPolicyManager->m_nextPolicyID );

      m_pMapExpr = gpModel->m_pExprEngine->AddExpr( (PCTSTR) name, value.val.vString, NULL );
...
void OutcomeInfo::ToString( CString &outcomeStr, bool useHtml )
   ...
   MAP_FIELD_INFO *pInfo = EnvModel::m_pIDULayer->FindFieldInfo( field );
...
GlobalConstraint::GlobalConstraint( LPCTSTR name, CTYPE type, LPCTSTR expr )
: m_name( name )
, m_type( type )
, m_expression( expr )
, m_pMapExpr( NULL )
...
, m_currentInitValue( 0 )
   ...
   m_pMapExpr = gpModel->AddExpr( name, expr );
   ASSERT ( m_pMapExpr->IsGlobal() );
   if ( gpModel->EvaluateExpr( m_pMapExpr, false, m_value ) == false )
      {
      CString msg;
...
void GlobalConstraint::Reset( void  )
   ...
   if ( m_pMapExpr != NULL && m_pMapExpr->IsGlobal() )
      {
      bool ok = gpModel->EvaluateExpr( m_pMapExpr, false, m_value );
...
bool GlobalConstraint::ApplyPolicyConstraint( PolicyConstraint *pConstraint, int idu, float area, bool useInitCost )
   ...
   switch( m_type )
      ...
      case CT_RESOURCE_LIMIT:
         ...
         if ( cost <= 0 && gpModel->m_addReturnsToBudget == false )      // generating a return?  Then ignore
            return true;
...
PolicyConstraint::PolicyConstraint( Policy *pPolicy, LPCTSTR name, CBASIS basis, LPCTSTR initCostExpr, LPCTSTR maintenanceCostExpr, int duration, LPCTSTR lookupExpr ) 
      : m_pPolicy( pPolicy )
      , m_name( name )
      , m_gcName( "" )
      , m_basis( basis )
         ...
         *end = NULL;
         ...
         if ( ! found )
            ...
            if ( bslash || fslash )
               ...
               {
               path = EnvModel::m_pIDULayer->m_path;
               path.MakeFullPath();
               path.RemoveFileSpec();
         ...
         m_pLookupTableIndex = new LTIndex( EnvModel::m_pIDULayer, m_pLookupTable );
         ...
         do {
            ...
            *value = NULL;  // terminate fieldname
            ...
            mapCol = EnvModel::m_pIDULayer->GetFieldCol( fieldName ); 
            ...
            if ( *value == '@' )
               ...
               *end = NULL;
               ...
               int col = EnvModel::m_pIDULayer->GetFieldCol( fieldRef );
               if ( col < 0 )
                  {
                  CString msg;
   ...
   ParseCost( initCostExpr, m_initialCost, m_colInitialCost );
   ParseCost( maintenanceCostExpr, m_maintenanceCost, m_colMaintenanceCost );
   }


bool PolicyConstraint::ParseCost( LPCTSTR costExpr, float &cost, int &col )
   {
   if ( costExpr == NULL )
      return false;
   
   // string?  Then this should correspond to a column name in the lookup table
   if ( isalpha( costExpr[ 0 ] ) || costExpr[ 0 ] == '_' )
      {
      if ( m_pLookupTable == NULL )
         return false;

      col = m_pLookupTable->GetCol( costExpr );
      if ( col < 0 )
         {
...
bool PolicyConstraint::GetInitialCost( int idu, float area, float &cost )
   {
   bool found = GetCost( idu, m_initialCost, m_colInitialCost, cost );
...
bool PolicyConstraint::GetMaintenanceCost( int idu, float area, float &cost )
   {
   bool found = GetCost( idu, m_maintenanceCost, m_colMaintenanceCost, cost );
...
// gets the non-basis-adjusted (raw) cost from the lookup table.  This should be adjusted to the 
// basis to estimate the total actual cost of the policy for an IDU
bool PolicyConstraint::GetCost( int idu, float costToUse, int colCost, float &cost )
   {
   if ( m_pLookupTable == NULL )
      {
      cost = costToUse;
...
Policy& Policy::operator = ( Policy &policy )
   ...
   m_pSiteAttrQuery = PolicyManager::m_pQueryEngine->ParseQuery( policy.m_siteAttrStr, 0, "Policy Copy Operator" );
...
Policy::~Policy()
   ...
   if ( m_pSiteAttrQuery != NULL && PolicyManager::m_pQueryEngine != NULL )
      PolicyManager::m_pQueryEngine->RemoveQuery( m_pSiteAttrQuery );
   }
...
bool Policy::SetOutcomes( LPCSTR outcomes )
   {
...
int Policy::GetPersistence()
   ...
   if ( m_persistenceMin < m_persistenceMax )
      actualPersistence = (int) EnvModel::m_randUnif.RandValue( m_persistenceMin, m_persistenceMax );
   else
...
float Policy::GetMaxApplicationArea( int idu )
   ...
   if ( maxArea < 0 )
      EnvModel::m_pIDULayer->GetData( idu, EnvModel::m_colArea, maxArea );
...
void Policy::GetIduObjectiveScores( int idu, float* scores )
   ...
   ASSERT( objectiveCount == EnvModel::GetMetagoalCount()+1 );   // includes global score
...
float Policy::GetIduObjectiveScore( int idu, int objective )  // -1 for global, zero-based for rest
   {
   ASSERT( (m_objectiveArray.GetSize()-1) == EnvModel::GetMetagoalCount() );
   ...
   for ( int j=0; j < modArray.GetSize(); j++ )
      ...
      switch( mod.type )
         ...
         case SM_SCENARIO:    // scenario-specific modification
            {
            if ( (int) mod.value == gpModel->GetScenarioIndex()  )
               {
               totalMods += mod.value;
               passedMods++;
               }
...
Policy *PolicyManager::FindPolicy( LPCSTR name, int *index )
   {
...
bool PolicyManager::CompilePolicy( Policy *pPolicy, MapLayer *pLayer )
   ...
   if ( pPolicy->m_prefSelStr.IsEmpty() )
      ...
      if ( Policy::m_parserVars == NULL )
         Policy::m_parserVars = new double[ EnvModel::m_pIDULayer->GetFieldCount() ];
      ...
      for ( int i=0; i < varCount; i++ )
         ...
         int col = EnvModel::m_pIDULayer->GetFieldCol( varName );
         ...
         if ( outcome.m_pFunction == NULL )
            ...
            if ( col < 0 )
               ...
               msg.Format( "Unable to find column %s when setting up policy %s", 
                              outcome.field, (LPCSTR) pPolicy->m_name );
               Report::ErrorMsg( msg );
               }
            }
         else
...
///////////////////////////////////////////////////////////////////////////////
// Functions
OutcomeFunction* OutcomeFunction::MakeFunction( LPCSTR functionName, Policy *pPolicy )
   {
...
bool BufferOutcomeFunction::Build()
   ...
   m_pQuery = PolicyManager::m_pQueryEngine->ParseQuery( queryString, 0, source);
   ...
   for ( int j=0; j < m_insideBufferMultiOutcomeArray.GetSize(); j++ )
      ...
      for ( int k=0; k < m.GetOutcomeCount(); k++ )
         ...
         if ( outcome.m_pFunction == NULL )
            {
            int col = EnvModel::m_pIDULayer->GetFieldCol( outcome.field );
            outcome.col = col;
...
bool BufferOutcomeFunction::Run( DeltaArray *pDeltaArray, int cell, int currentYear, int persistence )
   {
   ASSERT( EnvModel::m_pIDULayer != NULL );
   ...
   //
   Poly *pPoly = EnvModel::m_pIDULayer->GetPolygon( cell );
   ...
   if ( bufferUnion.GetVertexCount() > 0 )
      ...
      outcome = gpModel->SelectPolicyOutcome( m_outsideBufferMultiOutcomeArray );
      if (outcome >= 0)
         {
         const MultiOutcomeInfo& oMultiOutcome = m_outsideBufferMultiOutcomeArray.GetMultiOutcome( outcome );
         gpModel->ApplyMultiOutcome( oMultiOutcome, cell, persistence );      
         }
      }
   else
      return false;
...
bool ExpandOutcomeFunction::Run( DeltaArray *pDeltaArray, int idu, int currentYear, int persistence )
   {
   ASSERT( EnvModel::m_pIDULayer != NULL );
   MapLayer *pLayer = EnvModel::m_pIDULayer;
...
//applyOutcome:
   ...
   for ( INT_PTR i=0; i < m_expansionIDUs.GetSize(); i++ ) 
      ...
      if ( expIDU >= 0 )
         {
         int outcome = gpModel->SelectPolicyOutcome( m_multiOutcomeArray );
         if (outcome >= 0)
            {
            const MultiOutcomeInfo& bMultiOutcome = m_multiOutcomeArray.GetMultiOutcome( outcome );
            gpModel->ApplyMultiOutcome( bMultiOutcome, expIDU, persistence );
            }
         }
      }
...
float ExpandOutcomeFunction::AddIDU( int idu, bool &addToPool, float areaSoFar, float maxArea )
   ...
   float area = 0;
   EnvModel::m_pIDULayer->GetData( idu, m_colArea, area );
...
bool AddPointOutcomeFunction::Build()
   ...
   Map *pMap = EnvModel::m_pIDULayer->m_pMap;
...
bool IncrementOutcomeFunction::Build()
   ...
   m_pLayer = EnvModel::m_pIDULayer; //// pMap->GetLayer( m_layerName );
...
bool IncrementOutcomeFunction::Run( DeltaArray *pDeltaArray, int cell, int currentYear, int persistence )
   ...
   if ( gpModel->AddDelta( cell, m_col, currentYear, value, DT_POLICY ) < 0 )
      return false;  
...
void Policy::GetOutcomesFriendly( CString &str )
   ...
   for ( int i=0; i < multiCount; i++ )
      ...
      for ( int j=0; j < outcomeCount; j++ )
         ...
         MAP_FIELD_INFO *pInfo = EnvModel::m_pIDULayer->FindFieldInfo( oi.field );
         if ( pInfo != NULL )
...
int PolicyManager::LoadXml( TiXmlNode *pXmlPolicies, bool appendToExisting )
   ...
   while ( pXmlConstraint != NULL )
      ...
      if ( ok )
         ...
         GlobalConstraint *pConstraint = new GlobalConstraint( name, _type, value );
      ...
      if ( ok )
         ...
         if ( ! deleted  ) // && ( (policy.m_isShared && gpDoc->m_loadSharedPolicies) || ( policy.m_originator.CompareNoCase( gpDoc->m_userName ) == 0 ) ) )
            ...
            for ( int j=0; j < metagoalCount; j++ )  
               {
               METAGOAL *pInfo = EnvModel::GetMetagoalInfo( j );
            ...
            if ( pXmlGoalScores != NULL )
               {
               Report::WarningMsg( _T("<goal_score> syntax no longer supported - ignoring...") );
               usesScores = false;
               }
            ...
            if ( usesScores )
               ...
               while ( pObjectiveNode = pXmlScores->IterateChildren( "objective", pObjectiveNode ) )
                  ...
                  if ( objName.CompareNoCase( _T("Global") ) != 0 )
                     {
                     METAGOAL *pInfo = EnvModel::FindMetagoalInfo( objName );
                     //ENV_EVAL_MODEL *pInfo = EnvModel::FindModelInfo( objName );
                     if ( pInfo == NULL )
                        {
                        if ( reportError )
                           {
                     ...
                     int metagoalIndex = EnvModel::FindMetagoalIndex( pInfo->name );
                     if ( metagoalIndex >= 0 )
...
PolicyConstraint *PolicyManager::LoadXmlConstraint( TiXmlElement *pXmlConstraint, Policy *pPolicy )
   ...
   XML_ATTR attrs[] = { // attr              type           address         isReq checkCol
                      ...
                      { "duration",         TYPE_INT,      &duration,        false, 0 },
...
int PolicyManager::SaveXml( FILE *fp, bool includeHdr, bool useFileRef )
   ...
   for ( int i=0; i < constraintCount; i++ )
      ...
      fprintf( fp, "\n<global_constraint name='%s' type='%s' value='%s' />",
            (LPCTSTR) pConstraint->m_name, type, (LPCTSTR) pConstraint->m_expression );
      }
   ...
   for ( int i=0; i < GetPolicyCount(); i++ )
      ...
      for ( int j=0; j < constraintCount; j++ )
         ...
         if ( pConstraint->GetLookupTable() == NULL )
            {
            fprintf( fp, "\n\t\t<constraint name='%s' gcName='%s' basis='%s' initCost='%s' maintenanceCost='%s' duration='%i' />",
               (LPCTSTR) pConstraint->m_name, (LPCTSTR) pConstraint->m_gcName, basis, (LPCTSTR) pConstraint->m_initialCostExpr, (LPCTSTR) pConstraint->m_maintenanceCostExpr, pConstraint->m_duration );
            }
         else
            {
            fprintf( fp, "\n\t\t<constraint name='%s' gcName='%s' basis='%s' lookup='%s' initCost='%s' maintenanceCost='%s' duration='%i' />",
               (LPCTSTR) pConstraint->m_name, (LPCTSTR) pConstraint->m_gcName, basis, (LPCTSTR) pConstraint->m_lookupStr, (LPCTSTR) pConstraint->m_initialCostExpr, (LPCTSTR) pConstraint->m_maintenanceCostExpr, pConstraint->m_duration );
            }
         }
