...
bool OutputTreeCtrl::CanDropItem(HTREEITEM hDrag, HTREEITEM hDrop, EDropHint hint)
   ...
   msg.Format( "drop target: %s, hint=%s", (LPCTSTR) pDropInfo->label, (LPCTSTR) _hint );
   ...
   if ( m_outputTreeCtrl.GetParentItem( hSelectedItem ) == m_outputTreeCtrl.GetRootItem() )
      ...
      switch( m_pOutput->m_modelDomain )
         ...
         case MOD_HRU:
            ...
            m_varNames.SetWindowText( "hruVolMelt, hruVolSwe, hruVolTopSoil, hruVolSGW, hruVolDGW, hruPrecip, hruRainfall, hruSnowfall, hruAirTemp, hruET, HRU_TO_AQ" );
            break;
...
//       case MOD_LINK:
         default:
            m_varNames.SetWindowText("reachOutflow");
