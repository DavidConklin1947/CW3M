...
#include "EnvView.h"
#include "EnvEngine\SocialNetwork.h"
...
int ResultsPanel::AddDynamicGraphNodes( ResultNode *pGraphNode )
   ...
   ResultNode *pEvalModelNode = AddTreeNode( pGraphNode, run, VC_OTHER, 0, VT_GRAPHS, _T("Eval Model Scores") );

   // add eval model raw scores
   for ( int i=0; i < EnvModel::GetResultsCount(); i++ )
      {
      ENV_EVAL_MODEL *pModel = EnvModel::GetResultsModelInfo( i );
      ASSERT( pModel->showInResults );
      AddTreeNode( pEvalModelNode, run, VC_GRAPH, i, VT_GRAPH_DYNAMIC, pModel->name );
      }
   ...
   for ( int i=0; i < apCount; i++ )
      ...
      if ( pInfo->outputVarFn != NULL )
         ...
         if ( varCount > 0 )
            ...
            ResultNode *pModelNode = AddTreeNode( pGraphNode, run, VC_MODEL_OUTPUTS, i+1, VT_GRAPH_MODEL_OUTPUTS, pInfo->name );
   ...
   int emCount = EnvModel::GetModelCount();
...
bool ResultsPanel::AddRun( int run )
   ...
   // add map visualizers
   for ( int j=0; j < EnvModel::GetVisualizerCount(); j++ )
      {
      ENV_VISUALIZER *pViz = EnvModel::GetVisualizerInfo( j );

      if ( pViz->type & VT_POSTRUN_MAP )
         AddTreeNode( pMapNode, run, VC_MAP, j, VT_MAP_VISUALIZER, pViz->name );
      }
   ...
   for ( int j=0; j < EnvModel::GetVisualizerCount(); j++ )
      {
      ENV_VISUALIZER *pViz = EnvModel::GetVisualizerInfo( j );
      if ( pViz->type & VT_POSTRUN_GRAPH )
         AddTreeNode( pGraphNode, run, VC_GRAPH, j, VT_GRAPH_VISUALIZER, pViz->name );
      }
   ...
   for ( int j=0; j < EnvModel::GetVisualizerCount(); j++ )
      {
      ENV_VISUALIZER *pViz = EnvModel::GetVisualizerInfo( j );
      if ( pViz->type & VT_POSTRUN_VIEW )
         AddTreeNode( pViewNode, run, VC_VIEW, j, VT_VIEW_VISUALIZER, pViz->name );
      }
...
void ResultsPanel::AddGraphWnd( ResultNode *pNode )
   ...
   switch( type )
      ...
      case VT_GRAPH_DYNAMIC:         
         ...
         ENV_EVAL_MODEL *pModel = EnvModel::GetResultsModelInfo( (int) extra );
         ...
         title.Format( "%s - Run %i (%s)", (LPCTSTR) pModel->name, run, (LPCTSTR) ri.pScenario->m_name );
         }
         break;
...
*/
      ...
      case VT_GRAPH_MULTI_DYNAMIC:
         {
/*         ASSERT( extra >=0 );
         ENV_EVAL_MODEL *pModel = EnvModel::GetResultsModelInfo( extra );
...
void ResultsPanel::AddCrossRunGraph( ResultNode *pNode ) //RESULT_TYPE type, int extra )
   ...
   switch( type )
      ...
      case VT_GRAPH_DYNAMIC:         
         ...
         int col  = pRunData->GetCol( pModel->name );
         ...
         title.Format( "%s - Cross-Run Comparison", (LPCTSTR) pModel->name );
         }
         break;
...
void ResultsPanel::ExportModelOutputs( int run /*=-1*/, bool promptForPath /*=false*/) 
   ...
   for ( int i=startRun; i < endRun; i++ )
      {
      DataManager *pDataManager = gpModel->m_pDataManager;
      DataObj *pDataObj = pDataManager->GetDataObj( DT_MODEL_OUTPUTS, i );
      ASSERT( pDataObj != NULL );
      ...
      CString filename;
      filename.Format( "%s%s_ModelOutputs_Run%i.csv", (LPCTSTR) path, (LPCTSTR) ri.pScenario->m_name, i + gpModel->m_startRunNumber );    

      if ( promptForPath )
         {
         CFileDialog dlg( FALSE, ".csv", filename, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT, "Envision Data Files|*.csv|All files|*.*||" );
...
void ResultsPanel::AddVisualizerWnd( ResultNode *pNode )   // extra has offset of visualizer in EnvModel visualizer array
   ...
   ASSERT( index < EnvModel::GetVisualizerCount() );

   ENV_VISUALIZER *pEnvViz = EnvModel::GetVisualizerInfo( index );
   ASSERT( pEnvViz != NULL );
   ...
   try
      {
      BOOL success;
      VisualizerWnd *pVizWnd = gpView->m_vizManager.CreateWnd( m_pResultsWnd, pEnvViz, run, success ); 

      if ( success )
         pEnvViz->initWndFn( &(pVizWnd->m_envContext), pVizWnd->m_hWnd );
      ...
      //m_pResultsWnd->AddWnd( (CWnd*) pVizWnd, RCW_VISUALIZER, &VisualizerWnd::Replay, &VisualizerWnd::SetYear );
      }
