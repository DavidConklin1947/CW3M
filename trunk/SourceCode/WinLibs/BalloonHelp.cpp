...
CBalloonHelp* CBalloonHelp::s_this = NULL;
...
void CBalloonHelp::SetTimeout(unsigned int unTimeout)
   ...
   if (NULL != m_hWnd)
      ...
      if (m_unTimeout > 0)
         {
         m_unTimerClose = (UINT)SetTimer(ID_TIMER_CLOSE, m_unTimeout, NULL);
         }
      else
         {
...
void CBalloonHelp::SetKeyboardHook()
   ...
   if (NULL == m_hKeyboardHook)
      {
      m_hKeyboardHook =
         SetWindowsHookEx(WH_KEYBOARD, KeyboardHookProc, NULL, ::GetCurrentThreadId());
      }
   }
...
void CBalloonHelp::SetCallWndRetHook()
   ...
   if (NULL == m_hCallWndRetHook)
      {
      m_hCallWndRetHook = ::SetWindowsHookEx(WH_CALLWNDPROCRET, CallWndRetProc, NULL, ::GetCurrentThreadId());
      }
   }
...
LRESULT CBalloonHelp::KeyboardHookProc(int code, WPARAM wParam, LPARAM lParam)
   ...
   if (code >= 0 && !(lParam & 0x80000000) && NULL != s_this->m_hWnd)
      {
...
LRESULT CBalloonHelp::MouseHookProc(int code, WPARAM wParam, LPARAM lParam)
   {
   if (code >= 0 && NULL != s_this->m_hWnd)
      {
      switch ((UINT)wParam)
         {
         case WM_NCMOUSEMOVE:
         case WM_MOUSEMOVE:
            if ((s_this->m_unOptions & unCLOSE_ON_MOUSE_MOVE))
               {
               CPoint pt;
               ::GetCursorPos(&pt);
               if ((abs(pt.x - s_this->m_ptMouseOrig.x) > s_this->m_nMouseMoveTolerance || abs(pt.y - s_this->m_ptMouseOrig.y) > s_this->m_nMouseMoveTolerance))
                  s_this->PostMessage(WM_CLOSE);
               }
            break;
         case WM_NCLBUTTONDOWN:
         case WM_LBUTTONDOWN:
            if ((s_this->m_unOptions & unCLOSE_ON_LBUTTON_DOWN))
               s_this->PostMessage(WM_CLOSE);
            break;
         case WM_NCMBUTTONDOWN:
         case WM_MBUTTONDOWN:
            if ((s_this->m_unOptions & unCLOSE_ON_MBUTTON_DOWN))
               s_this->PostMessage(WM_CLOSE);
            break;
         case WM_NCRBUTTONDOWN:
         case WM_RBUTTONDOWN:
            if ((s_this->m_unOptions& unCLOSE_ON_RBUTTON_DOWN))
               s_this->PostMessage(WM_CLOSE);
            break;
         case WM_NCLBUTTONUP:
         case WM_LBUTTONUP:
            if ((s_this->m_unOptions & unCLOSE_ON_LBUTTON_UP))
               s_this->PostMessage(WM_CLOSE);
            break;
         case WM_NCMBUTTONUP:
         case WM_MBUTTONUP:
            if ((s_this->m_unOptions & unCLOSE_ON_MBUTTON_UP))
               s_this->PostMessage(WM_CLOSE);
            break;
         case WM_NCRBUTTONUP:
         case WM_RBUTTONUP:
            if ((s_this->m_unOptions & unCLOSE_ON_RBUTTON_UP))
               s_this->PostMessage(WM_CLOSE);
            break;
         }
      }
...
LRESULT CBalloonHelp::CallWndRetProc(int code, WPARAM wParam, LPARAM lParam)
   ...
   if (code >= 0 && NULL != s_this->m_hWnd)
      ...
      if (WM_MOVE == pcwpr->message && pcwpr->hwnd == s_this->m_hwndAnchor)
