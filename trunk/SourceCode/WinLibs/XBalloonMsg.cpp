// XBalloonMsg.cpp  Version 1.3 - see article at CodeProject.com
//
...
//     - Added VB and DLL projects for VS2005
...
BOOL	CXBalloonMsg::m_bSubclassParent = TRUE;
...
//=============================================================================
{
	if (m_wndProc && IsWindow(m_hParent))
		::SetWindowLongPtr(m_hParent, GWLP_WNDPROC, (LONG_PTR)m_wndProc);
	m_wndProc = 0;
}
						...
						BOOL bSubclassParent /*= TRUE*/)
{
	...
	m_nTimerId = 0;
	m_bSubclassParent = bSubclassParent;
...
void CXBalloonMsg::Destroy()
{
	...
	if (m_wndProc && IsWindow(m_hParent))
		::SetWindowLongPtr(m_hParent, GWLP_WNDPROC, (LONG)m_wndProc);
	m_wndProc = 0;
											   ...
											   LPARAM lParam)
{
		...
		case WM_WINDOWPOSCHANGED:
		{
			if (!m_bInit && (m_hWndBalloon == 0))
			{
				...
				if (m_hWndBalloon)
				{
					...
					{
						if (m_bSubclassParent)
							m_wndProc = (WNDPROC)::SetWindowLongPtr(m_hParent, GWLP_WNDPROC, (LONG)DefWindowProcX);
					...
					if (IsWindow(m_hWndBalloon))
					{
						...
						{
							m_nTimerId = ::SetTimer(0, (UINT_PTR) 1, m_nTimeOutSeconds*1000, TimerProcX);
						}
	...
	if (m_bSubclassParent && (hWnd == m_hParent))
	{
		if (m_wndProc && IsWindow(m_hParent))
			return (*m_wndProc)(hWnd, message, wParam, lParam);
		else
...
// PositionBalloon()
//
// Purpose:     The PositionBalloon() function returns the rect for the 
//              message balloon.
//
