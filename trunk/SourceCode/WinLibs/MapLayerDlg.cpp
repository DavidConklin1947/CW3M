...
void MapLayerDlg::DoDataExchange(CDataExchange* pDX)
   ...
   DDX_Control(pDX, IDC_OUTLINECOLOR, m_outlineColorBtn);
   DDX_Control(pDX, IDC_LWIDTH, m_outlineWidth );
   ...
   DDX_Text(pDX, IDC_LFILE, m_file);
   DDX_Text(pDX, IDC_LIB_LAYER, m_layerName);
   DDX_Text(pDX, IDC_LTYPE, m_type);
   DDX_Check(pDX, IDC_VISIBLE, m_visible);
   DDX_Check(pDX, IDC_SHOWOUTLINES, m_showOutlines);
   ...
   DDX_Text(pDX, IDC_EXTENTS2, m_actualExtents);
   DDX_Control(pDX, IDC_TRANSPARENCY, m_transparencySlider);
   DDX_Control(pDX, IDC_TRANSPARENCY_VALUE, m_percentTransparency);
   }


BEGIN_MESSAGE_MAP(MapLayerDlg, CDialog)
	ON_BN_CLICKED(IDC_SHOWOUTLINES, OnShowoutlines)
   ON_BN_CLICKED(IDC_LEDITFIELDINFO, &MapLayerDlg::OnBnClickedEditfieldinfo)
   ON_BN_CLICKED(IDC_EDITLABELINFO, &MapLayerDlg::OnBnClickedEditlabelinfo)
   ...
   if ( id == IDC_TRANSPARENCY )
      ...
      switch( nSBCode )
         ...
         case SB_THUMBTRACK:
            ...
            GetDlgItem( IDC_TRANSPARENCY_VALUE )->SetWindowText( buffer ); 
