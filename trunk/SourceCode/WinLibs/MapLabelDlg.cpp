...
DDX_Control(pDX, IDC_FONTSAMPLE, m_sample);
DDX_Control(pDX, IDC_LLAYERS, m_layers);
DDX_Control(pDX, IDC_LIB_FIELDS, m_fields);
DDX_Check(pDX, IDC_SHOWLABELS, m_showLabels);
   }


BEGIN_MESSAGE_MAP(MapLabelDlg, CDialog)
   ON_BN_CLICKED(IDC_SETFONT, &MapLabelDlg::OnBnClickedSetfont)
   ON_BN_CLICKED(IDC_SHOWLABELS, &MapLabelDlg::OnBnClickedShowlabels)
   ON_BN_CLICKED(IDOK, &MapLabelDlg::OnBnClickedOk)
   ON_CBN_EDITCHANGE(IDC_LLAYERS, &MapLabelDlg::OnCbnEditchangeLayers)
END_MESSAGE_MAP()
