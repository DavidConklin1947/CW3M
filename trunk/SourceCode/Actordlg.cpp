...
extern MapLayer *gpCellLayer;  // used only to make sure it exits
...
BOOL ActorDlg::OnInitDialog() 
   ...
   m_actorInitMethod = (int) gpActorManager->m_actorInitMethod;

   int groups = gpActorManager->GetActorGroupCount();
...
void ActorDlg::SaveActorsToDoc()
   ...
   for ( int i=0; i < m_actorGroupArray.GetCount(); i++ )
      ...
      gpActorManager->AddActorGroup( new ActorGroup( *pActorGroup ) );
      }

   gpActorManager->m_actorInitMethod = (ACTOR_INIT_METHOD) m_actorInitMethod;
   gpActorManager->CreateActors();
...
void ActorDlg::OnBnClickedExport()
   ...
   CFileDialog dlg( FALSE, "xml", gpActorManager->m_path, OFN_HIDEREADONLY | OFN_OVERWRITEPROMPT,
   ...
   if ( retVal == IDOK )
      {
      SaveActorsToDoc();
      gpActorManager->SaveXml( dlg.GetPathName() );
      }
   }
