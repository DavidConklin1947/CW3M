...
{
public:
   MParser() : MTParser() { this->defineFunc(new TimeFct()); this->defineFunc(new ExpFct()); } 
};
...
public:
   ...
   Variable() : m_value( 0 ), m_cumValue( 0 ), m_queryArea( 0 ), m_isOutput( false ), m_col( -1 ), 
         m_varType( VT_UNDEFINED ), m_dataType( TYPE_DOUBLE ), m_pParser( NULL ), m_pQuery( NULL ), m_extent( EXTENT_IDU ),
...
class EvalModel : public ModelBase, public EnvEvalModel
{
public:
...
public:
   EvalModel( ModelCollection *pCollection ) : ModelBase( pCollection ), EnvEvalModel(), m_evalType( ET_UNDEFINED ), 
...
class AutoProcess : public ModelBase, public EnvAutoProcess
{
public:
   AutoProcess( ModelCollection *pCollection ) : ModelBase( pCollection ), EnvAutoProcess() { }
