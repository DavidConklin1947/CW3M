...
SpatialAllocator  *theProcess = NULL;

static AFX_EXTENSION_MODULE SpatialAllocatorDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("SpatialAllocator.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(SpatialAllocatorDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(SpatialAllocatorDLL);
     
      ASSERT( theProcess == NULL );
      theProcess = new SpatialAllocator;
      ASSERT( theProcess != NULL );
	}
    
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("SpatialAllocator.DLL Terminating!\n");

      /**** Delete any instantiated models ****/
      if ( theProcess != NULL)
         delete theProcess;

		// Terminate the library before destructors are called
      /**** TODO: Update module name in AfxTermExtensionModule ****/
		AfxTermExtensionModule(SpatialAllocatorDLL);
	}
	return 1;   // ok
}
