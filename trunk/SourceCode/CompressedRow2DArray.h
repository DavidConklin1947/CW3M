...
//*******************************************************************
//             ORIGINAL CODE BEFORE long long indexes
//*******************************************************************
#pragma once
#include<iostream>
#include<cstdio>
...
private:
  ...
  vector<int> colNdx; // index of column in the uncompreseed array
  vector<int> rowStartNdx; // index of the first element of uncompressed row[i]
  vector<int> rowEndNdx; // index of the last element of uncompressed row[i]
...
public:
  ...
  long long GetRowNdxs(const int &row, long long *ndxs);
  int GetRowValues(const int &row, T *values);
...
CompressedRow2DArray<T>::CompressedRow2DArray(FILE *iFile) {
  ...
  VERIFY(fread((void *)&numRows, 1, sizeof(int), iFile) == sizeof(int));

  rowStartNdx.resize(numRows,-1);

  rowEndNdx.resize(numRows,-1);

  VERIFY(fread((void *)&numCols, 1, sizeof(int), iFile) == sizeof(int));

  VERIFY(fread((void *)&allocSz, 1, sizeof(int), iFile) == sizeof(int));

  VERIFY(fread((void *)&crntAddRow, 1, sizeof(int), iFile) == sizeof(int));

  VERIFY(fread((void *)&crntAddCol, 1, sizeof(int), iFile) == sizeof(int));

  // jpb = changed from size_t to UINT
  UINT numElements;
  VERIFY(fread((void *)&numElements, 1, sizeof(UINT), iFile) == sizeof(UINT));
  ...
  for(unsigned i=0;i<colNdx.size();i++) {
    VERIFY(fread((void *)&colNdx[i], 1, sizeof(int), iFile) == sizeof(int));
  }

  for(unsigned i=0;i<rowStartNdx.size();i++) {
    VERIFY(fread((void *)&rowStartNdx[i], 1, sizeof(int), iFile) == sizeof(int));
  }

  for(unsigned i=0;i<rowEndNdx.size();i++) {
    VERIFY(fread((void *)&rowEndNdx[i], 1, sizeof(int), iFile) == sizeof(int));
  }

  VERIFY(fread((void *)&defaultValue, 1, sizeof(T), iFile) == sizeof(T));

  VERIFY(fread((void *)&nullValue, 1, sizeof(T), iFile) == sizeof(T));

  for(unsigned i=0;i<values.size();i++) {
    VERIFY(fread((void *)&values[i], 1, sizeof(T), iFile) == sizeof(T));
  }
...
} // unsigned CompressedRow2DArray::GetRowSum(int elRow)
...
CompressedRow2DArray<T> *CompressedRow2DArray<T>::Duplicate(void) {
	...
	for(int i=0; i<=crntAddRow; i++) {
		for(int j=0; j<=crntAddCol; j++) {
...
CompressedRow2DArray<T> *CompressedRow2DArray<T>::Invert(void) {
	...
	for(int i=0; i<numCols; i++) { // all the cols
		for(int j=0; j<=crntAddRow; j++) { // all the rows so far
...
CompressedRow2DArray<T> *CompressedRow2DArray<T>::Transpose(void) {
	...
	int maxColNdx = 0;
	for(int ndx = 0; ndx < oldArray->colNdx.size(); ndx++) {
	...
	int lastNdx = 0;
	for(int ndx = 0; ndx < newArray->numRows; ndx ++) {
	...
	int
		stopColNdx,
		maxColArrNdx = (int)oldArray->colNdx.size() - 1;
		...
		oldRowNdx++) 
	{
		...
		if(nextRowNdx >= oldArray->numRows)
			stopColNdx = (int)oldArray->colNdx.size();
		
		// drop the elements from the row of the oldArray
		// into the proper place of the newArray
		for(int oldArrayNdx = oldArray->rowStartNdx[oldRowNdx];
	...
	} // for(int oldRowNdx = 0; oldRowNdx < oldArray->numRows; oldRowNdx++)

	// set the crntAddRow and crntAddCol

	newArray->crntAddRow = (int)newArray->rowStartNdx.size();
...
int CompressedRow2DArray<T>::GetRowStartNdx(int elRow) {
	...
	return rtrnVal;
} // CompressedRow2DArray::GetRowStartNdx(int elRow) {
...
int CompressedRow2DArray<T>::GetRowEndNdx(int elRow) {
	...
	return rtrnVal;
} // CompressedRow2DArray::GetRowEndNdx(int elRow) {
...
/*
template<class T>
int CompressedRow2DArray<T>::GetRowNdxs(const int &arrRow, long long *ndxs) {
	// Get the col ndxs for a row
...
template<class T>
int CompressedRow2DArray<T>::GetRowValues(const int &arrRow, T *values) {
	int
		startRowNdx,
		endRowNdx,
		colCnt = 0;
	...
	return colCnt;
} // int CompressedRow2DArray<T>::GetRowValues(const int &row, T *values) 
...
void CompressedRow2DArray<T>::DisplayArray(const char *fname) {
	...
	if (fopen_s(&outFile, fname, "w")!=0) ASSERT(0);
...
void CompressedRow2DArray<T>::WriteToFile(FILE *oFile) {
	...
	VERIFY(fwrite((void *)&numRows, 1, sizeof(int), oFile) == sizeof(int));

	VERIFY(fwrite((void *)&numCols, 1, sizeof(int), oFile) == sizeof(int));

	VERIFY(fwrite((void *)&allocSz, 1, sizeof(int), oFile) == sizeof(int));

	VERIFY(fwrite((void *)&crntAddRow, 1, sizeof(int), oFile) == sizeof(int));

	VERIFY(fwrite((void *)&crntAddCol, 1, sizeof(int), oFile) == sizeof(int));

	// size_t --> UINT
	UINT numElements = static_cast<UINT>(colNdx.size());
	VERIFY(fwrite((void *)&numElements, 1, sizeof(UINT), oFile) == sizeof(UINT));
  
	// Writing out the vector class contents for row and col ndx's
	for(unsigned i=0;i<colNdx.size();i++) {
		VERIFY(fwrite((void *)&colNdx[i], 1, sizeof(int), oFile) == sizeof(int));
	}

	for(unsigned i=0;i<rowStartNdx.size();i++) {
		VERIFY(fwrite((void *)&rowStartNdx[i], 1, sizeof(int), oFile) == sizeof(int));
	}

	for(unsigned i=0;i<rowEndNdx.size();i++) {
		VERIFY(fwrite((void *)&rowEndNdx[i], 1, sizeof(int), oFile) == sizeof(int));
	}

	// Writing out values associated with data

	VERIFY(fwrite((void *)&defaultValue, 1, sizeof(T), oFile) == sizeof(T));

	VERIFY(fwrite((void *)&nullValue, 1, sizeof(T), oFile) == sizeof(T));

	for(unsigned i=0;i<values.size();i++) {
		VERIFY(fwrite((void *)&values[i], 1, sizeof(T), oFile) == sizeof(T));
	}
...
void CompressedRow2DArray<T>::WriteToFile(const char *fName) {
	...
	VERIFY((oFile = fopen(fName,"wb")) != NULL);
