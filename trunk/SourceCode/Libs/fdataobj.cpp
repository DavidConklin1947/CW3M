...
//-------------------------------------------------------------------
FDataObj::FDataObj( int _cols, int _rows )
   : DataObj( _cols, _rows ),
     matrix  ( _rows, _cols ),     
     statArray()
   {
...
FDataObj::FDataObj( int _cols, int _rows, float initialValue )
   : DataObj( _cols, _rows ),
...
FDataObj::FDataObj( const FloatMatrix::type_data * data, int _cols, int _rows )
: DataObj( _cols, _rows ),
...
FDataObj::FDataObj( int _cols, int _rows, STATSFLAG *statFlagArray )
   : DataObj( _cols, _rows ),
     matrix  ( _rows, _cols ),
     statArray()
   {
...
float FDataObj::Get( int col, int row )
   {
   if ( col < 0 || col >= this->GetColCount() )
      {
      CString msg;
      msg.Format( "Invalid column index (%i) passed to FDataObj '%s' (cols=%i,rows=%i) when getting a value!",
   ...
   if ( row < 0 || row >= this->GetRowCount() )
      {
      CString msg;
      msg.Format( "Invalid row index (%i) passed to FDataObj '%s' (cols=%i,rows=%i) when getting a value!",
         row, this->m_name, this->GetColCount(), this->GetRowCount() );
      Report::ErrorMsg( msg );
      ASSERT(0);
      return 0;
      }
...
float FDataObj::IGet( float x, int xcol, int ycol, IMETHOD method, int *startRow/*=NULL*/, int *leftRowIndex /*=NULL*/, int *rightRowIndex /*=NULL*/)
   {
   float x1;
   ...
   float x0 = Get( xcol, row-1 );
   float y0 = Get( ycol, row-1 );
   float y1 = Get( ycol, row   );
   float y=0;
...
float FDataObj::IGet( float x, float y, IMETHOD method )
   {
   float xget;
   float yget;
   ...
   if ( row == 1 )
      { // x occurred at or before the first row; return first element (doesn't wrap, doesn't extrapolate)
	  x0row = x1row = 1;
      }
   else if ((row == rows) || ((xget == x) && (row == rows - 1))) 
      { //-- x occurred at or after last row (row == rows - 1 and xget = x, or never found row); return last element (doesn't wrap or extrapolate)
	  x0row = x1row = rows-1;
      }
  else if ( xget == x )
      { // Found row is an exact match.
	  x0row = x1row = row;
      }
  else
     { // Interpolate.
   ...
   if ( col == 1 )
      { // y occurred at or before the first col; return first element (doesn't wrap, doesn't extrapolate)
      y0col = y1col = 1;
      }
   else if ((col == cols) || ((yget == y) && (col == cols - 1)))
      { // y occurred at or after last col (col == cols - 1 and yget = y, or never found col); return last element (doesn't wrap or extrapolate)
      y0col = y1col = cols-1;
      }
   //-- was the found col an exact match?
   else if ( yget == y )
      { // The found col is an exact match.
	   y0col = y1col = col;
      }
   else
      { // Interpolate.
   ...
   // Obtain interpolated y values for both the x0row and x1row
	  float y0 = Get(y0col,0);
	  float y1 = Get(y1col, 0);
	  
	  float y0_x0row = Get(y0col,x0row);
	  float y1_x0row = Get(y1col,x0row);
	  float yval_x0row = (y0col == y1col) ? y0_x0row : (((y-y0)/(y1-y0))*(y0_x0row - y1_x0row)+y1_x0row);

	  float y0_x1row = Get(y0col,x1row);
	  float y1_x1row = Get(y1col,x1row);
	  float yval_x1row = (y0col == y1col) ? y0_x1row : (((y-y0)/(y1-y0))*(y0_x1row - y1_x1row)+y1_x1row);

   //Proceed with standard interpolation using new y values (yval_x0row and yval_x1row)
   //-- not an exact match, interpolate between found and previous rows
   float x0 = Get( 0, x0row );
   float x1 = Get( 0, x1row );
   float val;
   ...
   switch( method )
      ...
      case IM_LINEAR:
      default:
         if (x0row == x1row) val = yval_x0row;
         else val = ((x-x0)/(x1-x0))*(yval_x1row - yval_x0row)+yval_x0row;
      }
   ...
   // method returns the first row encountered during the search (the lesser row in the case of the forward search and the greater row in the case of the reverse search)
	...
	//float FDataObj::IGet(float x, int xcol, int ycol/*= 1*/, IMETHOD method/*=IM_LINEAR*/, int startRow/*=0*/, int *returnRow/*=NULL*/, bool forwardSearch/*=true*/)
 //  {
	//   double x1;
	...
	//   {
	//	   //-- find appropriate row
	//	   for (row = startRow; row < this->GetRowCount(); row++)
	//	   {
	//		   x1 = this->Get(xcol, row);
	...
	//	   {
	//		   *returnRow = this->GetRowCount() - 1;
	//		   return this->Get(ycol, this->GetRowCount() - 1);
	//	   }
	...
	//   if (forwardSearch)
	//   {
	//	   double x0 = this->Get(xcol, row - 1);
	//	   double y0 = this->Get(ycol, row - 1);
	//	   double y1 = this->Get(ycol, row);
	...
	//	   double val1 = fabs(x0 - x);
	//	   double val2 = fabs(x - x1);
	//	   if (val1 < val2)
	//		   *returnRow = row - 1;
	//	   else
	//		   *returnRow = row;
	...
	//	   double val1 = fabs(x1 - x);
	//	   double val2 = fabs(x - x0);
...
float FDataObj::IGetInternal( float x, int col, IMETHOD method, int &offset, bool increment )
   {
   float x1;
   int rows = GetRowCount();
   int row = 0;
...
int FDataObj::Find( int col, VData &value, int startRecord /*=-1*/ )
   {
   if (startRecord < 0 )
      startRecord = 0;
...
bool FDataObj::SetSize( int _cols, int _rows )
   {
...
int FDataObj::AppendRow( COleVariant *varArray, int length )
   {
...
int FDataObj::AppendRow( VData *varArray, int length )
   {
   int cols = (int) GetColCount();
   ASSERT( cols == length );
...
int FDataObj::AppendRow( float *fArray, int length )
   {
   int cols = GetColCount();
   ASSERT( cols == length );
   ...
   if ( m_dataCols != cols )
      {
      _fArray = new float[ cols ];
      memcpy( _fArray, fArray, m_dataCols * sizeof( float ) );
      }
   else
      _fArray = fArray;
...
int FDataObj::SetRow( float *fArray, int setRow, int countCols )
   {
   float *_fArray;
   int cols = GetColCount();
   int rows = GetRowCount();
...
int FDataObj::AppendCols( int count )
   {
   float *dataArray =  new float[ GetRowCount() ];
...
int FDataObj::AppendCol( LPCSTR label ) //, float *dataArray /*=NULL*/ )
   {
   float *dataArray = new float[ GetRowCount() ];
...
int FDataObj::InsertCol( int insertBefore, LPCSTR label )
   {
   float *dataArray = new float[ GetRowCount() ];
...
bool FDataObj::CopyCol( int toCol, int fromCol )
   {
   ...
   for ( int i=0; i < this->GetRowCount(); i++ )
      this->Set( toCol, i, this->GetAsFloat( fromCol, i ) );
...
int FDataObj::ReadAscii( LPCSTR _fileName, TCHAR delimeter, BOOL showMsg )
   {
   CString fileName;
...
int FDataObj::ReadAscii( HANDLE hFile, TCHAR delimiter, BOOL showMsg )
   {
...
int FDataObj::ReadXls(LPCSTR fileName)
   {
...
int FDataObj::WriteAscii( LPCSTR fileName, TCHAR delimiter, int colWidth )
   {
   int *colWidthArray;  // store column widths if colWidth == 0
   ...
   if ( fp == NULL )
      {
      CString msg( "FDataObj: Couldn't open file " );
      msg += fileName;
      msg.Format("%s number %i", (LPCTSTR)msg, (int)err);
      Report::WarningMsg( msg );
      CString newFileName; newFileName.Format("%sx", fileName);
      fopen_s(&fp, newFileName, "wt");     // open for writing
      if (fp == NULL)
         {
         msg.Format("Couldn't open %s either.", newFileName);
         Report::WarningMsg(msg);
      return 0;
      }

      msg.Format("Opened %s instead.\nRemember to change the file name back to %s!", newFileName.GetString(), fileName);
      Report::WarningMsg(msg);
      fileName = newFileName.GetString();
      }
   ...
   for ( row=0; row < rows; row++ )
      ...
      for ( col=0; col < cols-1; col++ )
         ...
         fprintf( fp, "%f%c ", Get( col, row ), delimiter );
...
bool FDataObj::GetMinMax( int col, float *minimum, float *maximum, int startRow )
   {
...
//-------------------------------------------------------------------
float FDataObj::GetMean( int col, int startRow )
   {
   int rows = GetRowCount();
...
//-------------------------------------------------------------------
double FDataObj::GetStdDev( int col, int startRow )
   {
   int rows = GetRowCount();
