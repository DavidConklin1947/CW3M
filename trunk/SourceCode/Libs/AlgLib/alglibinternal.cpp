...
double ftbasegetflopestimate(ae_int_t n, ae_state *_state)
{
    ...
    result = ftbase_ftbaseinefficiencyfactor*(4*n*ae_log((double)n, _state)/ae_log(2, _state)-6*n+8);
    return result;
}
     ...
     ae_state *_state)
{
    ...
    if( plan->plan.ptr.p_int[entryoffset+3]==ftbase_fftbluesteinplan )
    {
        ...
        for(i=0; i<=n-1; i++)
        {
            bx = ae_cos(ae_pi*ae_sqr((double)i, _state)/n, _state);
            by = ae_sin(ae_pi*ae_sqr((double)i, _state)/n, _state);
