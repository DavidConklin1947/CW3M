...
//
//		From this template, we intstantiate two classes:  CStringA and
//		CStringW.  The name "CString" is just a #define of one of these,
...
//					has been causing me (and users of CString) problems for
...
//					to make the Intellisense work on with CString on VC6,
...
//                  to help CString build on SGI and for pointing out an
...
//                  'nCount' argument to Left() and Right() where CString was
...
//					CString is now used in my favorite commercial MP3 player!
//
...
//				  - Added a specialization of std::swap for CString.  Thanks
...
//					ability to build CString on Sun Unix systems.  He
//					sent me detailed build reports about what works and what
//					does not.  If CString compiles on your Unix box, you
...
//
//    2000-FEB-01 - You can now use CString on the Mac with CodeWarrior!
...
//					with my implementation of CString::FormatV in which
...
//					bug that MS has not fixed.  CString did nothing to fix
...
//					The resulting string would be empty.  Not with CString!
//
...
//
//	  1999-JUL-01 - Added a true CString facade.  Now you can use CString as
...
//					to a CArchive and later read it in as a CString.   I have
//					no idea why you would want to do this but you can. 
//
//	  1999-JUN-21 - Changed the CString class into the CStdStr template.
...
//				  - new sscpy() replacement of CString::CopyString()
...
//					persisting CStrings to/from DCOM IStream interfaces 
//				  - Added functional objects (e.g. StdStringLessNoCase) that
//					allow CStrings to be used as keys STL map objects with
...
//for Envision
//#include "no_mfc_files/NoMfc_types.h"
...
//      people encounter when switching from CString to CString.
//
...
//
//      However if you were to try this with CString, your program would
//      crash.
//
//          CString sName("Joe");
//          CString sTmp;
...
//      arguments).  The type const char* is a POD type.  The type CString
...
//      control over this in CString since I derive from whatever
...
//      etc.  If you pass a CString object to one of those functions, your
...
//      const CT*) that CString (and MFC's CString) provide.  That was the
...
// arguments than the standard dictates.  Since I'm trying to write CString
...
// =============================================================================
// INLINE FUNCTIONS ON WHICH CString RELIES
//
...
public:
       ...
       //envision additions - functions missed by original author
...
//		}
	...
	//		void _cdecl Formst(CStringA& PCSTR szFormat, ...)
	//		void _cdecl Format(PCSTR szFormat);
	//           
	// DESCRIPTION:
	//		This function does sprintf/wsprintf style formatting on CStringA
    ...
    //      and allow people to pass CString arguments as fillers for
   ...
   void Append( const CT* str ) { this->append( str ); }
	...
	// that CStringW gets instantiated even in non-Unicode builds.
...
//	void			UnlockBuffer(); { }	// why have UnlockBuffer w/o LockBuffer?
	...
	// FUNCTION: StreamSize
	// REMARKS:
	//		Returns how many bytes it will take to StreamSave() this CString
	...
	// FUNCTION: StreamSave
	// REMARKS:
	//		Saves this CString object to a COM IStream.
	// -------------------------------------------------------------------------
...
// -----------------------------------------------------------------------------
// MSVC USERS: HOW TO EXPORT CString FROM A DLL
//
// If you are using MS Visual C++ and you want to export CStringA and
// CStringW from a DLL, then all you need to
//
...
//
// Really, it is not necessary to export CString functions from a DLL.  I
// never do.  In my projects, I do generally link to the DLL version of the
// Standard C++ Library, but I do NOT attempt to export CString functions.
...
typedef CStdStr<OLECHAR>	CStringO;	// almost always CStringW
...
{
	CStringA sRet(SSREF(s1));
	sRet.append(s2);
	return sRet;
}
inline CStringA operator+(const CStringA& s1, CStringA::value_type t)
{
	CStringA sRet(SSREF(s1));
	sRet.append(1, t);
	return sRet;
}
inline CStringA operator+(const CStringA& s1, PCSTR pA)
{
	CStringA sRet(SSREF(s1));
	sRet.append(pA);
	return sRet;
}
inline CStringA operator+(PCSTR pA, const CStringA& sA)
{
	CStringA sRet;
	CStringA::size_type nObjSize = sA.size();
	CStringA::size_type nLitSize = 
		static_cast<CStringA::size_type>(sslen(pA));
...
inline CStringA operator+(const CStringA& s1, const CStringW& s2)
{
	return s1 + CStringA(s2);
}
inline CStringW operator+(const CStringW& s1, const CStringW& s2)
{
	CStringW sRet(SSREF(s1));
	sRet.append(s2);
	return sRet;
}
inline CStringA operator+(const CStringA& s1, PCWSTR pW)
{
	return s1 + CStringA(pW);
}

#ifdef UNICODE
	inline CStringW operator+(PCWSTR pW, const CStringA& sA)
	{
		return CStringW(pW) + CStringW(SSREF(sA));
	}
	inline CStringW operator+(PCSTR pA, const CStringW& sW)
	{
		return CStringW(pA) + sW;
	}
#else
	inline CStringA operator+(PCWSTR pW, const CStringA& sA)
	{
		return CStringA(pW) + sA;
	}
	inline CStringA operator+(PCSTR pA, const CStringW& sW)
	{
		return pA + CStringA(sW);
	}
#endif

// ...Now the wide string versions.
inline CStringW operator+(const CStringW& s1, CStringW::value_type t)
{
	CStringW sRet(SSREF(s1));
	sRet.append(1, t);
	return sRet;
}
inline CStringW operator+(const CStringW& s1, PCWSTR pW)
{
	CStringW sRet(SSREF(s1));
	sRet.append(pW);
	return sRet;
}
inline CStringW operator+(PCWSTR pW, const CStringW& sW)
{
	CStringW sRet;
	CStringW::size_type nObjSize = sW.size();
	CStringA::size_type nLitSize = 
		static_cast<CStringW::size_type>(sslen(pW));
...
inline CStringW operator+(const CStringW& s1, const CStringA& s2)
{
	return s1 + CStringW(s2);
}
inline CStringW operator+(const CStringW& s1, PCSTR pA)
{
	return s1 + CStringW(pA);
}
...
template<>
struct FmtArg<CStringA>
{
    explicit FmtArg(const CStringA& arg) : a_(arg) {}
    PCSTR operator()() const { return a_.c_str(); }
    const CStringA& a_;
private:
    FmtArg<CStringA>& operator=(const FmtArg<CStringA>&) { return *this; }
};
template<>
struct FmtArg<CStringW>
{
    explicit FmtArg(const CStringW& arg) : a_(arg) {}
    PCWSTR operator()() const { return a_.c_str(); }
    const CStringW& a_;
private:
    FmtArg<CStringW>& operator=(const FmtArg<CStringW>&) { return *this; }
};
...
// Special operators that allow us to serialize CStrings to CArchives.
// Note that we use an intermediate CString object in order to ensure that
// we use the exact same format.

#ifdef _MFC_VER
	inline CArchive& AFXAPI operator<<(CArchive& ar, const CStringA& strA)
	{
		CString strTemp(strA);
		return ar << strTemp;
	}
	inline CArchive& AFXAPI operator<<(CArchive& ar, const CStringW& strW)
	{
		CString strTemp(strW);
		return ar << strTemp;
	}

	inline CArchive& AFXAPI operator>>(CArchive& ar, CStringA& strA)
	{
		CString strTemp;
		ar >> strTemp;
		strA = strTemp;
	...
	inline CArchive& AFXAPI operator>>(CArchive& ar, CStringW& strW)
	{
		CString strTemp;
		ar >> strTemp;
		strW = strTemp;
...
//		CStringA WUFormat(UINT nId, ...);
//		CStringA WUFormat(PCSTR szFormat, ...);
//
// REMARKS:
//		This function allows the caller for format and return a CStringA
//		object with a single line of code.
// -----------------------------------------------------------------------------

inline CStringA WUFormatA(PCSTR szFormat, ...)
{
	va_list argList;
	va_start(argList, szFormat);
	CStringA strOut;
...
inline CStringW WUFormatW(PCWSTR szwFormat, ...)
{
	va_list argList;
...
#ifdef SS_ANSI
#else
	inline CStringA WUFormatA(UINT nId, ...)
	{
		va_list argList;
		va_start(argList, nId);
	...
	inline CStringW WUFormatW(UINT nId, ...)
	{
		va_list argList;
		va_start(argList, nId);
	...
	//	 CStringA WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID);
	//	 CStringW WUSysMessageW(DWORD dwError, DWORD dwLangId=SS_DEFLANGID);
	//           
	// DESCRIPTION:
	...
	//	 corresponding system string is returned in the form of a CStringA.
	//
	// PARAMETERS: 
	...
	//
	// RETURN VALUE: 
	//	 a CStringA equivalent of the error code.  Currently, this function
	...
	inline CStringA WUSysMessageA(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
	{
		CHAR szBuf[512];
	...
	inline CStringW WUSysMessageW(DWORD dwError, DWORD dwLangId=SS_DEFLANGID)
	{
		WCHAR szBuf[512];
...
//		CString objects.  This is useful for maps in which the key may be the
...
struct StdStringLessNoCaseW
	: std::binary_function<CStringW, CStringW, bool>
{
	inline
	bool operator()(const CStringW& sLeft, const CStringW& sRight) const
...
struct StdStringLessNoCaseA
	: std::binary_function<CStringA, CStringA, bool>
{
	inline
	bool operator()(const CStringA& sLeft, const CStringA& sRight) const
	{ return ssicmp(sLeft.c_str(), sRight.c_str()) < 0; }
};
...
//namespace std
//{
//	inline void swap(CStringA& s1, CStringA& s2) throw()
//	{
//		s1.swap(s2);
//	}
//	template<>
//	inline void swap(CStringW& s1, CStringW& s2) throw()
//	{
//		s1.swap(s2);
//	}
//}
...
//use envision's CArray override instead of the crappy stdsring.h implementation

//class CStringArray : public std::vector< CString >
//   {
//   public:
//      void Insert( CString &str ) { this->push_back( CString( str ) ); }
//   
//   };
...
typedef CArray<CString,CString> CStringArray;
 ...
 typedef CStringArray CStringArray;
