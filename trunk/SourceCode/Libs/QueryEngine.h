...
class QArgument : public VData
   ...
   public:
      ...
      QArgument( LPCSTR i ) : VData( i ), m_pNode( 0 ), m_extra(0) { }
...
class   LIBSAPI QueryEngine
   ...
   public:
      ...
      int AddError( int lineNo, LPCSTR msg );

      // string management
      LPCSTR FindString( LPCSTR label, bool add );
   ...
   public:
      ...
      //Query *ParseQuery( LPCSTR buffer );  // parse a single query, and add it to the engine
      Query *ParseQuery( LPCSTR queryStr, int lineNo, LPCTSTR source );  // parse a single query, and add it to the engine
   };
