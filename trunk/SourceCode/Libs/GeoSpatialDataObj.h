...
public:
	 ...
	 // bool  LoadPolyGridLookup(LPCTSTR ); 
	  ...
	  float Get(double xcoord, double ycoord, int &twoDindexvalue, int set3Dvalue, CString prjFile, bool isUTM, bool * pOKflag);
      ...
      virtual bool Set( int col, int row, LPCSTR value       );
      ...
      virtual int  AppendCol( LPCSTR name ){return 0;}
      virtual bool CopyCol( int toCol, int fromCol ){return false;}
      virtual int  InsertCol( int insertBefore, LPCSTR label ){return 0;}
      ...
      //-- File I/O --//
	  virtual int ReadAscii ( LPCSTR fileName, char delimiter=',', BOOL showMsg=TRUE ){return 0;}
      virtual int ReadAscii ( FILE *fp,        char delimiter=',', BOOL showMsg=TRUE ){return 0;}
	  virtual int WriteAscii( LPCSTR fileName, char delimiter=',', int colWidth=0    ){return 0;}
