...
MapExprEngine::MapExprEngine( MapLayer *pLayer ) 
: m_pLayer( pLayer )
, m_pQueryEngine( NULL )
, m_currentTime( 0 )
, m_currentRecord( -1 )
...
bool MapExprEngine::Compile( MapExpr *pExpr, LPCTSTR source /*=NULL*/ )
   ...
   // process query
   if ( ! pExpr->m_queryStr.IsEmpty() )
      {
      if ( m_pQueryEngine == NULL )
         m_pQueryEngine = new QueryEngine( m_pLayer );
         ...
         for ( int i=0; i < varCount; i++ )
            ...
            if ( col < 0 )  // not recognized as a column name
               ...
               if ( pMapExpr != NULL )
                  ...
                  if ( pMapVar != NULL )
                     {
                     CString var_with_parentheses = "(" + var + ")";
                     MapExpr *pMapExprTgt = FindMapExpr(var_with_parentheses);
...
MapExpr *MapExprEngine::FindMapExpr( LPCTSTR expression_string )
   {
   for ( int i=0; i < (int) m_exprArray.GetSize(); i++ )
      {
      if ( m_exprArray[ i ]->m_expression.CompareNoCase(expression_string) == 0 )
