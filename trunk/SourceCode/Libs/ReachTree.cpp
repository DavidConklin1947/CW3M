...
int ReachNode::ComputeSubnodeLength( float maxDeltaX  )
   ...
   // Note, as written, this algorithm returns n+1 nodes, each (n/(n+1))*maxDeltaX long, for reaches of length exactly n*maxDeltaX.
   // It would be more intuitive, and may have been the original author's intent, to 
   // return n nodes of length maxDeltaX when the reach is exactly n*maxDeltaX long.
   // assert(m_length > 0);
   // int integer_quotient = (int)(m_length/maxDeltaX);
   // float remainder = m_length - integer_quotient*maxDeltaX;
   // subnode_count = (remainder > 0) ? (integer_quotient + 1) : integer_quotient;
...
int ReachTree::SaveTopology(LPCSTR filename)
   {
   FILE *fp;
