...
int DbTable::Load( LPCSTR databaseName, LPCSTR sql, BOOL bExclusive, BOOL bReadOnly, LPCTSTR lpszConnect )
   {
   int recordCount = 0;
...
int DbTable::OpenDB( LPCSTR databaseName, BOOL bExclusive, BOOL bReadOnly, LPCTSTR lpszConnect )
   {
   int recordCount = 0;
   m_databaseName = databaseName;
...
int DbTable::LoadDataDBF( LPCSTR path )
   {
#ifndef NO_MFC
   m_databaseName = path;
   m_tableName = path;
...
int DbTable::SaveDataDBF( LPCSTR path /*=NULL*/, BitArray *pSaveRecordsArray /*=NULL*/)
   {
#ifndef NO_MFC
   CString _path;
   if ( path == NULL )
...
int DbTable::LoadDataCSV( LPCSTR path )
   {
   m_databaseName = path;
   m_tableName = path;
...
int DbTable::AddField( LPCSTR name, TYPE type, bool addCol /*=true*/ )
   {
   int width, decimals;
...
int DbTable::AddField( LPCSTR name, TYPE type, int width, int decimals, bool addCol /*=true*/ )
   {
...
int DbTable::InsertField( int insertBefore, LPCSTR name, TYPE type )
   {
   int width, decimals;
   GetTypeParams( type, width, decimals );
...
int DbTable::InsertField( int insertBefore, LPCSTR name, TYPE type, int width, int decimals )
   {
     FIELD_INFO info;
   m_fieldInfoArray.Add( info );
...
int DbTable::RemoveField( LPCSTR name )
   {
