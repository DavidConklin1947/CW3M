...
//void QCompilerError( LPCSTR type, LPCSTR varName, LPCSTR setName );
...
bool QNode::GetLabel( CString &label, int options )
   ...
   switch( m_nodeType )
      ...
      case STRING:
         {
         LPCSTR value = m_value.GetAsString();
         ASSERT( value != NULL );
...
Query *QueryEngine::ParseQuery( LPCSTR buffer, int lineNo, LPCTSTR source )
   {
   _gpQueryEngine = this;
...
QFNARG QueryEngine::AddFunctionArgs(int functionID, int polyIndex)
   ...
   QArgList *pArgList = new QArgList;
   pArgList->Add(QArgument(functionID, polyIndex));
...
QFNARG QueryEngine::AddFunctionArgs( int functionID, QNode *pNode )
   ...
   QArgList *pArgList = new QArgList;
   pArgList->Add( QArgument( functionID, pNode ) );
...
QFNARG QueryEngine::AddFunctionArgs(int functionID, QNode *pNode0, QNode *pNode1)
   ...
   QArgList *pArgList = new QArgList;
   pArgList->Add(QArgument(functionID, pNode0));
   pArgList->Add(QArgument(functionID, pNode1));
...
LPCSTR QueryEngine::FindString( LPCSTR label, bool add )
   {
...
void QueryEngine::RuntimeError( LPCSTR _msg )
   {
   if ( m_showRuntimeError )
      {
#ifndef NO_MFC
      CString msg;
      msg.Format( "%s while processing query %s.  Select <Cancel> to disable this message...", _msg, (LPCSTR) m_pCurrentQuery->m_text );
...
int QueryEngine::AddError( int lineNo, LPCSTR msg ) 
   { 
