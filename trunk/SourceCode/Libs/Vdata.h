...
//      VData( DATE v   )  { type = TYPE_DATE;  val.vDate = v; }
      ...
      VData( LPCSTR v, bool alloc=true ); // The arg, alloc, selects TYPE_DSTRING or TYPE_STRING.
...
//      VData& operator = (DATE   v) { type = TYPE_DATE;   val.vDate   = v; return *this; }
      ...
      VData& operator = (LPCSTR v);  // note:  no allocation, to dynamically allocate, use AllocateString()
...
//      bool SetKeepType(DATE   v) { VData t(v); if (t.ChangeType(type)) {*this=t; return true;} return false;}
      bool SetKeepType(LPCSTR v) { VData t(v); if (t.ChangeType(type)) {*this=t; return true;} return false;}
   ...
   public:
      ...
      // set()'s
      void SetAsDString( LPCSTR value );
...
struct AFX_EXT_CLASS VDataLess
{
