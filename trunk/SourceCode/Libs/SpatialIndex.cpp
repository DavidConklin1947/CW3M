...
int SpatialIndex::WriteIndex( LPCSTR filename )
   {
   if ( m_indexCount <= 0 )
      return -1;
...
int SpatialIndex::ReadIndex( LPCSTR filename, MapLayer *pLayer, MapLayer *pToLayer, int includeRecordCount  )
   {
...
bool SpatialIndex::HasExtendedSpatialInfoEx(LPCSTR filename, int & errorCode)
   {
   errorCode = 0;
...
int SpatialIndex::GetSpatialIndexInfo( LPCSTR filename, float &distance, int &indexCount )
   {
   FILE *fp;
