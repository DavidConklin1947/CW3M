...
class  LIBSAPI DDataObj : public DataObj
   ...
   public:
      ...
      virtual bool Set( int col, int row, LPCSTR value ) { double v=atof( value ); return Set( col,row,v ); }
      ...
      virtual int  AppendCol( LPCSTR label ); //, float *dataArray /*=NULL*/ );
      virtual bool CopyCol( int toCol, int fromCol );
      virtual int  InsertCol( int insertBefore, LPCSTR label );
	...
	//  double IGet(double x, int xcol, int ycol/*= 1*/, IMETHOD method/*=IM_LINEAR*/, int startRow/*=0*/, int *returnRow/*=NULL*/, bool forwardSearch/*=true*/);
      //-- concatenation --//
      ...
      //-- File I/O --//
      int   ReadAscii ( LPCSTR fileName, TCHAR delimiter=0, BOOL showMsg=TRUE );
      int   ReadAscii ( HANDLE hFile,    TCHAR delimiter=0, BOOL showMsg=TRUE );

      int   WriteAscii( LPCSTR fileName, TCHAR delimiter=',', int colWidth=0 );
#ifndef NO_MFC
#ifndef _WIN64
      int   ReadXls(LPCSTR fileName);
#endif
#endif
   };
